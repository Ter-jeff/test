...
namespace Cautogen.AutoCZ.CharPreProcessor.InputReader
{
    ...
    {
        // data member
        public static Dictionary<string, PatternData> PatternList = new Dictionary<string, PatternData>();
        ...
        {
            LogHelper.Info("Reading Pattern List...");
        ...
        private static PatternData ReadRow(IDictionary<string, int> headerOrder, IList<string> strArray)
        {
            ...
            if (headerOrder.TryGetValue("TIMESET LATEST", out int value6))
            {
                patternData.TimesetVersion =
                    Path.GetFileNameWithoutExtension(strArray[value6]);
            }

            if (headerOrder.TryGetValue("TIMESET VERSION", out int value7))
            {
                patternData.TimesetVersion =
                    Path.GetFileNameWithoutExtension(strArray[value7]);
            }

            if (headerOrder.TryGetValue("FILE VERSIONS", out int value8))
            {
                patternData.FileVersion = strArray[value8].ToLower();
            }

            if (headerOrder.TryGetValue("OPCODE", out int value9))
            {
                patternData.OpCode = strArray[value9].ToLower();
            }

            if (headerOrder.TryGetValue("SCANMODE", out int value10))
            {
                patternData.ScanMode = strArray[value10].ToLower();
            }

            if (headerOrder.TryGetValue("HALT", out int value11))
            {
                patternData.Halt = strArray[value11].ToLower();
            }

            if (headerOrder.TryGetValue("ORIGINAL TIMING MODE", out int value12))
            {
                patternData.OriginalTimingMode = strArray[value12].ToLower();
            }

            if (headerOrder.TryGetValue("CHECK", out int value13))
            {
                patternData.Check = strArray[value13].ToLower();
            }
