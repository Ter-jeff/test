...
''            If Not EFUSE_IDS_Dic.Exists(MappingPin_ids) Then EFUSE_IDS_Dic.item(MappingPin_ids) = gl_IDS_INFO_Dic(MappingPin_ids)       ''Only store from Single Pin, pin group value will from Calc Sum
            If Not EFUSE_IDS_Dic.Exists(LCase(CStr(MappingPin))) Then EFUSE_IDS_Dic.item(LCase(CStr(MappingPin))) = gl_IDS_INFO_Dic(UCase(CStr(MappingPin)))       ''Only store from Single Pin, pin group value will from Calc Sum
