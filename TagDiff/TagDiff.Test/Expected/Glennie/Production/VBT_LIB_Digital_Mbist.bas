...
On Error GoTo errHandler
    ...
    If glb_TesterType = "Jaguar" Then
        ...
        '221019 T-col
        'Patgen halt modes "tlHaltOnFail", "tlHaltOnHRAMFull" and "tlHaltOnCMEMFull" have been deprecated for UFPLUS.
    End If
    ...
    MBIST_BLOCK = right(MBIST_BLOCK, Len(MBIST_BLOCK) - 5)
    'RSCR site loop
...
Public Function MbistRetentionLevelWait_new(mS_Time As Double, RampStep As Double, Optional RampWaitTime As Double = 0, Optional WaitTimeOnly As Boolean = False, _
                                        Optional RET_TTR As Boolean = False, Optional b_ReadSpec_Auto As Boolean = True, _
                                        Optional Ret_SRAMPins As PinList, Optional Ret_LogicPins As PinList, Optional OriCategory As String, Optional RampCategory As String)
    ...
    Else
        ...
        Else
            ...
            If b_ReadSpec_Auto Then
            'Add Start, Leon Li, 20210114, default assign value from DC spec for all core power pin
        ...
        '        Call Sub_VoltageRamping(Retention_Pins_Ary(), RampDown_Step, RampDown_Time, RampDown)
            ...
            Else
            ''Do for specified pin without doing all core power pin, from T-Col, 20230531
                If Ret_SRAMPins <> "" Or Ret_LogicPins <> "" Then
                    Call MbistRetentionLevelWait_SpecifiedPinGrp(mS_Time, Ret_SRAMPins, Ret_LogicPins, OriCategory, RampCategory, RampWaitTime, RampDown_Step)
                Else
                ''
                End If
            End If
...
Exit Function
errHandler:
    Call Print_Error_Message(Error_Info, "VBT_LIB_Digital_Mbist", "MbistRetentionLevelWait")
    If AbortTest Then Exit Function Else Resume Next
End Function
Public Function MbistRetentionLevelWait(mS_Time As Double, RampStep As Double, Optional RampWaitTime As Double = 0, Optional WaitTimeOnly As Boolean = False, _
                                        Optional RET_TTR As Boolean = False, Optional RET_Pins As String = "")
...
'        Dim lclPinListData_CurrentCorePower As New PinListData:: Set lclPinListData_CurrentCorePower = Nothing
        ...
        Else
            s_RET_Pins = Split(RET_Pins, ",")
            ReDim lclStrArr_Pins(UBound(s_RET_Pins))
            lclStrArr_Pins = s_RET_Pins
        End If
        ...
        For i = 0 To UBound(lclStrArr_Pins)
            If TheExec.DataManager.ChannelType(lclStrArr_Pins(i)) <> "N/C" Then
                pinType = SortPinInstrument(lclStrArr_Pins(i))
                lclPinListData_PreviousCorePower.AddPin lclStrArr_Pins(i)
                If Not pinType Like "*VS*" Then
                    lclPinListData_PreviousCorePower.pins(lclStrArr_Pins(i)).value = Format(TheHdw.DCVI.pins(lclStrArr_Pins(i)).Voltage.value, "0.000")
                Else
                    lclPinListData_PreviousCorePower.pins(lclStrArr_Pins(i)).value = Format(TheHdw.DCVS.pins(lclStrArr_Pins(i)).Voltage.value, "0.000")
                End If
                lclPinListData_CurrentCorePower.AddPin lclStrArr_Pins(i)
                lclPinListData_CurrentCorePower.pins(lclStrArr_Pins(i)).value = Format(TheExec.Specs.DC.item(Replace(Replace(lclStrArr_Pins(i), "_DCVI", ""), "_DCVS", "") & "_VAR").Categories(lclStr_DcCategory).Selectors(lclStr_DcSelector).ContextValue, "0.000")
...
'        Call Sub_VoltageRamping(Retention_Pins_Ary(), RampDown_Step, RampDown_Time, RampDown)
    ...
    If TheExec.enableWord("ZZ_OL_Check_DC_Spec") = True And TheExec.TesterMode = testModeOffline Then
    
        Dim DCCategory As String
        Dim DCSelector As String
        Dim Temp_DCSelector As String
        Dim PatternName_Output As String: PatternName_Output = "N/A"
        Dim CurrentInst_Name As String
        
        Call TheExec.DataManager.GetInstanceContext(DCCategory, DCSelector, "", "", "", "", "", "")
        CurrentInst_Name = TheExec.DataManager.instancename
        
        If UCase(DCSelector) Like "MAX" Then
           Temp_DCSelector = "HV"
        ElseIf UCase(DCSelector) Like "MIN" Then
           Temp_DCSelector = "LV"
        ElseIf UCase(DCSelector) Like "TYP" Then
           Temp_DCSelector = "NV"
        Else
           TheExec.ErrorLogMessage "DC Selector not found"
        End If
          
        If LCase(CurrentInst_Name) Like "*waittimeonly*" Then
            TheExec.Datalog.WriteComment "<offline debug> DC Spec:" + CurrentInst_Name + ":" + PatternName_Output + ":" + DCCategory + " " + Temp_DCSelector
        Else
            TheExec.Datalog.WriteComment "<offline debug> DC Spec:" + CurrentInst_Name + ":" + PatternName_Output + ":" + DCCategory + " " + Temp_DCSelector
            TheExec.Datalog.WriteComment "<offline debug> DC Spec:" + CurrentInst_Name + ":" + PatternName_Output + ":" + DCCategory + " " + Temp_DCSelector
        End If
...
'    MBISTFAILBLOCKSHEETS(2) = "MBISTFailBlock_SOC"
    ...
    If Flag_MBISTFailBlock_INIT = False Then
        MFP_TableIndex.RemoveAll
        MFP_TableIndex.compareMode = 1
            ...
            For k = 1 To MaxRow - 1
               ...
               ' If arr1(2, 1) = "" Then Exit For
                If (k = 1) And (sheetIdx = 0) Then
                    ...
                    tpCycleBlockInfor(m).strPattName = PatName
                    tpCycleBlockInfor(m).Dic_VectorIdx.RemoveAll
                    tpCycleBlockInfor(m).Dic_CycleIdx.RemoveAll
                Else
                    If tpCycleBlockInfor(m).strPattName = PatName Then
                        If n > 1800 Then
                ...
                If Not MFP_TableIndex.Exists(tpCycleBlockInfor(m).strPattName) Then
                    MFP_TableIndex.Add tpCycleBlockInfor(m).strPattName, m
                End If
                ...
                'tpCycleBlockInfor(m).tpMbistCycleBlock(n).Dic_VectorIdx.Add Int(arr1(k, 3)), n
                If Not tpCycleBlockInfor(m).Dic_VectorIdx.Exists(Int(arr1(k, 3))) Then
                    tpCycleBlockInfor(m).Dic_VectorIdx.Add Int(arr1(k, 3)), n
                End If
                tpCycleBlockInfor(m).tpMbistCycleBlock(n).lCycle = Int(arr1(k, 4))
                'tpCycleBlockInfor(m).tpMbistCycleBlock(n).Dic_CycleIdx.Add Int(arr1(k, 4)), n
                If Not tpCycleBlockInfor(m).Dic_CycleIdx.Exists(Int(arr1(k, 4))) Then
                    tpCycleBlockInfor(m).Dic_CycleIdx.Add Int(arr1(k, 4)), n
                End If
