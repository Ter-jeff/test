...
#If ApplyBinCut Then
    ...
    '20211222, Modify for sram bincut dynamic structure
    If Flag_IDS_Mapping_enable = True Then
        For i = cntVddbinPin To VddbinPmodeDict.Count
            If LCase(VddbinPmodeDict.Keys(i)) Like LCase(AllBinCut(VddBinStr2Enum(fieldname_mod)).IDS_MAPPING) & "*" Then
                Set idsField = BdfDataBase.DicVddBinIdsFieldMap(LCase(VddbinPmodeDict.Keys(i)))
                Exit For
            End If
        Next i
    Else
...
#If ApplyBinCut Then
    ...
    If BdfDataBase.DicVddBinIdsFieldMap.Exists(fieldname_mod) Then
        ...
        If result = 999 Then
'            VBIN_RESULT(m_Pmode).GRADE = 0
            GRADEVDD = 0
            GlbUtility.WriteErrDlg "The EQN Number Can Not Be Found!!"
        End If

'        VBIN_RESULT(m_Pmode).GRADEVDD = m_GRADEVDD
    Else
...
End If
        ...
        For Each fieldStr In bank.Fields
            Set field = bank.Fields(fieldStr)
                If (field.Algorithm = alg_vddbin And Not LCase(field.name) Like "*_shadow") Then
                    If Not VddbinPmodeDict.Exists(UCase(Replace(CStr(fieldStr), "_shadow", ""))) Then
                    ...
                    Else
                        m_Pmode = VddBinStr2Enum(Replace(CStr(fieldStr), "_shadow", ""))
                        If BdfDataBase.DicVddBinPmodeMap.Exists(LCase(fieldStr)) Then
                            Call Print_Error_Message(Error_Info, "VBT_ZeFuseVddBin_Ids_Glb", "GetVddBinPmodeMap", "Duplicated category existed! " & bankstr & ">>" & fieldStr)
                        Else
                            BdfDataBase.DicVddBinPmodeMap.Add LCase(Replace(CStr(fieldStr), "_shadow", "")), m_Pmode
                            BdfDataBase.DicVddBinBankMap.Add LCase(Replace(CStr(fieldStr), "_shadow", "")), bankstr
                        End If
                                ...
                                If bank.HadVddBinFuse Then
                                           For Each fieldStr In bank.Fields.Keys
                                                    ...
                                                    ElseIf field.Algorithm = alg_ids And _
                                                            LCase(field.name) Like "ids_" & LCase(powerPin) & "*" And UCase(field.BlowLocation) = "CP1" Then
                                                        Set GetMapIdsField = field
                                                        Exit Function
                                                    End If
                                           Next
