...
Public Function MTR_TDiode_Test(Optional TestSequence As String, Optional MeasV_PinS As String, Optional MeasI_PinS As String, Optional MeasI_Range As String, _
                            Optional Interpose_PreCondition As String, Optional Interpose_PreMeas As String, Optional Interpose_PostTest As String, Optional ForceV_Val As String, Optional ForceI_Val As String, _
                            Optional WaitTime_VIRZ As String, Optional Meas_StoreName As String, Optional Calc_Eqn As String, Optional Validating_ As Boolean) As Long
...
''        Call PrLoadPattern(patSet.value)
    ...
    End If
    ''TestSequence = "V,I"
    ''MeasV_PinS = "MTR_TD_ES, MTR_TD_BS + "
    ''MeasI_pinS = "+ MTR_TD_B"
    ''MeasI_Range
    ''Interpose_PreCondition = "MTR_TD_B:SetupFV:0.1,0.002,0.01"
    ''Interpose_PreMeas = "MTR_TD_E:SetupFI:0.00002|0.00003|0.00004|0.00005|0.00006|0.00008|0.0001|0.00012|0.00014|0.00016|0.00018|0.0002|0.00024|0.00028|0.00032|0.00036|0.0004|0.00048|0.00052|0.00056|0.0006"
        ''Interpose_PreMeas = "20uA, 30uA, 40uA, 50uA, 60uA, 80uA, 100uA, 120uA, 140uA, 160uA, 180uA, 200uA, 240uA, 280uA, 320uA, 360uA, 400uA, 440uA, 480uA, 520uA, 560uA and 600uA "
    ''Interpose_PostTest = "MTR_TD_B:restoredcvi:0.002,0.002,0.002"
    ''ForceV_Val = " | 0"
    ''ForceI_Val
    ''WaitTime_VIRZ = "0.01"
    ''Meas_StoreName
    ...
    Dim s_InstName As String
    Dim s_temp_Ary() As String
    Dim s_TestSeq_Ary() As String
    Dim s_MeasVPinS_Ary() As String
    Dim s_MeasIpinS_Ary() As String
    Dim s_InterposePreMeas_Ary() As String
    
    Dim s_MeasurementPin_Ary() As String
    
    Dim s_InterposePreMeas As String
    
    glb_TestInstance = ""
    glb_TestInstance = UCase(TheExec.DataManager.instancename)

    If InStr(TestSequence, ",") <> 0 Then
        s_TestSeq_Ary = Split(TestSequence, ",")
    Else
        ReDim s_TestSeq_Ary(0)
        s_TestSeq_Ary(0) = TestSequence
    End If
    
    TheHdw.Digital.ApplyLevelsTiming True, True, True, tlPowered
    
''    If InStr(MeasV_PinS, "+") <> 0 Then
''        s_MeasVPinS_Ary = Split(MeasV_PinS, "+")
''    Else
''        ReDim s_MeasVPinS_Ary(0)
''        s_MeasVPinS_Ary(0) = MeasV_PinS
''    End If
''
''    If InStr(MeasI_PinS, "+") <> 0 Then
''        s_MeasIpinS_Ary = Split(MeasI_PinS, "+")
''    Else
''        ReDim s_MeasIpinS_Ary(0)
''        s_MeasIpinS_Ary(0) = MeasI_PinS
''    End If
''    If theexec.TesterMode = testModeOffline Then
        'Mason DBG
        'thehdw.DCVI.Pins("MTR_TD_ES,MTR_TD_BS,MTR_TD_B,MTR_TD_E").Alarm(tlDCVIAlarmOverRange) = tlAlarmOff
        'thehdw.DCVI.Pins("MTR_TD_ES,MTR_TD_BS,MTR_TD_B,MTR_TD_E").Alarm(tlDCVIAlarmMode) = tlAlarmOff
        'thehdw.DCVI.Pins("MTR_TD_ES,MTR_TD_BS,MTR_TD_B,MTR_TD_E").Alarm(tlDCVIAlarmCapture) = tlAlarmOff
        'Mason DBG
''    End If
    If Interpose_PreCondition <> "" Then ''Interpose_PreCondition = "MTR_TD_B:SetupFV:0.1,0.002,0.01"
        Call SetForceCondition(Interpose_PreCondition)
    End If
    
    Call ProcessInputToGLB(, TestSequence, , , , , , MeasV_PinS, , , , , , , MeasI_PinS, MeasI_Range, , , , , , , , , , , , , , , , , , , , , , , , , , , Meas_StoreName, Calc_Eqn, , , , , ForceV_Val, ForceI_Val, , , WaitTime_VIRZ)

    If Interpose_PreMeas <> "" Then ''Interpose_PreMeas = "MTR_TD_E:SetupFI:0.00002|0.00003|0.00004|0.00005|0.00006|0.00008|0.0001|0.00012|0.00014|0.00016|0.00018|0.0002|0.00024|0.00028|0.00032|0.00036|0.0004|0.00048|0.00052|0.00056|0.0006"
        If InStr(Interpose_PreMeas, "|") <> 0 Then
            s_InterposePreMeas_Ary = Split(Interpose_PreMeas, "|")
        Else
            ReDim s_InterposePreMeas_Ary(0)
            s_InterposePreMeas_Ary(0) = Interpose_PreMeas
        End If
        For i = 0 To UBound(s_InterposePreMeas_Ary)
''thehdw.Utility("K168").State = tlUtilBitOn
            If i = 0 Then
                Call SetForceCondition(s_InterposePreMeas_Ary(0) & ";STOREPREMEAS")
            Else
                s_temp_Ary = Split(s_InterposePreMeas_Ary(0), ":")
                s_temp_Ary(2) = s_InterposePreMeas_Ary(i)
                s_InterposePreMeas = Join(s_temp_Ary, ":")
                Call SetForceCondition(s_InterposePreMeas & ";STOREPREMEAS")
            End If

            For Ts = 0 To UBound(s_TestSeq_Ary)
                TestSeqNum = 0
                If UCase(s_TestSeq_Ary(Ts)) = "V" Then
                    Call HardIP_MeasureVolt
                    'Mason DBG
'                    thehdw.Alarms.Check
'                    theexec.Datalog.WriteComment "Meas V Pin: " & s_InterposePreMeas_Ary(i)
                End If
                
                If UCase(s_TestSeq_Ary(Ts)) = "I" Then
                    Call HardIP_MeasureCurrent
                    'Mason DBG
'                    thehdw.Alarms.Check
'                    theexec.Datalog.WriteComment "Meas I Pin: " & s_InterposePreMeas_Ary(i)
                End If
    ...
    If Interpose_PostTest <> "" Then ''Interpose_PreCondition = "MTR_TD_B:SetupFV:0.1,0.002,0.01"
...
''    thehdw.DCVI.Pins("MTR_TD_ES,MTR_TD_BS,MTR_TD_B,MTR_TD_E").Alarm(tlDCVIAlarmOverRange) = tlAlarmForceFail
''    thehdw.DCVI.Pins("MTR_TD_ES,MTR_TD_BS,MTR_TD_B,MTR_TD_E").Alarm(tlDCVIAlarmMode) = tlAlarmForceFail
''    thehdw.DCVI.Pins("MTR_TD_ES,MTR_TD_BS,MTR_TD_B,MTR_TD_E").Alarm(tlDCVIAlarmCapture) = tlAlarmForceFail
''    thehdw.Alarms.Check
Exit Function
errHandler:
    TheExec.Datalog.WriteComment "error in MTR_Diode_Test"
'    Resume Next
    If AbortTest Then Exit Function Else Resume Next
End Function
...
Public Function MetrologyICTS_DAP_PN_TRIM(Optional pat As Pattern, Optional DigCap_Pin As PinList, Optional Cap_Trimwidth As Long, Optional DigCap_Sample_Size As Long, Optional DigSrc_pin As PinList, Optional DigSrc_Sample_Size As String, Optional DigSrc_Equation As String, Optional DigSrc_Assignment As String, Optional CUS_Str_MainProgram As String = "", Optional CUS_Str_DigCapData As String = "", Optional TrimStoreName As String, Optional TrimTarget As Double, Optional TrimStart As String, Optional TrimCodeSize As Long, Optional Calc_Eqn As String, Optional Interpose_PrePat As String, Optional TrimMethod As String, Optional Trimming_Direction_Increase As Boolean, Optional Validating_ As Boolean)
    ...
    For j = Trim_step To -1 Step -1
        ...
        If Trim_Flag Then
            ...
            For Each site In TheExec.sites.Active
                'trim_Wf = OutWf_2s.divide(2 ^ 16)
                trim_Wf = OutWf_2s.divide(2 ^ 11)
                For i = 0 To TestSequenceNumber - 1
...
errHandler:
    TheExec.Datalog.WriteComment "error in MetrologyICTS_DAP_PN_TRIM"
    If AbortTest Then Exit Function Else Resume Next
End Function
