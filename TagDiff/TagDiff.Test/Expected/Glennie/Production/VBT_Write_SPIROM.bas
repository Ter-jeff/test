...
On Error GoTo errHandler
    ...
    Else
        ...
        DspArray(0) = EraseCMD
    End If
    TheExec.Datalog.WriteComment ("SPIROM Bulk Erase in process...")
...
Public Function RTOS_Parse_Info()
    '========= Print binary ver. if Sector/BulkErase enable word is off ===========

    Dim RomFileName1 As String
    If UCase(currentJobName) Like "CP*" Then
        RomFileName1 = Worksheets("SpiromCodeFile").Cells(1, 2).value
    ElseIf UCase(currentJobName) Like "WLFT*" Then
        RomFileName1 = Worksheets("SpiromCodeFile").Cells(1, 2).value
    ElseIf UCase(currentJobName) Like "FT*" Then
        RomFileName1 = Worksheets("SpiromCodeFile").Cells(2, 2).value
    Else
        RomFileName1 = "\NA"
    End If
    
    Dim path_decompose_ary() As String
        
    If LCase(RomFileName1) Like "*\*" Then
        path_decompose_ary = Split(RomFileName1, "\")
        gls_Binary_Ver = path_decompose_ary(UBound(path_decompose_ary))
    ElseIf LCase(RomFileName1) Like "*/*" Then
        path_decompose_ary = Split(RomFileName1, "/")
        gls_Binary_Ver = path_decompose_ary(UBound(path_decompose_ary))
    End If
