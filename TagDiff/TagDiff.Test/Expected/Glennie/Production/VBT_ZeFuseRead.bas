...
Public Function Bank_SingleDoubleBitsCheck(opbank As eFuseBdfBank, Optional printdecode As Boolean, Optional Validating_ As Boolean)
On Error GoTo errHandler
    Dim result As New SiteLong
Dim nBits As Long
Dim ParallelWave As New DSPWave
    Dim out1 As New DSPWave, out2 As New DSPWave '20220803, Add for single double bit debug print
Dim site As Variant 'Carter, 20240304
    ''202004xx for ap
    'If Not opbank.DoubleBits Then GoTo errHandler: 'Not double bits, can't do the check!!
    If opbank.pgmMode = pgm_DAA Then
        Set result = DoubleBitChecks(opbank.DaaCapturedWave, out1, out2) '20220803, Add for single double bit debug print
    Else
        ''202004xx for ap
        ...
        If nBits = 2 Then
            ...
            Set result = DoubleBitChecks(ParallelWave, out1, out2) '20220803, Add for single double bit debug print
        Else
            '1-bit case
...
GlbUtility.TimerStarter
    ...
    If opbank.IsExistMultiTypeKey(TheExec.DataManager.instancename, instNameKey) Or MultitypJob <> "" Then
        ...
        End If
    End If
    If PseudoFuseEnable And Not MixPseudoFuseEnable Then printdecode = True
    ...
    If PseudoFuseEnable And Not MixPseudoFuseEnable Then
        ...
        End If
        If opbank.DoubleBits = True Then
            Bank_SingleDoubleBitsCheck opbank, printdecode
        End If
    Else
        '20211214, Modify for ECID JTAG Read decode
        If False = opbank.NeedJTAGRead And opbank.DoubleBits = True Then
            Bank_SingleDoubleBitsCheck opbank
        End If
    End If
...
'                                            Next
                                    ...
                                    Case Algorithm.alg_vddbin
                                       If field.BlowLocation = GlbUtility.currStage Or GlbUtility.testedStages.Exists(field.BlowLocation) Then
                                            ...
                                            If LCase(field.name) Like "*_shadow" And field.FuseMeasureValue = 0 Then
                                                oValue(iItem) = field.FuseMeasureValue
                                                B_Hlimit(iItem) = 0: B_Llimit(iItem) = 0
                                            Else
            ...
            If gB_eFuse_Disable_ChkLMT_Flag Then
                If Disable_chk_arr(i) = 0 Then
                    If LoopLimitPrint(i) = True Then
                        ...
                        Next
                                ElseIf (gB_notUseHiLimit And oHlimit(i) = -999) Then
...
Dim dicLotInfo As New Dictionary
    ...
    For i = 0 To UBound(fusebankarr)
        ...
        If opbank.HadDeidFuse Then  'And (GlbUtility.testedStages.Count = 0 Or (field.BlowLocation = TheExec.CurrentJob)) Then
...
Dim fieldstr_ As Variant
        ...
        If (LCase(opbank.name) = "config" Or LCase(opbank.name) = LCase(CfgBankName)) And opbank.HadEarlyFuse Then
                ...
                End If
                If bLumpStages Then
                'If GlbUtility.testedStages.Count > 0 Or bLumpStages Then
