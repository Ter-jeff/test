...
Public Function UVI80_Continuity(digital_pins As PinList, force_i As Double, LowLimit As Double, hiLimit As Double, TestLimitMode As tlLimitForceResults, Optional PN_Disconnect As Boolean = False, _
                                        Optional Separate_limit As Boolean = False, Optional LowLimit2 As Double, Optional HiLimit2 As Double, Optional connect_all_pins As PinList, _
 ...
 On Error GoTo errHandler
    ...
    'Dim Flag_Short As String: Flag_Short = "F_short"
    'Dim Flag_Open As String: Flag_Open = "F_open"
    Dim PinNameCustomize As String
    Dim TmpTname As String
...
'''        Dim Tname_Prefix As String
'''        Dim Tname_Suffix As String
'''        Call Continuity_Tname_Format(CStr(digital_pins), Tname_Prefix, Tname_Suffix)
'''    End If
''''' ------  20191218 -- JIM,CCPANJ
...
''    End With
    ...
    '''                TheHdw.DCVI.Pins("MyPin").Connect tlDCVIConnectHighSense
                ...
                If LCase(GetInstrument(CStr(DUTPin), 0)) = "dc-07" Then
                    PPMUMeasure.pins(DUTPin) = TheHdw.DCVI.pins(DUTPin).Meter.Read(tlStrobe, 1, 1000000)
                Else
                    PPMUMeasure.pins(DUTPin) = TheHdw.DCVI.pins(DUTPin).Meter.Read(tlStrobe, 1) '''only for DC30
                End If
                
                'reset
                With TheHdw.DCVI.pins(DUTPin)
                    .Current = 0#
                    ''20210507 Avoid next instance mode alarm
                    '.mode = tlDCVIModeVoltage          ''aaaa
                    .Gate = False
                    .Disconnect tlDCVIConnectDefault
                End With
            
            ElseIf glb_TesterType = "UltraFLEXplus" Then
                If TheExec.DataManager.ChannelType(DUTPin) Like "*DCVS*" Then
                    'opensocket alarm off
                    'thehdw.DCVS.Pins(DUTPin).Alarm(tlDCVSAlarmOverrange) = tlAlarmOff
                    'thehdw.DCVS.Pins(DUTPin).Alarm(tlDCVSAlarmMode) = tlAlarmOff
'                        ''for DCVI pins change to DCVS inst, connect first to avoid without applylevelstiming
'                        With TheHdw.DCVS.Pins(DUTPin)
'                            .Voltage.Value = 0#
'                            .Connect
'                            .Gate = tlOn
'                        End With
'                        Call p2p_short_Power_FVMI(temp_PinList, 0.2, -0.001, 0.1, tlForceNone, 0, DigitalPin)
                     With TheHdw.DCVS.pins(DUTPin)
                        .Disconnect
                        ...
                        Else
                                ...
                                .Voltage.value = -1 - 0.5
                        End If
                        .CurrentRange.value = Abs(force_i)
                        .CurrentLimit.Source.FoldLimit.level.value = Abs(force_i)
                        .CurrentLimit.Sink.FoldLimit.level.value = Abs(force_i)
                        ...
                        .Connect
                                ...
                                .mode = tlDCVSModeCurrent
                                                End If
                    End With
                    TheHdw.Wait (0.005) 'follow Cyprus to do DC TTR by KWLU 180601
                    
                    
                    PPMUMeasure.pins(DUTPin) = TheHdw.DCVS.pins(DUTPin).Meter.Read(tlStrobe)
                    
                    With TheHdw.DCVS.pins(DUTPin)
                        '.Current = 0# 'UltraFlex+ DCVS not have .current setting / need double check
                        .mode = tlDCVSModeHighImpedance
                        .Gate = False
                        .Disconnect
                    End With
        ...
        If TheExec.DataManager.ChannelType(DUTPin) <> "N/C" Then
            ...
            ElseIf TestLimitMode = tlForceNone Then
                If gB_Enable_AP Or gB_Enable_RF Then
                    'Hlimit = HiLimit2
                    'Llimit = LowLimit2
                    'If HiLimit2 = 0 And LowLimit2 = 0 Then Llimit = LowLimit: Hlimit = HiLimit
                    TheExec.flow.TestLimit resultVal:=PPMUMeasure.pins(DUTPin), lowVal:=LowLimit, hiVal:=hiLimit, scaletype:=scaleNone, unit:=unitVolt, formatStr:="%.3f", Tname:=Tname, ForceVal:=force_i, ForceUnit:=unitAmp, ForceResults:=tlForceNone
                    'Judge failed open or failed short for tlForceNone
                    Call GlbCustomizeSet.SetContiFlagOnelimit(hiLimit, LowLimit, PPMUMeasure.pins(DUTPin), Flag_Open, Flag_Short)
...
'''        TnameFormatStr = Tname_Prefix & Replace(CStr(DUTPin), "_", "") & Tname_Suffix & CStr(pinsequence) '' 20191218 -- JIM,CCPANJ
'''        If TestLimitMode = tlForceFlow Then
'''            If Enable_Conti_Tname_Format Then ''Continuity Tname "HAC" Format '' 20191218 -- JIM,CCPANJ
'''                TheExec.Flow.TestLimit resultVal:=PPMUMeasure.Pins(DUTPin), scaletype:=scaleNone, unit:=unitVolt, formatStr:="%.3f", Tname:=TnameFormatStr, ForceVal:=force_i, ForceUnit:=unitAmp, ForceResults:=tlForceFlow
'''            Else '' Original
...
'''            If Enable_Conti_Tname_Format Then ''Continuity Tname "HAC" Format '' 20191218 -- JIM,CCPANJ
'''                TheExec.Flow.TestLimit resultVal:=PPMUMeasure.Pins(DUTPin), lowVal:=LowLimit2, hiVal:=HiLimit2, scaletype:=scaleNone, unit:=unitVolt, formatStr:="%.3f", Tname:=TnameFormatStr, ForceVal:=force_i, ForceUnit:=unitAmp, ForceResults:=tlForceNone
'''            Else '' Original
'''                TheExec.Flow.TestLimit resultVal:=PPMUMeasure.Pins(DUTPin), lowVal:=LowLimit2, hiVal:=HiLimit2, scaletype:=scaleNone, unit:=unitVolt, formatStr:="%.3f", Tname:=Tname, ForceVal:=force_i, ForceUnit:=unitAmp, ForceResults:=tlForceNone
'''            End If
'''
...
'''    End If
    ...
    Exit Function
errHandler:
        TheExec.Datalog.WriteComment "Error in Continuity"
...
Public Function p2p_short_Power_FVMI(allPowerPins As String, _
                     ...
                     Optional FixCurrentRange As String, Optional MeasStorePins As String) As Long
On Error GoTo errHandler
   ''++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    'michael for dram
    Dim Arr_MeasStorePins() As String
    ''--------------
    ''Datalog Format
    ''--------------
    Set GlbCustomizeSet = New CustomizeSetting
...
''                    If Not A_Slot_Type = "hexvs" And val < 0.0002 Then val = val * 10
''                    If A_Slot_Type = "hexvs" And val < 0.1 Then val = 0.1 ''[20240605] HEXVS Bin999 issue
''                    If val < 0.0002 Then val = 0.0002 ' add for T-Tah minimun current range
...
'                            If TheExec.DataManager.instancename Like "POWER_SHORT_ALL_UVS256_25C_WLFT*" Then
'                            If Not (Pin_Ary(i) = "vddio12_aop" Or Pin_Ary(i) = "vdd12_usb_debug") Then
'                            val = val * 2
'                            End If
'                            End If
                            
                            If Pin_Ary(i) = "vddio12_aop" Then
                                TheHdw.Wait 0.01
'                            If Pin_Ary(i) = "vddio12_aop" Or Pin_Ary(i) = "vdd12_usb_debug" Then
'                                val = val * 100
                            End If
                            
                            
                            If Not LCase(A_Slot_Type) = "hexvs" And val < 0.0002 Then val = val * 10
                            If LCase(A_Slot_Type) = "hexvs" And val < 0.1 Then val = 0.1 ''[20240605] HEXVS Bin999 issue
                            If val < 0.0002 Then val = 0.0002 ' add for T-Tah minimun current range

'                            If Pin_Ary(i) = "vdd1" Or Pin_Ary(i) = "vdd2" Then
'                                val = val * 2
'                            End If
''
                            'Fix V03A FT2 VDD_SOC power short alarm issue
                            If UCase(TheExec.DataManager.instancename) = "POWER_SHORT_ALL_HEXVS_85C_FT" And UCase(Pin_Ary(i)) = "VDD_SOC" Then
                             val = "1"
                            End If
                            
                            Call SetCurrentRange_CRWT(LCase(Pin_Ary(i)), val, WaitTime)
                            
'                            If Pin_Ary(i) = "vdd_pcpu" Or Pin_Ary(i) = "vdd_soc" Or Pin_Ary(i) = "vddio12_aop" Then
'                                thehdw.DCVS.Pins(Pin_Ary(i)).CurrentRange = 0.2
'                            End If
                           
                            
                        Case glbConstIns_DC07, glbConstIns_DC30:
                            If Not LCase(A_Slot_Type) = "hexvs" And val < 0.0002 Then val = val * 10
                            If LCase(A_Slot_Type) = "hexvs" And val < 0.1 Then val = 0.1
                            If val < 0.0002 Then val = 0.0002
                        Case Else:
                            s_ErrorMsg = "Use power pin:" & Pin_Ary(i) & "No support!!"
                            Call Print_Error_Message(Error_Warning_Info.Warning_Info, "VBT_LIB_DC_Conti", sFuncName, s_ErrorMsg)
                    End Select
                If i <> 0 Then TheHdw.DCVS.pins(Pin_Ary(i - 1)).Connect
                If F_p2pWT And (Not LCase(A_Slot_Type) = "hexvs") Then
                    TheHdw.Wait WaitTime
                    TheExec.Datalog.WriteComment Pin_Ary(i) & " wait time = " & WaitTime
                Else
...
''                        TheHdw.Wait 0.01   '20230406 add set current range wait time  '240607
''
''                        If (UCase(Pin_Ary(i)) = "VDDIO12_AOP") Or (UCase(Pin_Ary(i)) = "VDDIO12_GRP") Then
''                            TheHdw.Wait 0.01
''                        End If
''
                        End If
                    ...
                    TheHdw.DCVS.pins(Pin_Ary(i)).Disconnect
                    TheHdw.DCVS.pins(Pin_Ary(i)).Voltage.Main.value = 0#
                    'thehdw.DCVS.Pins(Pin_Ary(i)).Connect
...
''                    End If
                    ...
                    pinsequence = pinsequence + 1
                    'michael for dram
                    If MeasStorePins <> "" Then
                        If glb_PowerShort.ForceCondition.Exists(Pin_Ary(i)) Then
                            glb_PowerShort.ForceCondition.Remove (Pin_Ary(i))
                            glb_PowerShort.InstName.Remove (Pin_Ary(i))
                        Else
                        End If
                        glb_PowerShort.ForceCondition.Add (Pin_Ary(i)), CDbl(ForceV)
                        glb_PowerShort.InstName.Add (Pin_Ary(i)), TmpTname
            ...
            ElseIf (p_dc07 <> "" Or p_dc30 <> "") Then
                If (p_dc07 <> "") Then
                    ...
                    '=========================================================================
                    If GroupNum > 0 Then
                        TmpTname = Replace(CustomizeTname, "<PinName>", Pin_Ary(0))
        ''                UVI80MeasRangeList = TheHdw.DCVI.Pins(DC07_Power_data.Pins(i).Name).Meter.CurrentRange.List
'                        If StepVoltageMode = False Then
'                            ForceV = ForceValue(Pin_dcvi_Ary(GroupNum))
'                        End If
                        TheHdw.DCVI.pins(Pin_dcvi_Ary(0)).Voltage = ForceV
                    
                        TheHdw.DCVI.pins(DC07_Power_data.pins(GroupNum).name).CurrentRange.Autorange = True
                        TheHdw.Wait 2 * ms
                        DC07_Power_data.pins(0).value = TheHdw.DCVI.pins(DC07_Power_data.pins(GroupNum).name).Meter.Read(tlStrobe, 10, , tlDCVIMeterReadingFormatAverage)
                        TheHdw.DCVI.pins(Pin_dcvi_Ary(0)).Voltage = 0
                        
                        'offline mode simulation
                        If TheExec.TesterMode = testModeOffline Then
                            For Each site In TheExec.sites
                                DC07_Power_data.pins(Pin_dcvi_Ary(GroupNum)).value(site) = 0.01 + Rnd() * 0.0001
                            Next site
                        End If
            ...
            ''                UVI80MeasRangeList = TheHdw.DCVI.Pins(DC07_Power_data.Pins(i).Name).Meter.CurrentRange.List
                    ...
                    For i = 0 To UBound(Pin_dcvi_Ary)
                        ...
                        If MeasStorePins <> "" Then
                            ...
                            If glb_PowerShort.ForceCondition.Exists(Pin_dcvi_Ary(i)) Then
                                glb_PowerShort.ForceCondition.Remove (Pin_dcvi_Ary(i))
                                glb_PowerShort.InstName.Remove (Pin_dcvi_Ary(i))
                            Else
                            End If
                            glb_PowerShort.ForceCondition.Add (Pin_dcvi_Ary(i)), CDbl(ForceV)
                            glb_PowerShort.InstName.Add (Pin_dcvi_Ary(i)), TmpTname
            ...
            '---------------------------------------
            If GroupNum > 0 Then
                sPin = LCase(Pin_Ary(0))
                If sPin = "vdd_pcpu" Or sPin = "vdd_soc" Or sPin = "vddio12_aop" Or sPin = "vdd_gpu" Then
                Else
                    If LCase(gl_GetInstrumentType_Dic(sPin)) Like "*dcvs*" Then
                        TheHdw.DCVS.pins(sPin).CurrentRange = d_OrgSource_PinCurrentRange(0)
                    ElseIf UCase(gl_GetInstrument_Dic(sPin)) = glbConstIns_DC07 Or UCase(gl_GetInstrument_Dic(sPin)) = glbConstIns_DC30 Then
                        TheHdw.DCVI.pins(sPin).CurrentRange = d_OrgSource_PinCurrentRange(0)
                    End If
               End If
            Else
                For i = 0 To UBound(Pin_Ary)
    ...
    '---------------------------------------
    If MeasStorePins <> "" Then
        Arr_MeasStorePins = Split(MeasStorePins, ",")
        For i = 0 To UBound(Arr_MeasStorePins)
             sPin = LCase(Arr_MeasStorePins(i))
             If gl_GetInstrument_Dic.Exists(sPin) Then

                A_Slot_Type = UCase(gl_GetInstrument_Dic(sPin))
                'classfy power pin
                If glb_PowerShort.result.Exists(sPin) Then
                    glb_PowerShort.result.Remove (sPin)
                    TheExec.Datalog.WriteComment "glb_PowerShort Over write " & sPin & " value"
                Else
                End If
                
                Select Case A_Slot_Type
                    Case glbConstIns_HEXVS:       glb_PowerShort.result.Add (sPin), HexVS_Power_data.pins(sPin)
                    Case glbConstIns_VSM:         glb_PowerShort.result.Add (sPin), VSM_Power_data.pins(sPin)
                    Case glbConstIns_VHDVS:       glb_PowerShort.result.Add (sPin), UVS_Power_data.pins(sPin)
                    Case glbConstIns_VS5A:        glb_PowerShort.result.Add (sPin), UVS64_Power_data.pins(sPin)
                    Case glbConstIns_VS800MA:     glb_PowerShort.result.Add (sPin), UVS256hp_Power_data.pins(sPin)
                    Case glbConstIns_DC07:        glb_PowerShort.result.Add (sPin), DC07_Power_data.pins(sPin)
                    Case glbConstIns_DC30:        glb_PowerShort.result.Add (sPin), DC30_Power_data.pins(sPin)
                End Select
            End If
        Next i
    End If
...
Public Function PPMU_Measure_Contact_Resistance_Corner_Vss(digital_pins As PinList, force_i_S As Double, force_i_E As Double, force_i_Step As Double, LowLimit As Double, hiLimit As Double, TestLimitMode As tlLimitForceResults, PairNum As Double, Optional Separate_limit As Boolean = False, Optional independt_meas As Boolean) As Long
    ...
    Dim PPMUMeasure As New PinListData
    Dim site As Variant
    Dim MeasV_Force50mA As New PinListData
    Dim MeasV_Force0mA As New PinListData
    Dim Calculate_Contact_R As New PinListData
    Dim pld_temp_R As New PinListData
    ...
    '///////////////////////ZB add code///////////////////////////////////////
            For Each site In TheExec.sites
                If force_i = 0.05 Then
                    MeasV_Force50mA.pins(DUTPin).value = PPMUMeasure.pins(DUTPin).value
                    TheExec.Datalog.WriteComment "50mA, " & "pin:" & DUTPin & ", site" & site & ": " & MeasV_Force50mA.pins(DUTPin).value
                Else
                    MeasV_Force0mA.pins(DUTPin).value = PPMUMeasure.pins(DUTPin).value
                    TheExec.Datalog.WriteComment "0mA, " & "pin:" & DUTPin & ", site" & site & ": " & MeasV_Force0mA.pins(DUTPin).value
                End If
            Next site
    '/////////////////////////////////////////////////////////////////////////
            'offline mode simulation
            ...
            If independt_meas = False Then
                TheHdw.PPMU.pins(DUTPin).ForceV 0
                TheHdw.PPMU.pins(DUTPin).ClampVHi = 1.2
            Else
                TheHdw.PPMU.pins(DUTPin).ForceI 0
                TheHdw.PPMU.pins(DUTPin).ClampVHi = 1.2
                TheHdw.PPMU.pins(DUTPin).Gate = tlOff
...
'///////////////////////////////////ZB add for measure contact resistance//////////////////////////////////////////////////////
    For Each DUTPin In pins
        pld_temp_R.pins(DUTPin) = MeasV_Force50mA.pins(DUTPin).Subtract(MeasV_Force0mA.pins(DUTPin)).divide(0.05)
        Calculate_Contact_R.pins(DUTPin) = pld_temp_R.pins(DUTPin).Subtract(CurrentJob_Card_RAK.pins(DUTPin))
    Next DUTPin
...
'///////////////////////////////////End measure contact resistance//////////////////////////////////////////////////////
    ...
    Exit Function
errHandler:
        TheExec.Datalog.WriteComment "Error in Resistance Corner Vss"
        If isDebugMode Then TheExec.AddOutput "Error in Resistance Corner Vss"
        If AbortTest Then Exit Function Else Resume Next
End Function
...
'''***** End - Define the number of PinGroup *****
    ...
    If s_tempDCVSPins <> "" Then ''TheHdw.DCVS.Pins(s_tempDCVSPins).Voltage.Main.value = 0# '''Set all power pins to 0 V first
        ...
        With TheHdw.DCVS.pins(s_tempDCVSPins)
          .Voltage.value = 0# '''Set all power pins to 0 V first
          .Meter.mode = tlDCVSMeterCurrent
        End With
    End If
    
    If s_tempDCVIPins <> "" Then
    
        With TheHdw.DCVI.pins(s_tempDCVIPins)
          .Voltage.value = 0# '''Set all power pins to 0 V first
          .Meter.mode = tlDCVIMeterCurrent
        End With
    End If
...
'''***** End - DataLog printing *****
        ...
        End If
        If p_uvs_Group(GroupIndex) <> "" Then
            For i = 0 To UBound(Split(p_uvs_Group(GroupIndex), ","))
                If TheHdw.DCVS.pins(Split(p_uvs_Group(GroupIndex), ",")(i)).CurrentRange.value <= 0.002 Then
                    TheHdw.DCVS.pins(Split(p_uvs_Group(GroupIndex), ",")(i)).CurrentRange.value = 0.2
                End If
            Next
...
'Public Function SetCurrentRange(ByRef pin_info() As AutoRange_Info, PinsGroup As String, Grp_Val_Hi As String, FlowLimitInit As Boolean, _
                                    WaitTime As Double, ArgHiLimit As Double, ArgLowLimit As Double, Optional PinsInitRange200mA As String, Optional PinsInitRange100mA As String, Optional PinsInitRange1A As String)
...
Public Function DC_conti_WalkingZ_PN_Disconnect(patset As Pattern, digital_pins As PinList, disconnect_pair As PinList, setlowpins As PinList)
    ...
    Dim pin As Variant
    Dim site As Variant
    Dim ContiFailPin As Variant
      ''' prevent connect PPMU and digital at same time
     With TheHdw
     .Digital.pins(setlowpins).Disconnect
     .PPMU(setlowpins).Disconnect
    End With
    
    Dim s_Pin As String
    '''s_Pin = "VDD_FIXED_LPDP_RX_DCVI,VDD_FIXED_LPDP_TX_DCVI,VDD_FIXED_PCIE_DCVI,VDD12_LPDP_RX_DCVI,VDD12_PCIE_DCVI"  'Tahiti
    '''Update for Thera By David 240529
    s_Pin = "VDD_FIXED_LPDP_RX,VDD_FIXED_LPDP_TX,VDD_FIXED_PCIE_DCVI,VDD12_LPDP_RX_DCVI,VDD12_PCIE_DCVI"
...
If glb_TesterType = "Jaguar" Then
    ...
    If TheExec.DataManager.instancename Like "*WalkingZ_DC_Continuity*" And InStr(UCase(TheExec.CurrentChanMap), UCase("ChannelMap_WLCP_5_site")) <> 0 Then         'Modify for CP DCVI     2021/12/21
        With TheHdw.DCVI.pins("VDD_FIXED_LPDP_RX_DCVI,VDD_FIXED_LPDP_TX_DCVI,VDD_FIXED_PCIE_DCVI,VDD12_LPDP_RX_DCVI,VDD12_PCIE_DCVI")
            .mode = tlDCVIModeVoltage
            .SetCurrentAndRange 800 * mA, 1000 * mA
            TheHdw.Wait 2 * ms
...
ElseIf glb_TesterType = "UltraFLEXplus" Then
    If TheExec.DataManager.instancename Like "*WalkingZ_DC_Continuity*" And UCase(TheExec.CurrentJob) Like "*CP*" Then         'Modify for CP DCVI     2021/12/21
        With TheHdw.DCVS.pins(s_Pin)
            .mode = tlDCVSModeVoltage
            .SetCurrentRanges 800 * mA, 800 * mA
            TheHdw.Wait 2 * ms
            .Voltage = 0
            .Gate = True
            .Connect
...
'    If TheExec.DataManager.instancename Like "*WalkingZ_DC_Continuity*" And UCase(TheExec.CurrentJob) Like "*FT*" Then         'Modify for FT DCVI     2021/12/21
'        With TheHdw.DCVI.Pins("vdd12_pcie_ft,vdd12_lpdp_rx_ft,vdd_fixed_lpdp_tx_ft,vdd_fixed_lpdp_rx_ft,vdd_fixed_pcie_ft")
'            .mode = tlDCVIModeVoltage
'            .SetCurrentAndRange 800 * mA, 1000 * mA
'            TheHdw.Wait 2 * ms
'            .Voltage = 0
...
'    End If
      TheHdw.Wait 0.005
    '''set to GND
      TheHdw.Digital.pins(setlowpins).initState = chInitLo
    ...
    End If
        ...
        'thehdw.Patterns(inti_patset).Load
        'thehdw.Patterns(inti_patset).test pfAlways, 0
        
    If disconnect_pair <> "" Then
        TheHdw.Digital.pins(disconnect_pair).Disconnect
    End If
...
'' float pin before fuction run
    ...
    ' ContiFailPin = "" 'clear fail pins
    
    If TheExec.DataManager.instancename Like "DC_Continuity*" Then
        ...
        For Each site In TheExec.sites
            ...
            FailPinsGroup = TheHdw.Digital.FailedPins(site)
              ...
              For Each pin In FailPinsGroup
               ...
               If ContiFailPin = "" Then
                   ContiFailPin = pin
               Else
                   ContiFailPin = ContiFailPin & "," & pin
               End If
