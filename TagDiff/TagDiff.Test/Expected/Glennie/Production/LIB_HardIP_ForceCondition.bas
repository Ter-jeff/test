...
'    Analyze_shmoo_setup ' for trace compensation
    ...
    ''' Add for Donan to NORESTORE force condition @William 230301
    If (UCase(Setup_string) Like "*NORESTORE*") Then
        Setup_string = Replace(UCase(Setup_string), ":NORESTORE", vbNullString)     '20240131 Hidra by YM
        Setup_string = Replace(UCase(Setup_string), ";NORESTORE", vbNullString)
    ...
    End If
    Setup_string = LCase(Setup_string)
    '''
...
'===============================================================================================
    ...
    For Each cat_split_temp In Cat_temp
        ...
        If (UBound(Pin_info_temp) >= 2) Then
            ...
            'If LCase(Pin_info_temp(2)) = "char_retest" And TheExec.DevChar.Setups.IsRunning = True Then Exit Function
            
            If LCase(Pin_info_temp(2)) = "char_retest" Then
        ...
        '==== Gating N/C pin set force condition issue ===== -- 20230307
        If UCase(gl_GetInstrumentType_Dic(LCase(PinName))) = "N/C" Then
            Call Print_Error_Message(Warning_Info, "LIB_Digital_Shmoo_Setup", "Force condition using N/C pin error")
            Exit Function
        End If
...
'                End If
                If LCase(Pin_value) = "char_retest" Then
                    TheHdw.DCVS.pins(var).Voltage.Alt.ValuePerSite = sd_retest_char_ForceCondiction
                    For Each site In TheExec.sites.Active
                        TheExec.Datalog.WriteComment "Site" & site & " " & var & ":Valt: " & sd_retest_char_ForceCondiction(site)
                    Next site
                Else
                TheHdw.DCVS.pins(var).Voltage.Alt.value = CDbl(Pin_value)
                End If
...
On Error GoTo errHandler
    ...
    Call TheExec.DataManager.DecomposePinList(pin, PinList, typesCount) ' CN add to fix no pinlist issue 20240522
    ...
    ElseIf pin_instrument_type = "HEXVS" Or pin_instrument_type = "VHDVS" Or pin_instrument_type = "VS-800MA" Or pin_instrument_type = "VS-5A" Then
        ...
        If LCase(force_val_Str) = "char_retest" Then
            Pin_value = force_val_Str
        Else
        ...
        For Each var In PinList
            ...
            Else
                ...
                Else
                    ...
                    'If thehdw.DCVS.Pins(var).Gate = False Then
                    If (TheHdw.DCVS.pins(var).Gate = False And Not LCase(glb_TesterType) = "jaguar") Or (TheHdw.DCVS.pins(var).Gate = False And Not LCase(var) Like "*vdd*") Then
                        With TheHdw.DCVS.pins(var)
                                                        .Voltage.Output = tlDCVSVoltageMain
                            .Disconnect tlDCVSConnectDefault
                            .Meter.mode = tlDCVSMeterCurrent
                    ...
                    If LCase(force_val_Str) = "char_retest" Then
                        Pin_value = force_val_Str
                         TheHdw.DCVS.pins(var).Voltage.Main.ValuePerSite = sd_retest_char_ForceCondiction
                    Else
                    Pin_value = CStr(Spec_Evaluate_DC_for_flow_loop(var, "V", pin_condition_ary(0)))
                    TheHdw.DCVS.pins(var).Voltage.Main.value = CDbl(Pin_value)
                End If
...
'        For Each var In PinList
            With TheHdw.DCVI.pins(pin) '' High impedence mode
                ...
                Else
                    ...
                    ''[20240604] MTRICTS alarm issue
                    If TheExec.DataManager.instancename Like "MTRICTS_*" Then
                        TheHdw.Wait 0.005
                    End If
                    .CurrentRange.Autorange = True
