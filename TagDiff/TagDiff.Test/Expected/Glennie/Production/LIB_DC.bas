        ...
        Case glbConstIns_HEXVS, glbConstIns_VHDVS, glbConstIns_VSM, glbConstIns_VS800MA, glbConstIns_VS5A:
            ...
            For j = 0 To UBound(pin_info.Range_List) - 1
                If val >= pin_info.Range_List(j) And val <= pin_info.Range_List(j + 1) Then
                    ...
                    SattleTime = pin_info.WaitTime_List(j + 1)
...
Public Sub ParseIDSMappingTable(Optional d_Check_FUSE_Sheet As Boolean = False)
    On Error GoTo errHandler
    ...
    If Flag_IDSMappingTable = False Then
        ...
        sheetName = "IDS_Mapping_Table_VBT"
...
Public Function IDS_Store2Dic_Mapping(Core_Power_Pin As String, All_Power_data As PinListData, patt As Pattern)
...
ProcessBinOut:  'Binout if get something error
    ...
    'From Tahiti
    For Each site In TheExec.sites
        nTestNumber = TheExec.sites.item(site).TestNumber
        Call TheExec.Datalog.WriteFunctionalResult(site, nTestNumber, logTestFail)
        ''IDS disconnect - bin out FLAG 20230118
        TheExec.sites.item(site).FlagState("F_IDS_PAT_VERSION_CHECK_Flag") = logicTrue
    Next site
...
On Error GoTo errHandler
    ...
    If SetCurrentValue < 0.0002 Then SetCurrentValue = 0.0002 ' add for T-Tah minimun current range
...
'                        End If
                        ...
                        ''[20240605] IFold setting issue
                        If LCase(gl_GetInstrument_Dic(powerPin)) = "hexvs" And SetCurrentValue < 0.1 Then
                            TheHdw.DCVS.pins(powerPin).SetCurrentRanges 0.1, 0.1
                            'TheHdw.Wait 0.03
                            TheHdw.DCVS.pins(powerPin).CurrentLimit.Source.FoldLimit.level.value = 0.1
                        Else
                            TheHdw.DCVS.pins(powerPin).SetCurrentRanges .Range_List(k), .Range_List(k)
                            'TheHdw.Wait 0.03
                            If .Range_List(k) < TheHdw.DCVS.pins(powerPin).CurrentLimit.Source.FoldLimit.level.Min Then
                                TheHdw.DCVS.pins(powerPin).CurrentLimit.Source.FoldLimit.level.value = TheHdw.DCVS.pins(powerPin).CurrentLimit.Source.FoldLimit.level.Min
                            Else
                                TheHdw.DCVS.pins(powerPin).CurrentLimit.Source.FoldLimit.level.value = .Range_List(k)
                            End If
                        End If
            ...
            ElseIf s_InstType Like "*DCVI*" Then
                For k = 0 To UBound(.Range_List) - 1
                    If (SetCurrentValue <= .Range_List(k)) Or (k = UBound(.Range_List)) Then
...
Public Function SetCurrentRange_CRWT(powerPin As String, SetCurrentValue As Double, WaitTime As Double, Optional SetCurrentStep As Long)
On Error GoTo errHandler
    Dim k As Long
    Dim SattleTime As Double
    Dim s_InstType As String
    Dim s_Slot_Type As String
    Dim PowerPin_range As AutoRange_Info
    
    If gl_GetInstrumentType_Dic.Exists(LCase(powerPin)) Then
        s_Slot_Type = UCase(gl_GetInstrument_Dic(LCase(powerPin)))
        s_InstType = gl_GetInstrumentType_Dic(LCase(powerPin))
        
        If F_p2pWT Then
            PowerPin_range = PowerPin_range_ary_CRWT(gl_dicPowerPinIndex(LCase(powerPin)))
        Else
            PowerPin_range = PowerPin_range_ary(gl_dicPowerPinIndex(LCase(powerPin)))
        End If
        ...
        With PowerPin_range
            If s_InstType Like "*DCVS*" Then
                For k = 0 To UBound(.Range_List)
                    If (SetCurrentValue <= .Range_List(k)) Or (k = UBound(.Range_List)) Then
                        TheHdw.DCVS.pins(powerPin).SetCurrentRanges .Range_List(k), .Range_List(k)
                            If (UCase(powerPin) = "VDD_AMPH_DDR") Or (UCase(powerPin) = "VDDQL_DDR") Then
                             TheHdw.Wait 0.025
                            End If
                        ...
                        If F_p2pWT And Not UCase(s_Slot_Type) = "HEXVS" And (Not Dict_CRWT_TuneTime.Exists(UCase(powerPin) & "-" & CStr(.Range_List(k)))) Then
                            TheExec.Datalog.WriteComment "Warning: Pin " & UCase(powerPin) & " with range " & CStr(.Range_List(k)) & " is not in TuneCRWTResult_" & UCase(currentJobName)
                        End If
                        
                        SetCurrentStep = k
                        WaitTime = .WaitTime_List(k)
...
Exit Function
errHandler:
    Call Print_Error_Message(Error_Info, "LIB_DC", "SetCurrentRange")
    If AbortTest Then Exit Function Else Resume Next
End Function
