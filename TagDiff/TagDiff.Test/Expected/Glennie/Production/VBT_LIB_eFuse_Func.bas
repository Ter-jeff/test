...
'    If ((GlbUtility.currStage Like "*CP*") Or (GlbUtility.currStage Like "*WLFT*")) Then
    If UCase(TheExec.CurrentChanMap) Like "CHANNELMAP_CP*" Or UCase(TheExec.CurrentChanMap) Like "CHANNELMAP_WL*" Then  'JackChou 20240329
...
Dim TrimMissingCnt As New SiteLong
    ...
    For Each bank In BdfDataBase.Banks
        ...
        For Each m_site In TheExec.sites
            ...
            If GlbUtility.testedStages.Exists(field.BlowLocation) And UCase(opbank.name) = "CFG" Then
...
On Error GoTo errHandler
    ...
    For i = 0 To power_pin_number - 1
        ...
        TheExec.flow.TestLimit resultVal:=result.Multiply(1000), lowVal:=0, hiVal:=2000, Tname:=temp_power_pin
    Next
...
On Error GoTo errHandler
    ...
    Dim m_value As New SiteVariant
    Dim siteIndex As New SiteVariant
    Dim opbank As eFuseBdfBank
    Dim field As eFuseBdfField
    Dim m_loop As Long
    Dim m_catename As String
    Dim DramType As Variant, item As Variant
    Dim enableWdCnt As Integer: enableWdCnt = 0
    Dim siteCtrlArray() As String
    Dim enableWord As String
    Dim i As Integer
    Dim msg As String
    ...
    For Each DramType In BdfDataBase.DicDramMap
        If (DramType Like "DRAM_*") Then
            If TheExec.flow.enableWord(DramType) = True Then
                GlbUtility.WriteDlg "<<< The Dram Enable = " & DramType & " >>>"
                enableWdCnt = enableWdCnt + 1
                enableWord = DramType
                If (enableWdCnt > 1) Then
                    msg = "<<< [Error] more than One Dram EnableWord selected!!! >>>"
                    GoTo skip
                End If
            End If
        End If
    ...
    If enableWord = "" Then
        msg = "<<< [Error] The Dram EnableWord isn't selected!!! >>>"
        GoTo skip
    End If
    ...
    For m_loop = 0 To BdfDataBase.DramFieldSize - 1
        m_catename = BdfDataBase.DicDramMap.item("fieldname")(m_loop)
            For Each site In TheExec.sites
                ...
                End If
'                m_value(site) = GlbUtility.String2Dbl(CStr(BdfDataBase.DicDramMap.item(enableWord)("fusedata")(m_Loop)))
            Next site
...
Public Function DRAM_eFuse_CheckByHip(FuseType As String)
On Error GoTo errHandler
    Dim funcName As String:: funcName = "DRAM_eFuse_CheckByHip"

    Dim hip_name As Variant
    Dim opbank As eFuseBdfBank
    Dim field As eFuseBdfField
    Dim fieldstage As String
    ...
    Dim limit_tmp As String
    Dim limit_value As Long
    Dim m_value As New SiteDouble
    Dim mrrLists As New SiteVariant
    Dim keyword As New SiteVariant
    
    
    If Not BdfDataBase.DicDramMap.Exists("hipname") Then
        GlbUtility.WriteDlg "<<< The Dram Hip name didn't exist in BdfDataBase.DicDramMap!!! >>>"
        TheExec.flow.TestLimit resultVal:=0, lowVal:=1, hiVal:=1, Tname:="DRAM_eFuse_CheckByHip"
        Exit Function
    End If
    
    hip_name = BdfDataBase.DicDramMap("hipname")
    For m_loop = 0 To UBound(hip_name)
        mrrLists = GetStoredData(hip_name(m_loop) & "_para")
        If m_loop = 0 Then
            keyword = mrrLists
        Else
            For Each site In TheExec.sites
                keyword = CStr(keyword) + "_" + CStr(mrrLists)
            Next
        End If
    Next
    ...
    Set opbank = GetBdfBank(FuseType)
    For Each site In TheExec.sites
        GlbUtility.WriteDlg "Site:" & CStr(site) & ", <<< The Dram Hip Value = " & CStr(keyword(site)) & " >>>"
        For m_loop = 0 To BdfDataBase.DramFieldSize - 1
            m_catename = BdfDataBase.DicDramMap.item("fieldname")(m_loop)
            Set field = opbank.Fields(m_catename)
            ...
            If fieldstage = TheExec.CurrentJob Or GlbUtility.testedStages.Exists(field.BlowLocation) Then
                m_value = field.TrimAteDecValue
                If BdfDataBase.DicDramMap.Exists(CStr(keyword(site))) Then
                    limit_tmp = BdfDataBase.DicDramMap(CStr(keyword(site)))("fuseData")(1)(m_loop)
                    If LCase(limit_tmp) Like "x*" Then
                        limit_value = GlbUtility.Hex2Dbl(limit_tmp)
                    ElseIf LCase(limit_tmp) Like "b*" Then
                        limit_value = GlbUtility.Bin2Dec(limit_tmp)
                    Else
                        limit_value = limit_tmp
                    End If
                    TheExec.flow.TestLimit resultVal:=m_value, lowVal:=limit_value, hiVal:=limit_value, Tname:=m_catename
                Else
                    GlbUtility.WriteDlg "<<< The Dram HIP Value didn't match DRAM Table!!! >>>"
                    TheExec.flow.TestLimit resultVal:=0, lowVal:=1, hiVal:=1, Tname:="DRAM_eFuse_CheckByHip"
                End If
            End If
        Next
...
Exit Function
errHandler:
    Call Print_Error_Message(Error_Info, "VBT_LIB_eFuse_Func", "DRAM_eFuse_CheckByHip")
    If AbortTest Then Exit Function Else Resume Next
...
Dim site As Variant 'Carter, 20240304
    ...
    For i = 0 To UBound(bankarr)
        For Each site In TheExec.sites
            ...
            TheExec.flow.TestLimit resultVal:=TheExec.sites(site).SiteVariableValue(m_siteVar), lowVal:=-1, hiVal:=1, Tname:=m_siteVar, PinName:=bankarr(i) + "_Chk_Var"
        Next site
    Next i
...
Public Function BinFuse_eFuse_setWrite()
On Error GoTo errHandler
    
    Dim funcName As String:: funcName = "BinFuse_eFuse_setWrite"
    
    Dim m_value_PI As New SiteVariant, m_value_FSTP As New SiteVariant, m_value As New SiteVariant
    Dim opbank As eFuseBdfBank
    Dim field As eFuseBdfField
    ...
    If UCase(TheExec.CurrentJob) = "CP1" Then
        m_value_PI = field.TrimAteDecValue
    ElseIf UCase(TheExec.CurrentJob) = "CP2" Then
        m_value_PI = field.DsscDecValue
    End If
    
'    Set field = opbank.fields("gfx_fstp_harvest")
'    If UCase(TheExec.CurrentJob) = "CP1" Then
'        m_value_FSTP = field.TrimAteDecValue
'    ElseIf UCase(TheExec.CurrentJob) = "CP2" Then
'        m_value_FSTP = field.DsscDecValue
'    End If
    
    For Each site In TheExec.sites
'        If m_value_PI(site) = 0 And m_value_FSTP(site) = 0 Then 'Bin1 & all core pass
'            m_value(site) = 1
'        ElseIf m_value_PI(site) = 0 And _
'            (m_value_FSTP(site) = 1 Or m_value_FSTP(site) = 2 Or m_value_FSTP(site) = 4 Or m_value_FSTP(site) = 8 Or m_value_FSTP(site) = 16) Then 'Bin1 & one core Fail
'            m_value(site) = 3
'        ElseIf (m_value_PI(site) = 1 Or m_value_PI(site) = 2) And m_value_FSTP(site) = 0 Then 'BinX or BinY & all core pass
'            m_value(site) = 5
'        Else
'            m_value(site) = 0
'        End If
        If m_value_PI(site) = 1 Then
            m_value(site) = 7
        ElseIf TheExec.sites.item(site).FlagState("BIN4_CAND") = logicTrue And TheExec.sites.item(site).FlagState("F_SOC_IS") = logicTrue Then
            m_value(site) = 6
        ElseIf TheExec.sites.item(site).FlagState("BIN4_CAND") = logicTrue And TheExec.sites.item(site).FlagState("F_SOC_ID") = logicTrue Then
            m_value(site) = 5
        ElseIf TheExec.sites.item(site).FlagState("BIN4_CAND") = logicTrue And (TheExec.sites.item(site).FlagState("F_ECPU_CORE_3_0_SUM_1") = logicTrue Or TheExec.sites.item(site).FlagState("F_PCPU_CORE_1_0_SUM_1") = logicTrue) Then
            m_value(site) = 4
        ElseIf TheExec.sites.item(site).FlagState("BIN3_CAND") = logicTrue Then
            m_value(site) = 3
        ElseIf TheExec.sites.item(site).FlagState("BIN2_CAND") = logicTrue Then
            m_value(site) = 2
    ...
    If UCase(TheExec.CurrentJob) = "CP1" Then
        Set field = opbank.Fields("bin_fuse_te_misc1")
    ElseIf UCase(TheExec.CurrentJob) = "CP2" Then
        Set field = opbank.Fields("bin_fuse")
    End If
...
Exit Function
errHandler:
    Call Print_Error_Message(Error_Info, "VBT_LIB_eFuse_Func", "BinFuse_eFuse_setWrite")
        ...
        For Each site In TheExec.sites
            If GlbUtility.testedStages.Exists(field_normal.BlowLocation) Then
                ...
                If Flag_BinX_Info_Parsed = True And CurrentPassBinCutNum_normal(site) = 2 Then
                    CurrentPassBinCutNum_normal(site) = 1
                End If
        ...
        If field_addition.DefaultOrReal = dr_real Then
            For Each site In TheExec.sites
                If GlbUtility.testedStages.Exists(field_addition.BlowLocation) Then
                    ...
                    If Flag_BinX_Info_Parsed = True And CurrentPassBinCutNum_additional(site) = 2 Then
                        CurrentPassBinCutNum_additional(site) = 1
                    End If
                        ...
                        Else
                            For Each site In TheExec.sites
                                ...
                                ElseIf GlbUtility.testedStages.Exists(field.BlowLocation) Then
                                    ...
                                    If LCase(field.name) Like "*_shadow" And field.FuseMeasureValue = 0 Then '220830 Add for skip shadow fields that not fuse value at tested stage
                                        TheExec.flow.TestLimit resultVal:=-1, lowVal:=-1, hiVal:=-1, Tname:=field.name & "_EQ"
                                    Else
                                    If LCase(field.BlowLocation) = LCase(BincutAdditionalSheetName) Then
                                        If need_calculate_addition(site) Then
...
Public Function DRAM_eFuse_WriteByHip(FuseType As String)
On Error GoTo errHandler
    Dim funcName As String:: funcName = "DRAM_eFuse_WriteByHip"

    Dim m_value As New SiteVariant
    Dim m_valuearr() As New SiteVariant
    Dim opbank As eFuseBdfBank
    Dim field As eFuseBdfField
    Dim keyword As New SiteVariant
    Dim m_loop As Long
    ...
    If Not BdfDataBase.DicDramMap.Exists("hipname") Then
        GlbUtility.WriteDlg "<<< The Dram Hip name didn't exist in BdfDataBase.DicDramMap!!! >>>"
        TheExec.flow.TestLimit resultVal:=0, lowVal:=1, hiVal:=1, Tname:="DRAM_eFuse_WriteByHip"
        Exit Function
    End If
    
    ReDim m_valuearr(BdfDataBase.DramFieldSize - 1)
    Set opbank = GetBdfBank(FuseType)
    ...
    For Each site In TheExec.sites
        ...
        For m_loop = 0 To BdfDataBase.DramFieldSize - 1
            ...
            If BdfDataBase.DicDramMap.Exists(CStr(keyword(site))) Then
                m_value = BdfDataBase.DicDramMap((CStr(keyword(site))))("fuseData")(1)(m_loop)
                If LCase(m_value) Like "x*" Then
                    m_value = GlbUtility.Hex2Dbl(CStr(m_value))
                ElseIf LCase(m_value) Like "b*" Then
                    m_value = GlbUtility.Bin2Dec(CStr(m_value))
                Else
                    m_value = CStr(m_value)
                End If
        
                m_valuearr(m_loop) = m_value
            Else
                GlbUtility.WriteDlg "Site:" & CStr(site) & ", <<< The Dram HIP Value didn't match DRAM Table!!! >>>"
                TheExec.flow.TestLimit resultVal:=0, lowVal:=1, hiVal:=1, Tname:="DRAM_eFuse_WriteByHip"
            End If
        Next
    Next site
    ...
    For m_loop = 0 To BdfDataBase.DramFieldSize - 1
        ...
        Set field = opbank.Fields(m_catename)
        If field.BlowLocation = GlbUtility.currStage Then
            opbank.SetEfuse field.name, m_valuearr(m_loop), , , , , True
        End If
    Next m_loop
    
Exit Function
errHandler:
    Call Print_Error_Message(Error_Info, "VBT_LIB_eFuse_Func", "DRAM_eFuse_WriteByHip")
   
        If AbortTest Then Exit Function Else Resume Next

End Function

Public Function DRAM_eFuse_CheckByEnableWd(FuseType As String)
On Error GoTo errHandler
    Dim funcName As String:: funcName = "DRAM_eFuse_CheckByEnableWd"
    ...
    Dim limit_value As Long
    Dim m_value As New SiteDouble
    Dim keyword_hip As String
    Dim enableWord As String
    Dim DramType As Variant
    
    For Each DramType In BdfDataBase.DicDramMap
        If DramType <> "fieldname" Then
            If TheExec.flow.enableWord(DramType) = True Then  'EX:DRAM_Hynix8GB
                enableWord = DramType
                Exit For
            End If
        End If
    ...
    If enableWord = "" Then
        GlbUtility.WriteDlg "<<< The Dram Enable isn't selected!!! >>>"
        TheExec.flow.TestLimit resultVal:=0, lowVal:=1, hiVal:=1, Tname:="DRAM_eFuse_CheckByEnableWd"
        Exit Function
    End If
    
    GlbUtility.WriteDlg "<<< The Dram Enable = " & "DRAM_" & enableWord & " >>>"
    ...
    For m_loop = 0 To BdfDataBase.DramFieldSize - 1
        ...
        If fieldstage = TheExec.CurrentJob Or GlbUtility.testedStages.Exists(field.BlowLocation) Then
            m_value = field.TrimAteDecValue
            If BdfDataBase.DicDramMap.Exists(enableWord) Then
                limit_tmp = BdfDataBase.DicDramMap.item(enableWord)("fuseData")(m_loop)
                If LCase(limit_tmp) Like "x*" Then
                    limit_value = GlbUtility.Hex2Dbl(limit_tmp)
                ElseIf LCase(limit_tmp) Like "b*" Then
                    limit_value = GlbUtility.Bin2Dec(limit_tmp)
...
Exit Function
errHandler:
    Call Print_Error_Message(Error_Info, "VBT_LIB_eFuse_Func", "DRAM_eFuse_CheckByEnableWd")
...
On Error GoTo errHandler
    ...
    If checkData.CombineFieldCheck Then
        ...
        If checkData.Width >= 32 Then
            over32Bits = True
            m_fusevalueHex = GlbUtility.Bin2HexStr(binaryStr, Celling(checkData.Width / 4))
        Else
...
End Function
Public Function DTS_AddStoredData(Path As String)
    On Error GoTo errHandler
    ...
    If lot_id <> "" Then ''200908 If the Lot ID is empty,it will skip parsing file
        ...
        FilePath = Path & lot_id & ".txt"   'open file path
        FileExists = Dir(FilePath) <> ""
        If FileExists = True Then
            TheExec.Datalog.WriteComment "DTS File::::: " + FilePath + "::::: Access"
            Flag_DTS_function = True
            DTS_parsing_CHK = True 'only parse one time
            ...
            Close #1
            Kill (FilePath)
            If FileisBlank = True Then
                TheExec.Datalog.WriteComment "DTS File:: " + FilePath + " doesn't have content, please check it!"
            End If
            
        ElseIf DTS_parsing_CHK = True Then
        
        Else
            TheExec.Datalog.WriteComment "DTS File:: " + FilePath + " doesn't exist, please check it!"
        End If
...
errHandler:
    Call Print_Error_Message(Error_Info, "VBT_LIB_eFuse_Func", "DTS_AddStoredData")
    
        If AbortTest Then Exit Function Else Resume Next

End Function

'20210623, add for DTS
...
'    End If
    ...
    Else
        ...
        TheExec.flow.TestLimit 0, 1, 1, Tname:="Without_OCR&DTS"
    End If
...
errHandler:
    Call Print_Error_Message(Error_Info, "VBT_LIB_eFuse_Func", "ECID_DTS")
    
        If AbortTest Then Exit Function Else Resume Next

End Function

Public Function PKGType_setWrite()
On Error GoTo errHandler
    
    Dim funcName As String:: funcName = "PKGType_setWrite"
    ...
    Set opbank = GetBdfBank("CFG")
    Set field = opbank.Fields("pkg_type")
    
    For Each site In TheExec.sites
    
        If TheExec.CurrentChanMap Like "*_WLFT_*" Then
            m_value(site) = 0
        ElseIf TheExec.CurrentChanMap Like "*_FT_MCM_*" Then
            m_value(site) = 1
        End If
    Next site
...
Exit Function
errHandler:
    Call Print_Error_Message(Error_Info, "VBT_LIB_eFuse_Func", "PKGType_setWrite")
  
        If AbortTest Then Exit Function Else Resume Next

End Function

Public Function EVSsorting_setWrite(bankName As String, fieldName As String, flagName As String)
On Error GoTo errHandler
    
    Dim funcName As String:: funcName = "EVSsorting_setWrite"
    ...
    Set field = opbank.Fields(fieldName)
...
Public Function EVSsorting_setFlag(bankName As String, fieldName As String, flagName As String)
On Error GoTo errHandler
    
    Dim funcName As String:: funcName = "EVSsorting_setFlag"
    ...
    For Each site In TheExec.sites
        If field.DsscDecValue(site) = 2 Then
            TheExec.sites.item(site).FlagState(flagName) = logicTrue
            TheExec.Datalog.WriteComment ("******EVS 2-step Condition: EVS_HIGHFAIL_LOWPASS******")
        Else
            TheExec.sites.item(site).FlagState(flagName) = logicFalse
            TheExec.Datalog.WriteComment ("******EVS 2-step Condition: EVS_HIGHPASS******")
        End If
    Next site
...
End Function
Public Function DieCorner_setWrite(bankName As String, fieldName As String, flagName1 As String, flagName2 As String)
On Error GoTo errHandler
    
    Dim funcName As String:: funcName = "DieCorner_setWrite"
    ...
    For Each site In TheExec.sites
        If TheExec.sites.item(site).FlagState(flagName1) = logicTrue Or TheExec.sites.item(site).FlagState(flagName2) = logicTrue Then
            m_value(site) = 1
        Else
            m_value(site) = 0
        End If
...
End Function
Public Function DieCorner_setFlag(bankName As String, fieldName As String, flagName As String)
On Error GoTo errHandler
    
    Dim funcName As String:: funcName = "DieCorner_setFlag"
    ...
    For Each site In TheExec.sites
        If field.DsscDecValue(site) = 1 Then
            TheExec.sites.item(site).FlagState(flagName) = logicTrue
            TheExec.Datalog.WriteComment ("******Die Corner test failed in CP1******")
        Else
            TheExec.sites.item(site).FlagState(flagName) = logicFalse
            TheExec.Datalog.WriteComment ("******Die Corner test passed in CP1******")
        End If
    Next site
...
End Function
' [20230425][T-Tah][OliverD] original function .open xlsx will cause issue, modify to .csv
Public Function ECID_Dict_Buildup()
...
Dim Index_of_Data_at_Row As Long:: Index_of_Data_at_Row = 2
...
''for debug
pathstr = "X:\Production\TMSQ70\TG55_ECID_Sorting_For_Harvesting.csv"

Set ECID_Dict = CreateObject("Scripting.Dictionary")
'Set ECID_Dict_other = CreateObject("Scripting.Dictionary")
...
For cnt = 0 To UBound(temp)
    If UCase(temp(cnt)) Like UCase("*Lot*") Then
        ECID_Info_ary = Split(temp(cnt), ",")
        For k = 0 To UBound(ECID_Info_ary)
            If UCase(Trim(ECID_Info_ary(k))) Like UCase("*Lot*") Then
                Lot_idx = k
            ElseIf UCase(Trim(ECID_Info_ary(k))) Like UCase("Wafer*") Then
                Wafer_num_idx = k
            ElseIf UCase(Trim(ECID_Info_ary(k))) = UCase("X") Then
                X_idx = k
            ElseIf UCase(Trim(ECID_Info_ary(k))) = UCase("Y") Then
                Y_idx = k
            ElseIf UCase(Trim(ECID_Info_ary(k))) Like UCase("*Bin*") Then
                HB_idx = k
            End If
        Next k
        Startidx = cnt + 1
...
ReDim Preserve ECID_CheckList.Category(i)

For CC = Startidx To UBound(temp)
    If UCase(Trim(temp(CC))) Like UCase("*END*") Then
        Exit For
    End If
    ECID_Info_ary = Split(temp(CC), ",")
    With ECID_CheckList.Category(i)
        ...
        '.OtherBin = my_sheet.Cells(Index_of_Data_at_Row, 6)
        ...
        'Create DataString for storing into ECID dictionary
         ...
         If Not ECID_Dict.Exists(DataString) Then
            ECID_Dict.Add DataString, .FT_HW_Bin
            'ECID_Dict_other.Add DataString, .OtherBin
...
errHandler:
    Debug.Print "Warning!!! Something error when creating ECID DataBase!!"
End Function
Public Function BinSorting_Compare()

On Error GoTo errHandler
    Dim funcName As String:: funcName = "BinSorting_Compare"
...
'########################################################################################################
    ...
    RecordDate = Date
    RecordDate = Replace(RecordDate, "/", "_")
'    Curpath = CurDir
'    Curpath = Curpath & "\" & "ECID_record_list.txt"
    CurTime = Now
    CurTime = Replace(CurTime, "/", "_")
    CurTime = Replace(CurTime, " ", "_")
    CurTime = Replace(CurTime, ":", "_")
    Curpath = "C:\" & RecordDate & "_" & "ECID_record_list.txt"

    If ECID_Dict Is Nothing Then
        TheExec.Datalog.WriteComment "<WARNING> ECID BinSort Dictionary not exist!!" & vbCrLf
    Else
        For Each site In TheExec.sites
    
            key = HramLotId & "," & HramWaferId & "," & HramXCoord & "," & HramYCoord
            
            If ECID_Dict.Exists(key) Then
'              TheExec.sites(Site).BinNumber = ECID_Dict(key)
'              TheExec.sites(Site).SortNumber = ECID_Dict(key)
              
              Open Curpath For Append As #1
              Print #1, HramLotId & ", "; HramWaferId & ", "; HramXCoord & ", "; HramYCoord & ", "; CurTime
              Close #1
              TheExec.flow.TestLimit 0, 1, 1, , , , , , "ECID_Screen_Out"
              'TheExec.sites(site).result = tlResultFail
'              TheExec.sites.item(Site).FlagState("F_BinSorting") = logicTrue
            Else
'              TheExec.sites(Site).BinNumber = 1
'              TheExec.sites(Site).SortNumber = 1
                     
              TheExec.flow.TestLimit 1, 1, 1, , , , , , "ECID_Sorting_pass"
              'TheExec.sites(site).result = tlResultPass
'              TheExec.sites.item(Site).FlagState("F_BinSorting") = logicFalse
              'TheExec.DataLog.WriteComment "<WARNING> ECID is NOT in excel sheet!!" & vbCrLf
