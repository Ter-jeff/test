...
''            End If
            Case "TMPS1_Untrim"
                            TmpString = CStr(Format(IDS_VDD_AVE(site), "0.00"))
            Case "TMPS2_Untrim"
                            TmpString = CStr(Format(IDS_VDD_DISP(site), "0.00"))
            Case "TMPS3_Untrim"
                            TmpString = CStr(Format(IDS_VDD_SRAM_SOC(site), "0.00"))
            Case "TMPS4_Untrim"
            ...
            Case "TMPS1"
                            TmpString = gS_TMPS1_Trim(site) 'from Tahiti
            Case "TMPS2"
                            TmpString = gS_TMPS2_Trim(site)
            Case "TMPS3"
                            TmpString = gS_TMPS3_Trim(site)
...
''                TmpString = SENFuse.Category(CategoryIndex).Read.ValStr(Site)
            Case "IDS_VDD_CPU"  'from Tahiti
                TmpString = CStr(Format(IDS_VDD_PCPU(site), "0.00"))
            Case "IDS_VDD_BACKUP"
                TmpString = CStr(Format(IDS_VDD_ECPU(site), "0.00"))
            Case "IDS_VDD_GPU"
                TmpString = CStr(Format(IDS_VDD_GPU(site), "0.00"))
            Case "IDS_VDD_CPU_SRAM"
                TmpString = CStr(Format(IDS_VDD_CPU_SRAM(site), "0.00"))
            Case "IDS_VDD_GPU_SRAM"
                TmpString = CStr(Format(IDS_VDD_SRAM_GPU(site), "0.00"))
            Case "IDS_VDD_SOC"
                TmpString = CStr(Format(IDS_VDD_SOC(site), "0.00"))
            Case "IDS_VDD_FIXED"
                TmpString = CStr(Format(IDS_VDD_FIXED(site), "0.00"))
            Case "IDS_VDD_LOW"
                TmpString = CStr(Format(IDS_VDD_LOW(site), "0.00"))
            Case "IDS_VDD_BACKUP1"
                TmpString = CStr(Format(IDS_VDD_DCS_DDR(site), "0.00"))

            Case "WLFT_DELTA_IDS_BACKUP1"   'from Tahiti
                TmpString = CStr(Format((Delta_IDS_VDD_AVE(site) * 1000), "0.0000"))
                If TmpString = "0.0000" Then TmpString = vbNullString
            Case "WLFT_DELTA_IDS_BACKUP"
                TmpString = CStr(Format((Delta_IDS_VDD_CPU_SRAM(site) * 1000), "0.0000"))
                If TmpString = "0.0000" Then TmpString = vbNullString
            Case "WLFT_DELTA_IDS_PCPU"
                TmpString = CStr(Format((Delta_IDS_VDD_PCPU(site) * 1000), "0.0000"))
                If TmpString = "0.0000" Then TmpString = vbNullString
            Case "WLFT_DELTA_IDS_ECPU"
                TmpString = CStr(Format((Delta_IDS_VDD_ECPU(site) * 1000), "0.0000"))
                If TmpString = "0.0000" Then TmpString = vbNullString
            Case "WLFT_DELTA_IDS_GPU"
                TmpString = CStr(Format((Delta_IDS_VDD_GPU(site) * 1000), "0.0000"))
                If TmpString = "0.0000" Then TmpString = vbNullString
            Case "WLFT_DELTA_IDS_DCS"
                TmpString = CStr(Format((Delta_IDS_VDD_DCS_DDR(site) * 1000), "0.0000"))
                If TmpString = "0.0000" Then TmpString = vbNullString
            Case Else
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/18
    ...
    If Flag_BinX_Info_Parsed = True And nBinCutNumber = 2 Then '20240118 Bin1 BinX merge, force passbinnum to 2
        CheckHaveBinCutSheet = 2
        Exit Function
    End If
...
Public Function Compare_ForceVal_BV(pin As String, val As String, Optional HLV As Boolean = True, Optional isUse_Product_Identifier As Boolean = False, Optional nBincutNum As Long = 999, Optional bIsHighLeakage As Boolean = False) As Variant
...
'    TheExec.DataManager.DecomposePinList Pin, Pin_ary, Pin_cnt
    ...
    If val = "" Then                        'Use BinCut limit need to set blank in flow
        ...
        If powerDomain <> "" Then
            If UCase(currentJobName) Like "*CP1*" Or bIsHighLeakage = True Then
                If UCase("*," & FullCorePowerinFlowSheet & ",*") Like UCase("*," & powerDomain & ",*") Then
                    '20211214,Modify for sram pmode is core power
                    If dict_IsCorePowerInBinCutFlowSheet.item(powerDomain) = True And UCase(powerDomain) Like "*VDD*_SRAM*" Then
                        highest_Pmode_Name = BinCut_Sram_Power_Seq(BinCut_Sram_Power_KeyMapping(powerDomain))(UBound(BinCut_Sram_Power_Seq(BinCut_Sram_Power_KeyMapping(powerDomain))))
                    Else
                        '''ex: "MC610" is the highest p_mode of "VDD_PCPU" in BinCut flow table "Non_Binning_Raill"
                        highest_Pmode_Name = BinCut_Power_Seq(VddBinStr2Enum(powerDomain)).Power_Seq(UBound(BinCut_Power_Seq(VddBinStr2Enum(powerDomain)).Power_Seq))
                    End If
                    If (nBincutNum = -999) Then
                        Hilimit_IDS = -999
                        Lolimit_IDS = -999
                        TheExec.Datalog.WriteComment "Get wrong Prouct Identifier!!!"
                    Else
                        If isUse_Product_Identifier Or nBincutNum = 999 Then
                            Hilimit_IDS = AllBinCut(VddBinStr2Enum(highest_Pmode_Name)).IDS_CP_LIMIT / 1000
                            Lolimit_IDS = 0.01 * AllBinCut(VddBinStr2Enum(highest_Pmode_Name)).IDS_CP_LIMIT / 1000  'Use 1% CPIDSMax for CP1/FT1. Mail from Ofer 20210706 1708
                        Else
                            Hilimit_IDS = BinCut(VddBinStr2Enum(highest_Pmode_Name), 1).IDS_CP_LIMIT(0) / 1000
                            Lolimit_IDS = 0.01 * BinCut(VddBinStr2Enum(highest_Pmode_Name), 1).IDS_CP_LIMIT(0) / 1000  'Use 1% CPIDSMax for CP1/FT1. Mail from Ofer 20210706 1708
                        End If
                    End If
                ElseIf UCase("*," & FullOtherRailinFlowSheet & ",*") Like UCase("*," & powerDomain & ",*") Then
                    ...
                    Else
                        If isUse_Product_Identifier Or nBincutNum = 999 Then
                            Hilimit_IDS = AllBinCut(VddBinStr2Enum(powerDomain)).IDS_CP_LIMIT / 1000
                            Lolimit_IDS = 0.01 * AllBinCut(VddBinStr2Enum(powerDomain)).IDS_CP_LIMIT / 1000  'Use 1% CPIDSMax for CP1/FT1. Mail from Ofer 20210706 1708
                        Else
                            Hilimit_IDS = CPIDS_Spec(VddBinStr2Enum(powerDomain), 1) / 1000
                            Lolimit_IDS = 0.01 * CPIDS_Spec(VddBinStr2Enum(powerDomain), 1) / 1000  'Use 1% CPIDSMax for CP1/FT1. Mail from Ofer 20210706 1708
                        End If
                    End If
                Else
...
''''    '                theexec.Flow.TestLimit resultVal:=999, lowval:=1, hiVal:=1, Tname:=Pin & "is not BinCut CorePower or OtherRail. Please check DCVS_IDS_main_auto_range_and_measure.Error!!!"
                End If
            ElseIf UCase(currentJobName) Like "*FT1*" Or UCase(currentJobName) Like "*WLFT*" Then
                If UCase("*," & FullCorePowerinFlowSheet & ",*") Like UCase("*," & powerDomain & ",*") Then
                    '20211214,Modify for sram pmode is core power
                    If dict_IsCorePowerInBinCutFlowSheet.item(powerDomain) = True And UCase(powerDomain) Like "*VDD*_SRAM*" Then
                        highest_Pmode_Name = BinCut_Sram_Power_Seq(BinCut_Sram_Power_KeyMapping(powerDomain))(UBound(BinCut_Sram_Power_Seq(BinCut_Sram_Power_KeyMapping(powerDomain))))
                    ...
                    If (nBincutNum = -999) Then
                        ...
                        TheExec.Datalog.WriteComment "Get wrong Prouct Identifier!!!"
                    Else
                        If isUse_Product_Identifier = False And nBincutNum = 999 Then
                            Hilimit_IDS = AllBinCut(VddBinStr2Enum(highest_Pmode_Name)).IDS_CP_LIMIT / 1000
                            Lolimit_IDS = 0.01 * AllBinCut(VddBinStr2Enum(highest_Pmode_Name)).IDS_CP_LIMIT / 1000  'Use 1% CPIDSMax for CP1/FT1. Mail from Ofer 20210706 1708
                        '''20240116'''
                        ElseIf Flag_BinX_Info_Parsed = True Then 'Bin1 BinX merge feature is true
                            If nBincutNum = 1 And Pmode2BinX_IDS_Dict.Exists(VddBinStr2Enum(highest_Pmode_Name)) = True Then 'Bin1 in Product_Identifier
                                Hilimit_IDS = Pmode2BinX_IDS_Dict(VddBinStr2Enum(highest_Pmode_Name)) / 1000
                                Lolimit_IDS = 0.01 * Pmode2BinX_IDS_Dict(VddBinStr2Enum(highest_Pmode_Name)) / 1000
                            ElseIf nBincutNum = 1 And Pmode2BinX_IDS_Dict.Exists(VddBinStr2Enum(highest_Pmode_Name)) = False Then
                                Call Print_Error_Message(Error_Info, "LIB_Common_AP", "Compare_ForceVal_BV", "Can't find Bin1 IDS limit for pmode" & highest_Pmode_Name & ", so use BinX IDS limit!!!")
                                Hilimit_IDS = BinCut(VddBinStr2Enum(highest_Pmode_Name), nBincutNum).IDS_CP_LIMIT(0) / 1000
                                Lolimit_IDS = 0.01 * BinCut(VddBinStr2Enum(highest_Pmode_Name), nBincutNum).IDS_CP_LIMIT(0) / 1000  'Use 1% CPIDSMax for CP1/FT1. Mail from Ofer 20210706 1708
                            Else 'BinX in Product_Identifier
                                Hilimit_IDS = BinCut(VddBinStr2Enum(highest_Pmode_Name), 1).IDS_CP_LIMIT(0) / 1000
                                Lolimit_IDS = 0.01 * BinCut(VddBinStr2Enum(highest_Pmode_Name), 1).IDS_CP_LIMIT(0) / 1000  'Use 1% CPIDSMax for CP1/FT1. Mail from Ofer 20210706 1708
                            End If
                        Else 'Bin1 BinX merge feature is false
                            Hilimit_IDS = BinCut(VddBinStr2Enum(highest_Pmode_Name), nBincutNum).IDS_CP_LIMIT(0) / 1000
                            Lolimit_IDS = 0.01 * BinCut(VddBinStr2Enum(highest_Pmode_Name), nBincutNum).IDS_CP_LIMIT(0) / 1000  'Use 1% CPIDSMax for CP1/FT1. Mail from Ofer 20210706 1708
                        End If
                        '''20240116'''
                ...
                ElseIf UCase("*," & FullOtherRailinFlowSheet & ",*") Like UCase("*," & powerDomain & ",*") Then
                    ...
                    Else
                        ...
                        '''20240116'''
                        ElseIf Flag_BinX_Info_Parsed = True Then 'Bin1 BinX merge feature is true
                            If nBincutNum = 1 And Pmode2BinX_IDS_Dict.Exists(VddBinStr2Enum(powerDomain)) = True Then 'Bin1 in Product_Identifier
                                Hilimit_IDS = Pmode2BinX_IDS_Dict(VddBinStr2Enum(powerDomain)) / 1000
                                Lolimit_IDS = 0.01 * Pmode2BinX_IDS_Dict(VddBinStr2Enum(powerDomain)) / 1000
                            ElseIf nBincutNum = 1 And Pmode2BinX_IDS_Dict.Exists(VddBinStr2Enum(powerDomain)) = False Then
                                Call Print_Error_Message(Error_Info, "LIB_Common_AP", "Compare_ForceVal_BV", "Can't find Bin1 IDS limit for pmode" & powerDomain & ", so use BinX IDS limit!!!")
                                Hilimit_IDS = CPIDS_Spec(VddBinStr2Enum(powerDomain), nBincutNum) / 1000
                                Lolimit_IDS = 0.01 * CPIDS_Spec(VddBinStr2Enum(powerDomain), nBincutNum) / 1000  'Use 1% CPIDSMax for CP1/FT1. Mail from Ofer 20210706 1708
                            Else 'BinX in Product_Identifier
                                Hilimit_IDS = CPIDS_Spec(VddBinStr2Enum(powerDomain), 1) / 1000
                                Lolimit_IDS = 0.01 * CPIDS_Spec(VddBinStr2Enum(powerDomain), 1) / 1000  'Use 1% CPIDSMax for CP1/FT1. Mail from Ofer 20210706 1708
                            End If
...
''''    '                theexec.Flow.TestLimit resultVal:=999, lowval:=1, hiVal:=1, Tname:=Pin & "is not BinCut CorePower or OtherRail. Please check DCVS_IDS_main_auto_range_and_measure.Error!!!"
            ...
            ElseIf UCase(currentJobName) Like "*CP2*" Or UCase(currentJobName) Like "*FT2*" Then
                If UCase("*," & FullCorePowerinFlowSheet & ",*") Like UCase("*," & powerDomain & ",*") Then
                    '20211214,Modify for sram pmode is core power
                    If dict_IsCorePowerInBinCutFlowSheet.item(powerDomain) = True And UCase(powerDomain) Like "*VDD*_SRAM*" Then
                        highest_Pmode_Name = BinCut_Sram_Power_Seq(BinCut_Sram_Power_KeyMapping(powerDomain))(UBound(BinCut_Sram_Power_Seq(BinCut_Sram_Power_KeyMapping(powerDomain))))
                    ...
                    Else
                        If isUse_Product_Identifier = False And nBincutNum = 999 Then
                            Hilimit_IDS = AllBinCut(VddBinStr2Enum(highest_Pmode_Name)).IDS_FT_LIMIT / 1000
                            Lolimit_IDS = 0.01 * AllBinCut(VddBinStr2Enum(highest_Pmode_Name)).IDS_FT_LIMIT / 1000         'Use 1% IDSMax_HOT for CP1/FT1. Mail from Ofer 20210706 1708
                        '''20240116'''
                        ElseIf Flag_BinX_Info_Parsed = True Then 'Bin1 BinX merge feature is true
                            If nBincutNum = 1 And Pmode2BinX_IDSHot_Dict.Exists(VddBinStr2Enum(highest_Pmode_Name)) = True Then 'Bin1 in Product_Identifier
                                Hilimit_IDS = Pmode2BinX_IDSHot_Dict(VddBinStr2Enum(highest_Pmode_Name)) / 1000
                                Lolimit_IDS = 0.01 * Pmode2BinX_IDSHot_Dict(VddBinStr2Enum(highest_Pmode_Name)) / 1000
                            ElseIf nBincutNum = 1 And Pmode2BinX_IDSHot_Dict.Exists(VddBinStr2Enum(highest_Pmode_Name)) = False Then
                                Call Print_Error_Message(Error_Info, "LIB_Common_AP", "Compare_ForceVal_BV", "Can't find Bin1 IDS limit for pmode" & highest_Pmode_Name & ", so use BinX IDS limit!!!")
                                Hilimit_IDS = BinCut(VddBinStr2Enum(highest_Pmode_Name), nBincutNum).IDS_FT_LIMIT(0) / 1000
                                Lolimit_IDS = 0.01 * BinCut(VddBinStr2Enum(highest_Pmode_Name), nBincutNum).IDS_FT_LIMIT(0) / 1000  'Use 1% CPIDSMax for CP1/FT1. Mail from Ofer 20210706 1708
                            Else 'BinX in Product_Identifier
                                Hilimit_IDS = BinCut(VddBinStr2Enum(highest_Pmode_Name), 1).IDS_FT_LIMIT(0) / 1000
                                Lolimit_IDS = 0.01 * BinCut(VddBinStr2Enum(highest_Pmode_Name), 1).IDS_FT_LIMIT(0) / 1000  'Use 1% CPIDSMax for CP1/FT1. Mail from Ofer 20210706 1708
                            End If
                        '''20240116'''
                        Else
                            Hilimit_IDS = BinCut(VddBinStr2Enum(highest_Pmode_Name), nBincutNum).IDS_FT_LIMIT(0) / 1000
                            Lolimit_IDS = 0.01 * BinCut(VddBinStr2Enum(highest_Pmode_Name), nBincutNum).IDS_FT_LIMIT(0) / 1000         'Use 1% IDSMax_HOT for CP1/FT1. Mail from Ofer 20210706 1708
                        End If
                    End If
                ElseIf UCase("*," & FullOtherRailinFlowSheet & ",*") Like UCase("*," & powerDomain & ",*") Then
                    If (nBincutNum = -999) Then
                    ...
                    Else
                        If isUse_Product_Identifier = False And nBincutNum = 999 Then
                            Hilimit_IDS = AllBinCut(VddBinStr2Enum(powerDomain)).IDS_FT_LIMIT / 1000
                            Lolimit_IDS = 0.01 * AllBinCut(VddBinStr2Enum(powerDomain)).IDS_FT_LIMIT / 1000
                        '''20240116'''
                        ElseIf Flag_BinX_Info_Parsed = True Then 'Bin1 BinX merge feature is true
                            If nBincutNum = 1 And Pmode2BinX_IDSHot_Dict.Exists(VddBinStr2Enum(powerDomain)) = True Then 'Bin1 in Product_Identifier
                                Hilimit_IDS = Pmode2BinX_IDSHot_Dict(VddBinStr2Enum(powerDomain)) / 1000
                                Lolimit_IDS = 0.01 * Pmode2BinX_IDSHot_Dict(VddBinStr2Enum(powerDomain)) / 1000
                            ElseIf nBincutNum = 1 And Pmode2BinX_IDSHot_Dict.Exists(VddBinStr2Enum(powerDomain)) = False Then
                                Call Print_Error_Message(Error_Info, "LIB_Common_AP", "Compare_ForceVal_BV", "Can't find Bin1 IDS limit for pmode" & powerDomain & ", so use BinX IDS limit!!!")
                                Hilimit_IDS = FTIDS_Spec(VddBinStr2Enum(powerDomain), nBincutNum) / 1000
                                Lolimit_IDS = 0.01 * FTIDS_Spec(VddBinStr2Enum(powerDomain), nBincutNum) / 1000  'Use 1% CPIDSMax for CP1/FT1. Mail from Ofer 20210706 1708
                            Else 'BinX in Product_Identifier
                                Hilimit_IDS = FTIDS_Spec(VddBinStr2Enum(powerDomain), 1) / 1000
                                Lolimit_IDS = 0.01 * FTIDS_Spec(VddBinStr2Enum(powerDomain), 1) / 1000  'Use 1% CPIDSMax for CP1/FT1. Mail from Ofer 20210706 1708
                            End If
                        '''20240116'''
                        Else
                            Hilimit_IDS = FTIDS_Spec(VddBinStr2Enum(powerDomain), nBincutNum) / 1000
                            Lolimit_IDS = FTIDS_Spec(VddBinStr2Enum(powerDomain), nBincutNum) / 1000                'Use 1% IDSMax_HOT for CP1/FT1. Mail from Ofer 20210706 1708
                        End If
                    End If
                Else
''''    '                theexec.Datalog.WriteComment Pin & "is not BinCut CorePower or OtherRail. Please check DCVS_IDS_main_auto_range_and_measure.Error!!!"
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/18
    If power_pin = "VDD_FIXED_ALL" Then power_pin = "VDD_FIXED"
...
On Error GoTo errHandler
    ...
    Dim CMEMValueIllegal As Boolean
    ...
    For Each v_site In TheExec.sites
        If Sbln_PatternPass(v_site) = False Then
            ...
            CMEMValueIllegal = False
            If UBound(indexArr) < 0 Then
                CMEMValueIllegal = True
            ElseIf indexArr(0) > UBound(dbl_CmemCycleDataArr) Then
                CMEMValueIllegal = True
            End If
            ...
            If CMEMValueIllegal = True Then
                TheExec.Datalog.WriteComment "SFC1,1," & CStr(v_site) & " CMEM is out of range!"
            ElseIf n_CycleCount > 1 Then
                For i = 0 To n_CycleCount - 1
                    TheExec.Datalog.WriteComment "SFC1,1," & CStr(v_site) & "," & mid(FlowName, 6, Len(FlowName)) & "," & s_InsName & "," & s_ModName _
                ...
                Next i
            ElseIf glb_CheckSSNToSFC = True Then
                TheExec.Datalog.WriteComment "SFC1,1," & CStr(v_site) & "," & mid(FlowName, 6, Len(FlowName)) & "," & s_InsName & "," & s_ModName _
                                            & "," & 1 & "," & CStr(dbl_CmemCycleDataArr(indexArr(0))) & "," & FailedPins
