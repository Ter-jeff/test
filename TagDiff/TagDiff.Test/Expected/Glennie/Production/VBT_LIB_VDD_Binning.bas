...
'''//==================================================================================================================================================================================//'''
    ...
    If TheExec.enableWord("T0TX_ROOM") = True Or TheExec.enableWord("T0TX_HOT") = True Then FlagInitVddBinningTable = False
    ...
    If FlagInitVddBinningTable = False Then
        ...
        Parsing_Static_MappingTable
        
        '20240124: Added for new MFSTP sheet
        Parsing_UserFunction_Sheet
    ...
    For p_mode = 0 To MaxPerformanceModeCount - 1
        VBIN_RESULT(p_mode).step_BINX_MFSTP = -1 '20240403 initial store BINX step for each pmode for MFSTP harvest on BIN1 but pass on BINX
    ...
    ReDim Bincut_Domain2BinX(MaxBincutPowerdomainCount)
...
Public Function PrintOut_VDD_BIN(Optional remove_printing_power As Boolean = False)
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    Dim site As Variant
    Dim p_mode As Integer
    ...
    Dim remove_printing_power_Result As New SiteDouble
    remove_printing_power_Result = 1
'''//==================================================================================================================================================================================//'''
'''//Note:
...
'''//==================================================================================================================================================================================//'''
            ...
            If IsExcludedVddBin(p_mode) = False And _
                ...
                If str_Efuse_read_pmode <> "" Or str_Efuse_write_pmode <> "" Then
                    ...
                    If TheExec.enableWord("T0TX_ROOM") = True And Not TheExec.enableWord("HardIP_RMA") = True Then 'WWWWW
                        dbl_GB_BinCutJob = BinCut(p_mode, CurrentPassBinCutNum).HTOL_RO_GB_ROOM(0)
                    ...
                    If TheExec.enableWord("T0TX_HOT") = True And Not TheExec.enableWord("HardIP_RMA") = True Then 'WWWWW
                        dbl_GB_BinCutJob = BinCut(p_mode, CurrentPassBinCutNum).HTOL_RO_GB_HOT(0)
                        str_PmodeGradeTestJob = VddBinName(p_mode) & bincutJobName
                    End If
                    ...
                    If str_PmodeGradeTestJob <> "" Then
                        ...
                        If remove_printing_power = True Then
                            If str_Efuse_write_pmode <> "" Then
                                If (VBIN_RESULT(p_mode).GRADE < dbl_CPVmin) Or (VBIN_RESULT(p_mode).GRADE > dbl_CPVmax) Then
                                    remove_printing_power_Result(site) = 0
                                End If
                            Else
                                If (VBIN_RESULT(p_mode).GRADE < (dbl_gradeVdd_lolimit - dbl_GB_BinCutJob)) Or (VBIN_RESULT(p_mode).GRADE > (dbl_gradeVdd_hilimit - dbl_GB_BinCutJob)) Then
                                    remove_printing_power_Result(site) = 0
                                End If
                            End If
                            
                            If (VBIN_RESULT(p_mode).GRADEVDD < dbl_gradeVdd_lolimit) Or (VBIN_RESULT(p_mode).GRADEVDD > dbl_gradeVdd_hilimit) Then
                                remove_printing_power_Result(site) = 0
                            End If
                        Else
                        ...
                        '''20210812: C651 Toby updated the rules that Product voltage=BinCut voltage+binning_GuardBand.
                        '''20210813: Modified to use BVmin(CPVmin) and BVmax(CPVmax) as lo_limit and hi_limit of BinCut voltage for p_mode, as C651 Toby's new rules about Guardband and product voltage.
                            If str_Efuse_write_pmode <> "" Then ' And Not LCase(TheExec.CurrentJob) = "wlft1"
    ...
    If remove_printing_power = True Then
        TheExec.flow.TestLimit remove_printing_power_Result, 1, 1, Tname:=" VDD PrintOut Result"
    End If
...
'''//Warning!!! This block is the special case for Crete A0 only, and it will be removed while Crete B0!!!
'''20210924: Modified to check product voltages of performance modes for BinCut powerDomain "VDD_PCPU" only while testJob="wlft2" in Crete A0.
'''20230705: Modified to clear all the product value since we need to do all the performance mode bin search while testJob="wlft2" in Tahiti.
'                If LCase(TheExec.CurrentJob) = "wlft1" Then
'                    '''20210928: Modified to check if p_mode is for BinCut search, as requested by TSMC ZYLINI.
'                    '''<org>
'                        'If LCase(AllBinCut(p_mode).powerPin) <> "vdd_pcpu" Then
'                    '''<new>
'                    If AllBinCut(p_mode).is_for_BinSearch = True Or LCase(VddBinName(p_mode)) Like "vdd_cpu_sram_mes*" Then
'                        str_Efuse_read_pmode = vbNullString
'                    End If
'                End If

                If LCase(TheExec.CurrentJob) = "wlft1" And is_BinCutJob_for_StepSearch = True Then
                    str_Efuse_read_pmode = vbNullString
                End If
'''********************************************************************************************************'''
...
'                    End If
'
'                    If bincutJobName = "ft_hot" And Flag_Vddbin_FT2_Shadow_NoBinout = True Then
'                        '''20220922: Coll FT2 uses shadow voltage (non-PCPU), no binout
'                        If Not UCase(VddBinName(p_mode)) Like "*PCPU*" And Not UCase(VddBinName(p_mode)) Like "*SRAM*" Then
'                            str_Efuse_read_pmode = get_Efuse_category_by_BinCut_testJob("read", VddBinName(p_mode)) & "_SHADOW"
'                        End If
'                    End If
'''********************************************************************************************************'''
                    ...
                    If TheExec.enableWord("T0TX_ROOM") = True And Not TheExec.enableWord("HardIP_RMA") = True Then dbl_GB_BinCutJob = BinCut(p_mode, CurrentPassBinCutNum).HTOL_RO_GB_ROOM(0) 'WWWWW20220631 T0TX
                    If TheExec.enableWord("T0TX_HOT") = True And Not TheExec.enableWord("HardIP_RMA") = True Then dbl_GB_BinCutJob = BinCut(p_mode, CurrentPassBinCutNum).HTOL_RO_GB_HOT(0) 'WWWWW20220631  T0TX
    ...
    If str_Efuse_read_PowerBinning <> "" Then
        ...
        For Each site In TheExec.sites
            ...
            ElseIf siteLong_fusepowerBinning(site) = 1 Then
                TheExec.sites.item(site).FlagState("F_PWRBIN_LOW") = logicFalse
                                ...
                                If Pmode2BinX_IDS_Dict.Exists(VddBinStr2Enum(powerDomain)) Then
                                    ...
                                    Else 'BinX IDS judge
                                        TheExec.flow.TestLimit IDS_for_BinCut(VddBinStr2Enum(powerDomain)).Real(site) / 1000, _
                                                    ids_lo_limit(VddBinStr2Enum(powerDomain)) / 1000, Pmode2BinX_IDS_Dict(VddBinStr2Enum(powerDomain)) / 1000, , _
...
Public Function adjust_VddBinning(Adjust_Max_Enable As Boolean, Adjust_Min_Enable As Boolean, Optional Adjust_Power_Max_list As String, Optional Adjust_Power_Min_list As String, Optional remove_printing_power As Boolean = False)
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    Dim site As Variant
    Dim p_mode As Integer
...
'''//==================================================================================================================================================================================//'''
    ...
    '''//Force to get non-shadow prodcut_identifier for CP1
    str_Efuse_write_ProductIdentifier = "Product_Identifier"
    ...
    ReDim strBINXOfPowerDomain(MaxBincutPowerdomainCount * site_num) As String
            ...
            Else
                ...
                (AllBinCut(p_mode).Used = True Or (dict_IsCorePower(AllBinCut(p_mode).powerPin) = True And UCase(AllBinCut(p_mode).powerPin) Like "*VDD*_SRAM*")) Then
                    ...
                    If str_Efuse_write_pmode <> "" Then
                        ...
                        If remove_printing_power = True Then
                            If VBIN_RESULT(p_mode).passBinCut < 1 Or VBIN_RESULT(p_mode).passBinCut > PassBinCut_ary(UBound(PassBinCut_ary)) Then
                                remove_printing_power_Result(site) = 0
                            End If
                            
                            If (VBIN_RESULT(p_mode).step_in_BinCut + 1) < 1 Or (VBIN_RESULT(p_mode).step_in_BinCut + 1) > EQN_lowest Then
                                remove_printing_power_Result(site) = 0
                            End If
                        Else
                            '''//PassBinNumber.
                        ...
                        If remove_printing_power = True Then
                            ...
                            If Flag_Get_column_Monotonicity_Offset = True Then
                                If Abs(CLng(VBIN_RESULT(p_mode).is_Monotonicity_Offset_triggered(site))) < 0 Or Abs(CLng(VBIN_RESULT(p_mode).is_Monotonicity_Offset_triggered(site))) > 1 Then
                                    remove_printing_power_Result(site) = 0
                                End If
                            End If
                        Else
                        ...
                        If remove_printing_power = True Then
                            If IDS_for_BinCut(VddBinStr2Enum(powerDomain)).Real(site) < 0 Or IDS_for_BinCut(VddBinStr2Enum(powerDomain)).Real(site) > dbl_ids_hi_limit Then
                                remove_printing_power_Result(site) = 0
                            End If
                        Else
...
'                        '''20230504: Decide correct bin number by EQN result
'                        If Flag_BinX_Info_Parsed = True And Flag_Fuse_BinX(site) = False Then
'                            If Pmode2BinX_IDS_Dict.Exists(p_mode) = True Then
...
'                    End If
    ...
    If remove_printing_power = True Then
        TheExec.flow.TestLimit remove_printing_power_Result, 1, 1, Tname:=" VDD Adjust Result"
    End If
...
'''20210908: Modified to add the special case to fuse the product_identifier and product voltages for testJob "wlft2" of T-Cre.
    If LCase(TheExec.CurrentJob) = "wlft1" Then
        str_Efuse_write_ProductIdentifier = vbNullString
    End If
'''********************************************************************************************************'''
    ...
    For Each site In TheExec.sites
        ...
        For i = 0 To MaxBincutPowerdomainCount - 1
...
Public Function GradeSearch_VT(FuncPat As Pattern, performance_mode As String, result_mode As tlResultMode, DecomposePatt As String, _
                               ...
                               Optional HarvPinGrp_Enable As Boolean = False, _
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    ''' auto shmoo initialize
    If Not autoShmooUtility.IsRunningShmoo Then
        Set autoShmoo = New AutoShmooMain
        autoShmoo.ShmooItem.AddPattern PrePatt, FuncPat
    Else
        autoShmoo.TestInstance
        If autoShmoo.IsEnableAutoShmoo Then Exit Function
    End If
    ...
    inst_info.enable_HarvPinGrp = HarvPinGrp_Enable
    ...
    If inst_info.enable_HarvPinGrp = True And Flag_HarvPinFlag_Mapping_Table_Parsed = False Then
        Call Print_Error_Message(Error_Info, "VBT_LIB_VDD_Binning", "GradeSearch_VT", "TestInst Argument 'HarvPinGrp_Enable' = True, but can't find HarvPinGrp Table! Error!!")
    End If
    ...
    '''vvv 220125 for Si request: when Bin4 only run Bin1 EQN 1
    For Each site In TheExec.sites
        If TheExec.sites.item(site).FlagState("BIN4_CAND") = logicTrue Then
            If inst_info.StartBin1E1_Flag(site) = False Then inst_info.StartBin1E1_Flag = True
        End If
    Next site
    ...
    '''//Prevent no any site active and cause error. This is due to addjust voltage will shut down site if it's from bin1 to binx in WLFT
...
'**********************************************
            For indexPatt = 0 To inst_info.count_FuncPat_decomposed - 1
                ...
                If inst_info.is_BinSearch = True Then
                    ...
                    '''==============================================================================================================================
                    '''20210225: Modified to exit pattern-loop if all site fail and "inst_info.is_BV_Payload_Voltage_printed = True".
                    '''20220125,HarvPinGrp need to do all payload, it could optimal for initial pat. if init pat all site fail, could go next step directly
...
'**********************************************
        ...
        If inst_info.enable_CMEM_Collection = True And inst_info.AllSiteFailPatt > 0 Then
            ...
            If CollectOnEachStep = True Then Call PostTestIPF(inst_info.performance_mode, failpins, inst_info.PrintSize, inst_info.Step_CMEM_Data, inst_info.str_patternPins, inst_info.PrintSize)
        End If
...
Public Function GradeSearch_HVCC_VT(FuncPat As Pattern, performance_mode As String, result_mode As tlResultMode, DecomposePatt As String, _
                                    ...
                                    Optional HarvestBinningFlag As String = vbNullString, _
                                    Optional HarvPinGrp_Enable As Boolean = False, _
                                    Optional HarvPinGrp_FailFlag As String = vbNullString, Optional failpins As String = vbNullString, Optional CaptureSize As Long, Optional UserFunction As String = vbNullString)
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    Dim site As Variant
    Dim inst_info As Instance_Info
    Dim indexPatt As Long
    ...
    Call check_arg_to_enable_HarvPinGrp(inst_info)    '20220512, force initial cmem for harvpingrp
...
'**********************************************
        For indexPatt = 0 To inst_info.count_FuncPat_decomposed - 1
            ...
            Else
                ...
                If inst_info.is_DevChar_Running = False Then '''for DevChar.
                    For Each site In TheExec.sites
                        If inst_info.funcPatPass(site) = False And inst_info.enable_HarvPinGrp = False Then
                            ...
                            Else
                                If inst_info.HarvestBinningFlag = "" Then '''HarvestBinning
        ...
        If inst_info.HarvestBinningFlag <> "" And inst_info.enable_HarvPinGrp = False Then
            For Each site In TheExec.sites
                If inst_info.funcPatPass(site) = False Then  '''HarvestBinning
                    '''20230821: Modified to support multi harvest flag, other pin group
                    ...
                    If strGlb_Flag_HarvestBinningFlag_AllCorePass <> "" Then
                        ...
                        Else '''If more than two Cores fails, bin out the failed DUT...
                            ...
                            'TheExec.sites.item(site).testResult = siteFail
                            TheExec.sites.item(site).FlagState(HarvPinGrp_FailFlag) = logicTrue
                            TheExec.sites.item(site).FlagState(strGlb_Flag_Vddbinning_Fail_Stop) = logicTrue
                        End If
                    Else
...
'**********************************************
    ...
    Call CtrlFailFlag_byHarvPinGrpResult(inst_info, HarvPinGrp_FailFlag)
    
    autoShmoo.start: Set autoShmoo = Nothing ''' start auto shmo
    '==================================================
...
Public Function GradeSearch_postBinCut_VT(FuncPat As Pattern, performance_mode As String, result_mode As tlResultMode, DecomposePatt As String, _
                                        ...
                                        Optional Flag_RetWait As Boolean = False, Optional Dbl_RetWaitTime_mS As Double = 100, Optional failpins As String = vbNullString, Optional CaptureSize As Long = 0, Optional UserFunction As String = vbNullString, Optional InitVmain_PayloadValt As Boolean = False) ', _
...
'''//==================================================================================================================================================================================//'''
    ...
    inst_info.Harvest_Core_DigSrc_Pin = "JTAG_TDI"
    inst_info.Harvest_Core_DigSrc_SignalName = "Harvest_Core_DigSrcSignal"
    inst_info.InitVmain_PayloadValt = InitVmain_PayloadValt
    inst_info.Pat2DCVSOutput.RemoveAll
    ...
    If InitVmain_PayloadValt = True Then
        Dim i As Integer
        ReDim inst_info.InitValt_CorePowerStored(UBound(pinGroup_BinCut))
        
        For i = 0 To UBound(pinGroup_BinCut)
            inst_info.InitValt_CorePowerStored(i) = 0
        Next i
        
        Call select_Pattern_DCVS_output(inst_info.Pat2DCVSOutput, inst_info.ary_FuncPat_decomposed)
        save_core_power_vddbinning inst_info.InitValt_CorePowerStored
    End If
    ...
    If inst_info.InitVmain_PayloadValt = False Then
        Set_PayloadVoltage_to_DCVS Flag_Enable_Rail_Switch, pinGroup_BinCut, BinCut_Payload_Voltage
        TheHdw.Wait 0.001
    End If
    ...
    ''' Add for checking if we need to do pin mask based on pattern
        ...
        If EnableBinOut Then
            If LCase(inst_info.inst_name) Like "*_bv" Then '''BV instance
                If LCase(inst_info.inst_name) Like "*outsidebincut_bv" Or LCase(inst_info.inst_name) Like "*postbincut_bv" Then
                    judge_PF_func inst_info.p_mode, inst_info.Test_Type, inst_info.funcPatPass, True
                Else
                    judge_PF_func inst_info.p_mode, inst_info.Test_Type, inst_info.funcPatPass, False
                End If
            Else '''HBV instance
                For Each site In TheExec.sites
...
'''//==================================================================================================================================================================================//'''
    ...
    If Flag_BinCut_Config_Printed = False Then
        ...
        TheExec.Datalog.WriteComment "Flag_Vddbin_HarvestBin4_Run_Bin1EQN1" & "=" & CStr(True)
        ...
        '''20231016: Added for T-Tup adjusting voltage before power binning
        TheExec.Datalog.WriteComment "Flag_Enable_PreAdjustVddbinning" & "=" & CStr(True)
...
'''//==================================================================================================================================================================================//'''
    ...
    For p_mode = 0 To UBound(VddBinName)
        If LCase(VddBinName(p_mode)) Like "*m*#*" And Not LCase(VddBinName(p_mode)) Like "mbist*" Then
            str_Efuse_write = get_Efuse_category_by_BinCut_testJob("write", VddBinName(p_mode))
        Else
            str_Efuse_write = vbNullString
        End If
        '''//Check if p_mode is for BinCut search.
        If AllBinCut(p_mode).Used = True And AllBinCut(p_mode).is_for_BinSearch = True And str_Efuse_write <> "" Then
            For Each site In TheExec.sites
                '''init
                selected_pmode = 0
            ...
            Next site
        '20211022: Added to support SRAM Voltage calculate with equation
        ElseIf dict_IsCorePower(AllBinCut(p_mode).powerPin) = True And UCase(AllBinCut(p_mode).powerPin) Like "*VDD*_SRAM*" And str_Efuse_write <> "" Then
            For Each site In TheExec.sites
                If VBIN_RESULT(p_mode).tested = False Then
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    '''for testNumber alignment
    Dim Org_Test_Number As Long
    '''for calculate instance have how many test numbers
...
'''//==================================================================================================================================================================================//'''
    ...
    If Not autoShmooUtility.IsRunningShmoo Then Set autoShmoo = New AutoShmooMain
    '''init
            ...
            If DYNAMIC_VBIN_IDS_ZONE(p_mode).Used(site) = True Then
                ...
                If VBIN_RESULT(p_mode).tested(site) = True Then
                    ...
                    For idx_step = 0 To DYNAMIC_VBIN_IDS_ZONE(p_mode).Max_Step(site)
                        If CDec(DYNAMIC_VBIN_IDS_ZONE(p_mode).Product_Voltage(idx_step)) = CDec(VBIN_RESULT(p_mode).GRADEVDD) Then
                            ...
                            Else
                                If (dict_IsCorePower(AllBinCut(p_mode).powerPin) = True And UCase(AllBinCut(p_mode).powerPin) Like "*VDD*_SRAM*") And CurrentPassBinCutNum(site) = 2 Then
                                    VBIN_RESULT(p_mode).step_in_IDS_Zone = BinCut(p_mode, 2).Mode_Step
                                    VBIN_RESULT(p_mode).passBinCut = CurrentPassBinCutNum(site)
                                    VBIN_RESULT(p_mode).step_in_BinCut = BinCut(p_mode, 2).EQ_Num(BinCut(p_mode, 2).Mode_Step) - 1 '''step_in_BinCut=EQN-1
                                Else
                ...
                Else '''If product voltage of p_mode isn't fused, adjust the correct step to align passbincut of P_mode with CurrentPassBinCutNum, and update Grade/GradeVDD for p_mode.
                    ...
                    If VBIN_RESULT(p_mode).passBinCut <> CurrentPassBinCutNum(site) Then
                        '''20210810: Modified to skip printing the info about the step-adjusted voltage for p_mode, requested by C651 Si and TSMC ZYLINI.
                        Adjust_Multi_PassBinCut_Per_Site p_mode, site, CurrentPassBinCutNum(site), True, True
                    End If
...
Public Function Set_E1_Voltage_ForPmode(str_setE1_flag As String, power_pin As String, performance_mode As String)
    Dim site As Variant
    Dim str_pmode As String
    Dim p_mode As Long
On Error GoTo errHandler

    For Each site In TheExec.sites
        If TheExec.sites(site).FlagState(str_setE1_flag) = logicTrue Then
            ''init
            str_pmode = ""
            p_mode = -1
            If UCase(performance_mode) Like "M*#" Then
                str_pmode = performance_mode
            Else
                str_pmode = TheExec.flow.CurrentFlowSheetName
            End If
            
            If str_pmode <> "" Then
                If str_pmode Like "*_*" Then
                    str_pmode = UCase(Split(str_pmode, "_")(1))    '' Get 2nd element: MG001
                Else
                    str_pmode = UCase(str_pmode)
                End If
                
                If Not str_pmode Like "M*#" Then
                    TheExec.Datalog.WriteComment "Cannot find correct Pmode from flow name!! Instance:" & TheExec.DataManager.instancename & " is skipped!!"
                    Exit Function
                End If
                p_mode = VddBinStr2Enum(power_pin & "_" & str_pmode) '"VDD_GPU_" & str_pmode
                
                If VBIN_RESULT(p_mode).tested = False Then
                    '' Set Vbin Result for this P mode
                    VBIN_RESULT(p_mode).passBinCut = CurrentPassBinCutNum(site)     ''20230523: Added for potential binX for T-Don
                    VBIN_RESULT(p_mode).step_in_IDS_Zone = DYNAMIC_VBIN_IDS_ZONE(p_mode).step_Mapping(VBIN_RESULT(p_mode).passBinCut, 1)
                    VBIN_RESULT(p_mode).step_in_BinCut = DYNAMIC_VBIN_IDS_ZONE(p_mode).EQ_Num(VBIN_RESULT(p_mode).step_in_IDS_Zone) - 1
                    VBIN_RESULT(p_mode).GRADE = DYNAMIC_VBIN_IDS_ZONE(p_mode).Voltage(VBIN_RESULT(p_mode).step_in_IDS_Zone)
                    VBIN_RESULT(p_mode).GRADEVDD = DYNAMIC_VBIN_IDS_ZONE(p_mode).Product_Voltage(VBIN_RESULT(p_mode).step_in_IDS_Zone)
                    CurrentPassBinCutNum(site) = VBIN_RESULT(p_mode).passBinCut
                    VBIN_RESULT(p_mode).tested = True

                    TheExec.Datalog.WriteComment "site:" & site & "," & VddBinName(p_mode) & ", p_mode is not tested, set VBIN_RESULT of p_mode for Set_E1_Voltage_ForPmode. EQN=" & _
                                                    DYNAMIC_VBIN_IDS_ZONE(p_mode).EQ_Num(VBIN_RESULT(p_mode).step_in_IDS_Zone) & ", Grade=" & VBIN_RESULT(p_mode).GRADE & ", GradeVdd=" & VBIN_RESULT(p_mode).GRADEVDD
                End If
            End If
        End If
    Next site
Exit Function
errHandler:
    TheExec.Datalog.WriteComment "Error encountered in VBT Function of Set_E1_Voltage_ForPmode"
    If AbortTest Then Exit Function Else Resume Next
End Function
Public Function SetHarvestFlagForBV()
    On Error GoTo errHandler
    Dim funcName As String:: funcName = "SetHarvestFlagForBV"
    ...
    For Each site In TheExec.sites.Selected
        If str_Efuse_read_ProductIdentifier <> "" Then
            Set field = BdfDataBase.Bank_Cfg.Fields(str_Efuse_read_ProductIdentifier)
            If field.DsscDecValue = 1 Then
                TheExec.sites.item(site).FlagState("BINX_CAND") = logicTrue
                ''' Since outside bincut doesn't know passbincut number, the binx will be recognized as bin1 in decision flow
                If TheExec.sites.item(site).FlagState("BIN1_CAND") = logicTrue Then
                    TheExec.sites.item(site).FlagState("BIN1_CAND") = logicFalse
                End If
            End If
        End If
        
        ''' initialize the flag
        TheExec.sites.item(site).FlagState("F_SRVGF_BV") = logicFalse
        TheExec.sites.item(site).FlagState("F_Bin1_HARVEST_BV") = logicFalse
        TheExec.sites.item(site).FlagState("F_Bin13X_BV") = logicFalse
        TheExec.sites.item(site).FlagState("F_Bin2_ED_BV") = logicFalse
        TheExec.sites.item(site).FlagState("F_Gfx_Core0_Bincut") = logicFalse
        TheExec.sites.item(site).FlagState("F_Gfx_Core1_Bincut") = logicFalse
        TheExec.sites.item(site).FlagState("F_Gfx_Core2_Bincut") = logicFalse
        TheExec.sites.item(site).FlagState("F_Gfx_Core3_Bincut") = logicFalse
        TheExec.sites.item(site).FlagState("F_Gfx_Core4_Bincut") = logicFalse
        TheExec.sites.item(site).FlagState("F_Bin4_ID_BV") = logicFalse
        TheExec.sites.item(site).FlagState("F_Bin4_IS_BV") = logicFalse
        TheExec.sites.item(site).FlagState("F_Bin4_PCPU") = logicFalse
        TheExec.sites.item(site).FlagState("F_Bin4_ECPU") = logicFalse
        TheExec.sites.item(site).FlagState("F_Bin4_PCPU_BV") = logicFalse
        TheExec.sites.item(site).FlagState("F_Bin4_ECPU_BV") = logicFalse
        
        If TheExec.sites.item(site).FlagState("BIN4_CAND") = logicTrue Then
            ''' remove ID with IS combination from V04A
'            If TheExec.sites.item(site).FlagState("F_SOC_ID") = logicTrue _
'                    And TheExec.sites.item(site).FlagState("F_SOC_IS") = logicTrue Then
'                TheExec.sites.item(site).FlagState("F_Bin4_ID_IS_BV") = logicTrue
            If TheExec.sites.item(site).FlagState("F_SOC_ID") = logicTrue Then
                TheExec.sites.item(site).FlagState("F_Bin4_ID_BV") = logicTrue
            ElseIf TheExec.sites.item(site).FlagState("F_SOC_IS") = logicTrue Then
                TheExec.sites.item(site).FlagState("F_Bin4_IS_BV") = logicTrue
                TheExec.sites.item(site).FlagState("F_SRVGF_BV") = logicTrue
            ElseIf TheExec.sites.item(site).FlagState("F_SOC_ED") = logicFalse Then
                TheExec.sites.item(site).FlagState("F_Bin13X_BV") = logicTrue
            End If
            ''' remove ECPU + PCPU combination from V04A
'            If TheExec.sites.item(site).FlagState("F_ECPU_CORE_3_0_SUM_0") = logicFalse _
'                    And TheExec.sites.item(site).FlagState("F_PCPU_CORE_1_0_SUM_0") = logicFalse Then
'                TheExec.sites.item(site).FlagState("F_Bin4_ECPU_PCPU") = logicTrue
            If TheExec.sites.item(site).FlagState("F_ECPU_CORE_3_0_SUM_0") = logicFalse Then
                TheExec.sites.item(site).FlagState("F_Bin4_PCPU") = logicTrue
                TheExec.sites.item(site).FlagState("F_SRVGF_BV") = logicTrue
            ElseIf TheExec.sites.item(site).FlagState("F_PCPU_CORE_1_0_SUM_0") = logicFalse Then
                TheExec.sites.item(site).FlagState("F_Bin4_ECPU") = logicTrue
                TheExec.sites.item(site).FlagState("F_SRVGF_BV") = logicTrue
            End If
            ''' add for bin4 harvesting die to test corresponding item for all-core pass and prevent bin1 die to test it
            If TheExec.sites.item(site).FlagState("F_ECPU_CORE_3_0_SUM_0") = logicTrue Then
                TheExec.sites.item(site).FlagState("F_Bin4_ECPU_BV") = logicTrue
            End If
            If TheExec.sites.item(site).FlagState("F_PCPU_CORE_1_0_SUM_0") = logicTrue Then
                TheExec.sites.item(site).FlagState("F_Bin4_PCPU_BV") = logicTrue
            End If
            
            If Not (TheExec.sites.item(site).FlagState("F_Bin4_ID_BV") = logicTrue _
                    Or TheExec.sites.item(site).FlagState("F_Bin4_IS_BV") = logicTrue _
                    Or TheExec.sites.item(site).FlagState("F_Bin4_ECPU") = logicTrue _
                    Or TheExec.sites.item(site).FlagState("F_Bin4_PCPU") = logicTrue) Then
                TheExec.sites.item(site).FlagState(strGlb_Flag_Vddbinning_Fail_Stop) = logicTrue
                TheExec.Datalog.WriteComment "Site:" & site & ", Can not find corresponding flag combination for Bin4 Harvesting. Error!!!"
            End If
        ElseIf TheExec.sites.item(site).FlagState("BIN3_CAND") = logicTrue Then
            If Not TheExec.sites.item(site).FlagState("F_GFX_CORE_4_0_SUM_0") = logicTrue Then
                If TheExec.sites.item(site).FlagState("F_Gfx_Core0") = logicTrue Then
                    TheExec.sites.item(site).FlagState("F_Gfx_Core0_Bincut") = logicTrue
                ElseIf TheExec.sites.item(site).FlagState("F_Gfx_Core1") = logicTrue Then
                    TheExec.sites.item(site).FlagState("F_Gfx_Core1_Bincut") = logicTrue
                ElseIf TheExec.sites.item(site).FlagState("F_Gfx_Core2") = logicTrue Then
                    TheExec.sites.item(site).FlagState("F_Gfx_Core2_Bincut") = logicTrue
                ElseIf TheExec.sites.item(site).FlagState("F_Gfx_Core3") = logicTrue Then
                    TheExec.sites.item(site).FlagState("F_Gfx_Core3_Bincut") = logicTrue
                ElseIf TheExec.sites.item(site).FlagState("F_Gfx_Core4") = logicTrue Then
                    TheExec.sites.item(site).FlagState("F_Gfx_Core4_Bincut") = logicTrue
                Else
                    TheExec.sites.item(site).FlagState(strGlb_Flag_Vddbinning_Fail_Stop) = logicTrue
                    TheExec.Datalog.WriteComment "Site:" & site & ", Can not find GFX harvesting core number. Error!!!"
                End If
            Else
                TheExec.sites.item(site).FlagState(strGlb_Flag_Vddbinning_Fail_Stop) = logicTrue
                TheExec.Datalog.WriteComment "Site:" & site & ", Gfx harvesting with All-Core pass. Error!!!"
            End If
            TheExec.sites.item(site).FlagState("F_Bin13X_BV") = logicTrue
        ElseIf TheExec.sites.item(site).FlagState("BIN2_CAND") = logicTrue _
                And TheExec.sites.item(site).FlagState("F_SOC_ED") = logicTrue Then
            TheExec.sites.item(site).FlagState("F_Bin2_ED_BV") = logicTrue
        ElseIf TheExec.sites.item(site).FlagState("BIN3_CAND") = logicTrue _
                Or TheExec.sites.item(site).FlagState("BINX_CAND") = logicTrue Then
            TheExec.sites.item(site).FlagState("F_Bin13X_BV") = logicTrue
            TheExec.sites.item(site).FlagState("F_SRVGF_BV") = logicTrue
        ElseIf TheExec.sites.item(site).FlagState("BIN1_CAND") = logicTrue Then
            If TheExec.flow.enableWord("Enable_Vddbinning_Harvest") = True Then
                TheExec.sites.item(site).FlagState("F_Bin1_HARVEST_BV") = logicTrue
            Else
                TheExec.sites.item(site).FlagState("F_Bin13X_BV") = logicTrue
            End If
            TheExec.sites.item(site).FlagState("F_SRVGF_BV") = logicTrue
        Else
            TheExec.sites.item(site).FlagState(strGlb_Flag_Vddbinning_Fail_Stop) = logicTrue
            TheExec.Datalog.WriteComment "Site:" & site & ", Can not find corresponding flag combination for Any Harvesting . Error!!!"
        End If
        
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_ED")), 0, 1, Tname:="F_SOC_ED"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_ID")), 0, 1, Tname:="F_SOC_ID"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_IS")), 0, 1, Tname:="F_SOC_IS"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_ECPU_CORE_3_0_SUM_0")), 0, 1, Tname:="F_ECPU_CORE_3_0_SUM_0"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_PCPU_CORE_1_0_SUM_0")), 0, 1, Tname:="F_PCPU_CORE_1_0_SUM_0"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN1_CAND")), 0, 1, Tname:="BIN1_CAND"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN2_CAND")), 0, 1, Tname:="BIN2_CAND"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN3_CAND")), 0, 1, Tname:="BIN3_CAND"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN4_CAND")), 0, 1, Tname:="BIN4_CAND"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BINX_CAND")), 0, 1, Tname:="BINX_CAND"
...
End Function
Public Function SetHarvestFlagForBV_Coll()
    On Error GoTo errHandler
    Dim funcName As String:: funcName = "SetHarvestFlagForBV_Coll"
    ...
    For Each site In TheExec.sites.Selected
        ...
        ''' initialize the flag
        TheExec.sites.item(site).FlagState("F_SOC_NO_IS_PR") = logicFalse
        TheExec.sites.item(site).FlagState("F_SOC_NO_ID_ED") = logicFalse
        TheExec.sites.item(site).FlagState("F_SOC_NO_IS_ID_ED") = logicFalse
        TheExec.sites.item(site).FlagState("F_SOC_ANE_NoRepair") = logicFalse
        TheExec.sites.item(site).FlagState("F_Gfx_Core0_Bincut") = logicFalse
        TheExec.sites.item(site).FlagState("F_Gfx_Core1_Bincut") = logicFalse
        TheExec.sites.item(site).FlagState("F_Gfx_Core2_Bincut") = logicFalse
        TheExec.sites.item(site).FlagState("F_Gfx_Core3_Bincut") = logicFalse
        TheExec.sites.item(site).FlagState("F_Gfx_Core4_Bincut") = logicFalse
        TheExec.sites.item(site).FlagState("F_Gfx_Core5_Bincut") = logicFalse
        TheExec.sites.item(site).FlagState("F_Gfx_Core0_Ref") = logicFalse
        TheExec.sites.item(site).FlagState("F_Gfx_Core1_Ref") = logicFalse
        TheExec.sites.item(site).FlagState("F_Bin1_HARVEST_BV") = logicFalse
        ''' reset for gfx harvesting
        TheExec.sites.item(site).FlagState(strGlb_Flag_HarvestBinningFlag_SingleFSTP) = logicFalse
        
        ''' check for Bin1 to do TD harvesting inside bincut for CP1 only
        If (LCase(TheExec.CurrentJob) = "cp1" Or LCase(TheExec.CurrentJob) = "cp2") And TheExec.sites.item(site).FlagState("BIN1") = logicTrue Then
            TheExec.sites.item(site).FlagState("F_Bin1_HARVEST_BV") = logicTrue
        End If
        
        ''' check IS and PR for MS mode
        If Not TheExec.sites.item(site).FlagState("F_SOC_IS") = logicTrue And Not TheExec.sites.item(site).FlagState("F_SOC_PRO") = logicTrue Then
            TheExec.sites.item(site).FlagState("F_SOC_NO_IS_PR") = logicTrue
        End If
        
        ''' check ID and ED for MI mode
        If Not TheExec.sites.item(site).FlagState("F_SOC_ID") = logicTrue And Not TheExec.sites.item(site).FlagState("F_SOC_ED") = logicTrue Then
            TheExec.sites.item(site).FlagState("F_SOC_NO_ID_ED") = logicTrue
            ''' check IS, ID and ED for MS SRT post bincut
            If Not TheExec.sites.item(site).FlagState("F_SOC_IS") = logicTrue Then
                TheExec.sites.item(site).FlagState("F_SOC_NO_IS_ID_ED") = logicTrue
            End If
        End If
        
        ''' check GFX harvest for MFSTP
        If TheExec.sites.item(site).FlagState("BIN2") = logicTrue Then
            If Not TheExec.sites.item(site).FlagState("F_GFX_CORE_5_0_SUM_0") = logicTrue Then
                If TheExec.sites.item(site).FlagState("F_Gfx_Core0") = logicTrue Then
                    TheExec.sites.item(site).FlagState("F_Gfx_Core0_Bincut") = logicTrue
                    TheExec.sites.item(site).FlagState("F_Gfx_Core1_Ref") = logicTrue
                ElseIf TheExec.sites.item(site).FlagState("F_Gfx_Core1") = logicTrue Then
                    TheExec.sites.item(site).FlagState("F_Gfx_Core1_Bincut") = logicTrue
                    TheExec.sites.item(site).FlagState("F_Gfx_Core0_Ref") = logicTrue
                ElseIf TheExec.sites.item(site).FlagState("F_Gfx_Core2") = logicTrue Then
                    TheExec.sites.item(site).FlagState("F_Gfx_Core2_Bincut") = logicTrue
                    TheExec.sites.item(site).FlagState("F_Gfx_Core0_Ref") = logicTrue
                ElseIf TheExec.sites.item(site).FlagState("F_Gfx_Core3") = logicTrue Then
                ...
                ElseIf TheExec.sites.item(site).FlagState("F_Gfx_Core4") = logicTrue Then
                    TheExec.sites.item(site).FlagState("F_Gfx_Core4_Bincut") = logicTrue
                    TheExec.sites.item(site).FlagState("F_Gfx_Core0_Ref") = logicTrue
                ElseIf TheExec.sites.item(site).FlagState("F_Gfx_Core5") = logicTrue Then
                    TheExec.sites.item(site).FlagState("F_Gfx_Core5_Bincut") = logicTrue
                    TheExec.sites.item(site).FlagState("F_Gfx_Core0_Ref") = logicTrue
                Else
                    TheExec.sites.item(site).FlagState(strGlb_Flag_Vddbinning_Fail_Stop) = logicTrue
                    TheExec.Datalog.WriteComment "Site:" & site & ", Can not find GFX harvesting core number. Error!!!"
            ...
            End If
        Else
            If TheExec.sites.item(site).FlagState("F_GFX_CORE_5_0_SUM_0") = logicTrue Then
                TheExec.sites.item(site).FlagState("F_Gfx_Core0_Ref") = logicTrue
            Else
                TheExec.sites.item(site).FlagState(strGlb_Flag_Vddbinning_Fail_Stop) = logicTrue
                TheExec.Datalog.WriteComment "Site:" & site & ", Non Bin2 with Gfx Core fail. Error!!!"
            End If
        End If
        
        ''' check ANE repair for MP mode
        If Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH1") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH2") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH3") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH4") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH5") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH6") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH7") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH8") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH9") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH10") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH11") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH12") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH13") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH14") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH15") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH16") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ANE_GH17") = logicTrue Then
            TheExec.sites.item(site).FlagState("F_SOC_ANE_NoRepair") = logicTrue
        End If
        
        ''' print out summary
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN1")), 0, 1, Tname:="BIN1"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN2")), 0, 1, Tname:="BIN2"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN3")), 0, 1, Tname:="BIN3"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN4")), 0, 1, Tname:="BIN4"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN5")), 0, 1, Tname:="BIN5"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN6")), 0, 1, Tname:="BIN6"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN7")), 0, 1, Tname:="BIN7"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BINX")), 0, 1, Tname:="BINX"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_IS")), 0, 1, Tname:="F_SOC_IS"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_ID")), 0, 1, Tname:="F_SOC_ID"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_ED")), 0, 1, Tname:="F_SOC_ED"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_PRO")), 0, 1, Tname:="F_SOC_PRO"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_Gfx_Core0_Bincut")), 0, 1, Tname:="F_Gfx_Core0_Bincut"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_Gfx_Core1_Bincut")), 0, 1, Tname:="F_Gfx_Core1_Bincut"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_Gfx_Core2_Bincut")), 0, 1, Tname:="F_Gfx_Core2_Bincut"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_Gfx_Core3_Bincut")), 0, 1, Tname:="F_Gfx_Core3_Bincut"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_Gfx_Core4_Bincut")), 0, 1, Tname:="F_Gfx_Core4_Bincut"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_Gfx_Core5_Bincut")), 0, 1, Tname:="F_Gfx_Core5_Bincut"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_Gfx_Core0_Ref")), 0, 1, Tname:="F_Gfx_Core0_Ref"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_Gfx_Core1_Ref")), 0, 1, Tname:="F_Gfx_Core1_Ref"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_NO_IS_PR")), 0, 1, Tname:="F_SOC_NO_IS_PR"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_NO_ID_ED")), 0, 1, Tname:="F_SOC_NO_ID_ED"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_NO_IS_ID_ED")), 0, 1, Tname:="F_SOC_NO_IS_ID_ED"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_ANE_NoRepair")), 0, 1, Tname:="F_SOC_ANE_NoRepair"
    Next site
...
Public Function SetHarvestFlagForBV_Tahiti()
    On Error GoTo errHandler
    Dim funcName As String:: funcName = "SetHarvestFlagForBV_Tahiti"
    ...
    For Each site In TheExec.sites.Selected
        ...
        ''' initialize the flag
        TheExec.sites.item(site).FlagState("F_SOC_NO_AV") = logicFalse
        TheExec.sites.item(site).FlagState("F_SOC_NO_IS_PR") = logicFalse
        TheExec.sites.item(site).FlagState("F_SOC_NO_ID_ED") = logicFalse
        TheExec.sites.item(site).FlagState("F_SOC_NO_IS_ID_ED_PRO") = logicFalse
        TheExec.sites.item(site).FlagState("F_Bin1_HARVEST_BV") = logicFalse
        ''' reset for gfx harvesting
        TheExec.sites.item(site).FlagState(strGlb_Flag_HarvestBinningFlag_SingleFSTP) = logicFalse
        ...
        ''' check AV0 and AV1 for MA mode
        If Not TheExec.sites.item(site).FlagState("F_SOC_AV0") = logicTrue And Not TheExec.sites.item(site).FlagState("F_SOC_AV1") = logicTrue Then
            TheExec.sites.item(site).FlagState("F_SOC_NO_AV") = logicTrue
        End If
        
        ''' check ID and ED for MI mode
        If Not TheExec.sites.item(site).FlagState("F_SOC_ID") = logicTrue And Not TheExec.sites.item(site).FlagState("F_SOC_ED") = logicTrue Then
            TheExec.sites.item(site).FlagState("F_SOC_NO_ID_ED") = logicTrue
            ''' check IS, ID, ED and PRO for MS SRT post bincut
            If Not TheExec.sites.item(site).FlagState("F_SOC_IS") = logicTrue And Not TheExec.sites.item(site).FlagState("F_SOC_PRO") = logicTrue Then
                TheExec.sites.item(site).FlagState("F_SOC_NO_IS_ID_ED_PRO") = logicTrue
            End If
        End If
        
        ''' check IS and ID for NRT post bincut
        If Not TheExec.sites.item(site).FlagState("F_SOC_IS") = logicTrue And Not TheExec.sites.item(site).FlagState("F_SOC_ID") = logicTrue Then
            TheExec.sites.item(site).FlagState("F_SOC_NO_IS_ID") = logicTrue
        End If
...
'        TheExec.Flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN3")), 0, 1, Tname:="BIN3"
        ...
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN6")), 0, 1, Tname:="BIN6"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN7")), 0, 1, Tname:="BIN7"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN8")), 0, 1, Tname:="BINX"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN9")), 0, 1, Tname:="BIN9"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN10")), 0, 1, Tname:="BIN10"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN11")), 0, 1, Tname:="BIN11"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN12")), 0, 1, Tname:="BIN12"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN13")), 0, 1, Tname:="BIN13"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("BIN14")), 0, 1, Tname:="BIN14"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_IS")), 0, 1, Tname:="F_SOC_IS"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_ID")), 0, 1, Tname:="F_SOC_ID"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_ED")), 0, 1, Tname:="F_SOC_ED"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_PRO")), 0, 1, Tname:="F_SOC_PRO"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_AV0")), 0, 1, Tname:="F_SOC_AV0"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_AV1")), 0, 1, Tname:="F_SOC_AV1"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_NO_AV")), 0, 1, Tname:="F_SOC_NO_AV"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_NO_IS_PR")), 0, 1, Tname:="F_SOC_NO_IS_PR"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_NO_ID_ED")), 0, 1, Tname:="F_SOC_NO_ID_ED"
        TheExec.flow.TestLimit CInt(TheExec.sites.item(site).FlagState("F_SOC_NO_IS_ID_ED_PRO")), 0, 1, Tname:="F_SOC_NO_IS_ID_ED_PRO"
    Next site
...
Public Function SetHarvestFlagForBV_Thera()
    On Error GoTo errHandler
    Dim funcName As String:: funcName = "SetHarvestFlagForBV_Thera"
    ...
    For Each site In TheExec.sites.Selected
        ...
        TheExec.sites.item(site).FlagState("F_SOC_NO_ID_ED") = logicFalse
        TheExec.sites.item(site).FlagState("F_Bin1_HARVEST_BV") = logicFalse
        TheExec.sites.item(site).FlagState("F_SOC_NO_PR_ED_IS") = logicFalse
        ''' reset for gfx harvesting
        TheExec.sites.item(site).FlagState(strGlb_Flag_HarvestBinningFlag_SingleFSTP) = logicFalse
        ...
        If (LCase(TheExec.CurrentJob) = "cp1" Or LCase(TheExec.CurrentJob) = "cp2") And TheExec.sites.item(site).FlagState("BIN1") = logicTrue Then
            TheExec.sites.item(site).FlagState("F_Bin1_HARVEST_BV") = logicTrue
        End If
        ''' check PR/ED/IS for MS mode
        If Not TheExec.sites.item(site).FlagState("F_SOC_PR") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_ED") = logicTrue _
            And Not TheExec.sites.item(site).FlagState("F_SOC_IS") = logicTrue Then
            TheExec.sites.item(site).FlagState("F_SOC_NO_PR_ED_IS") = logicTrue
        End If
...
Public Function JudgeBINXForHarvesting()
    Dim site As Variant
    
    For Each site In TheExec.sites
        If TheExec.sites.item(site).FlagState("BIN5") = logicTrue Or _
            TheExec.sites.item(site).FlagState("BIN6") = logicTrue Or _
            TheExec.sites.item(site).FlagState("BIN7") = logicTrue Or _
            TheExec.sites.item(site).FlagState("BIN9") = logicTrue Or _
            TheExec.sites.item(site).FlagState("BIN10") = logicTrue Or _
            TheExec.sites.item(site).FlagState("BIN11") = logicTrue Or _
            TheExec.sites.item(site).FlagState("BIN12") = logicTrue Or _
            TheExec.sites.item(site).FlagState("BIN13") = logicTrue Or _
            TheExec.sites.item(site).FlagState("BIN14") = logicTrue Then
            If TheExec.sites.item(site).FlagState("F_PassBinCut_2") = logicTrue Then
                TheExec.sites.item(site).FlagState("F_BINX_Harvesting") = logicTrue
                TheExec.sites.item(site).FlagState("F_BINY_Harvesting") = logicFalse
            ElseIf TheExec.sites.item(site).FlagState("F_PassBinCut_3") = logicTrue Then
                TheExec.sites.item(site).FlagState("F_BINX_Harvesting") = logicFalse
                TheExec.sites.item(site).FlagState("F_BINY_Harvesting") = logicTrue
            Else
                TheExec.sites.item(site).FlagState("F_BINX_Harvesting") = logicFalse
                TheExec.sites.item(site).FlagState("F_BINY_Harvesting") = logicFalse
            End If
...
Public Function JudgeBINXForWLFT()
    Dim site As Variant
    ...
    For Each site In TheExec.sites
        ...
        If Flag_Fuse_BinX(site) = True Then
            If fuse_bincutnum = 0 And Not LCase(TheExec.CurrentJob) = "cp1" Then
                TheExec.sites.item(site).FlagState("F_Bin1_To_BinX_InWLFT1") = logicTrue
                TheExec.sites.item(site).FlagState(strGlb_Flag_Vddbinning_Fail_Stop) = logicTrue
            End If
        End If
    Next site
