...
Public glb_ShmooEnable As Boolean
' ============
' Private Data
' ============
...
Public do_shmoo_hole_flag As Boolean '20231018
...
Public Function Functional_T_char(StartOfBodyF As InterposeName, _
                             ...
                             Optional BypassShmooHole As Boolean = False, Optional Harvest_Header As String, Optional RetentionMeasVIF As String = vbNullString, Optional MeasureAutoRange As Boolean = False, Optional Harv_FailFlag As String, Optional SSN_SpecifyFlag As String, Optional SSN_EnableCoreMask As Boolean = True) As Long
...
'==============Add by Leslie 200915 for bypass shmoo hole to find the LVCC============'
    ...
    If Step_ = subAllBody Or Step_ = subPrebody Then
        ...
        Else
            If InStr(1, INIT_Patset, ",") > 0 Then
                PatternAry_1 = Split(INIT_Patset, ",")
                InitialPatCnt = UBound(PatternAry_1) + 1
            Else
                PatternAry_1 = TheExec.DataManager.Raw.GetPatternsInSet(INIT_Patset, InitialPatCnt)
            End If
            If InStr(1, PL_Patset, ",") > 0 Then
                PatternAry_2 = Split(PL_Patset, ",")
                PayloadPatCnt = UBound(PatternAry_2) + 1
            Else
                PatternAry_2 = TheExec.DataManager.Raw.GetPatternsInSet(PL_Patset, PayloadPatCnt)
            End If
    ...
    If Step_ = subAllBody Or Step_ = subBody Then
        ...
        If g_FirstSetp = True Then
            ...
            g_FirstSetp = False
        End If
        Call Body(FloatPins, StrToDbl(PatternTimeout), g_CharPattInfoAry(InitialPatCnt).Pattern, ReportResult, ResultMode)
                ...
                If g_CharPattInfoAry(ShmooTestIdx).Pattern.value <> "" Then
                    ...
                    If .IsInitPattern = True And g_INIT_PAT_DONE = True Then GoTo skip_pat_run
                        ...
                        If .Sequence = 1 And TheExec.DevChar.Setups.IsRunning = True Then
                            ...
                            End If
                            If ALL_Power_DCVI_pins <> "" Then
                                TheExec.DataManager.DecomposePinList ALL_Power_DCVI_pins, ShmooPinAry, shmooPinCnt
                                For Each tmpShmooPin In ShmooPinAry
                                    TheHdw.DCVI.pins(tmpShmooPin).Voltage.value = g_ContextVmainValue.pins(tmpShmooPin).value
                                Next tmpShmooPin
                            End If
                        End If
...
errHandler:
    ...
    If AbortTest Then Exit Function Else Resume Next
End Function
Public Function ApplySafeVoltage_W_WaitTime(WaitTime As Double, Optional isApplyPower As Boolean = False)
    'WaitTime = 2
    
''add current measurement before cooling down
'Dim Power_data1 As New PinListData
'Power_data1 = TheHdw.DCVS.Pins("CorePower1").Meter.Read(tlStrobe, 1, , tlDCVSMeterReadingFormatAverage)
'TheExec.Flow.TestLimit resultVal:=Power_data1, scaletype:=scaleNone, ForceUnit:=unitVolt, ForceResults:=tlForceNone
...
''add current measurement after cooling down
'Dim Power_data2 As New PinListData
'Power_data2 = TheHdw.DCVS.Pins("CorePower1").Meter.Read(tlStrobe, 1, , tlDCVSMeterReadingFormatAverage)
'TheExec.Flow.TestLimit resultVal:=Power_data2, scaletype:=scaleNone, ForceUnit:=unitVolt, ForceResults:=tlForceNone
...
Public Function Adjust_IndexbyFlag(GLBIndex As String, flagName As String)
...
Dim waitT As Long
    ...
    For Each site In TheExec.sites.Selected
        ...
        End If
    Next site
''    TheExec.Datalog.WriteComment "  MTRTSNS excute  " + CStr(waitT) + "  Times"
    
    If EndFlow = True Then
        waitT = TheExec.flow.var(GLBIndex).value
        If waitT > 1 Then
            TheExec.Datalog.WriteComment "*************************************************"
            TheExec.Datalog.WriteComment "  MTRTSNS excute  " + CStr((waitT - 1) * 0.1) + "  (s) for cooling !!"
            TheExec.Datalog.WriteComment "*************************************************"
            
        End If
        TheExec.flow.var(GLBIndex).value = 100
    End If
End Function
...
On Error GoTo errHandler
    ...
    If TheExec.TesterMode = testModeOffline Then
        Pat_Path = "O:\TP-to-C651\Staten\SVN_user\Sync_K_Disk_Pattern"
    Else
        Pat_Path = "U:\TP-to-C651\Staten\SVN_user\Sync_K_Disk_Pattern"
    End If
    ...
    For index_CharPat = 0 To PatSets_in_char.Count - 1
        If dic_Patsets_All.Exists(PatSets_in_char.Keys(index_CharPat)) = True Then
            ...
            If Split(dic_Patsets_All(PatSets_in_char.Keys(index_CharPat)), ":")(1) <> Pat_Latest Then
                ...
                TheExec.AddOutput PatSets_in_char.Keys(index_CharPat) & " has the newer version " & Pat_Latest & ", please check the PatSets_all sheet."
            End If
        Else
            n = n + 1
            ReDim Preserve List_Error_Inst(n) As Error_Inst
...
'            End If ''End  If ErrorPatsetsCheck Exists
    ...
    If UBound(List_Error_Inst) = 0 Then
        If Flag_old_version = False Then
            TheExec.AddOutput "All Patterns in Functional_T_char have defined to the latest version!"
            TheExec.AddOutput "Feature_Check_in_Functional_T_char success !"
        Else
            TheExec.AddOutput "Feature_Check_in_Functional_T_char have issue !"
        End If
    Else
        For n = 1 To UBound(List_Error_Inst)
            With List_Error_Inst(n).Error_status
                If .Pmode_Naming_error = True Then
                    TheExec.AddOutput "Error Pmode " & List_Error_Inst(n).ErrorPmode & " in " & List_Error_Inst(n).instance_name & ", please check it out."
                ElseIf .Ret_Scenario_error = True Then
                    TheExec.AddOutput "In Retension test " & List_Error_Inst(n).instance_name & ", the Power_Run_Scenario should be 'INIT_NV_PL_NV'. Please check if it is for the special case or not."
                ElseIf .Ret_WaitTime_error = True Then
                    TheExec.AddOutput "No wait time in Retension test " & List_Error_Inst(n).instance_name & ", please check the wait time setting."
                 ElseIf .Patsets_error = True Then
                    TheExec.AddOutput "Unknown pattern " & List_Error_Inst(n).ErrorPatsets & ", please check the pattern exists ot not."
                End If
            End With
        Next n
        TheExec.AddOutput "Feature_Check_in_Functional_T_char have issue !"
...
Exit Function
errHandler:
    TheExec.ErrorLogMessage "Error encountered in VBT function of Feature_Check_in_Functional_T_char."
End Function
