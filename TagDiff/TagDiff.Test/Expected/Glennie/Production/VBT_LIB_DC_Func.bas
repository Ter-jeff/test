    ...
    For Each pin In ForceByPin
        ...
        If b_ForceDiffVolt = False Then
            If ForceValByPin(0) > 0.8 Then
                If LCase(pin) Like "*1p2*" Or LCase(pin) Like "*gpio0*" Then
...
''    ForceVal_Index = 0
    ...
    ElseIf Flag_SingleLimit = True Then
        ...
        Else
            For p = 0 To MeasureValue.pins.Count - 1
                TheExec.flow.TestLimitIndex = Temp_index  'restore 20230111
                If gl_UseStandardTestName_Flag = True Then                       'Roger add
                    TestNameInput = Report_TName_From_Instance("I", MeasureValue.pins(p), OutputTname, TestSeqNum, p)
                End If
                If p = 0 Then
...
Optional CharInputString As String, Optional ForceFunctional_Flag As Boolean = False, Optional MeasIGrpPinCnt_Str As String = "", Optional Meas_StoreName As String, Optional Calc_Eqn As String, _
Optional KeepEmptyLimit As Boolean = False, Optional CFG_GPIO_Pins As String, Optional MeasVGrpPinCnt As Integer = 0, Optional VDD_IO_1p2 As String, Optional VDD_IO_1p8 As String, Optional Validating_ As Boolean) As Long
On Error GoTo errHandler 'Add ErrHandler 2023/08/18
...
On Error GoTo errHandler
    ...
    TheHdw.Digital.ApplyLevelsTiming True, True, True, tlPowered 'Apply max ifold, ZYCHOUA 220120
    ...
    Else
        ...
        Else
            ...
            If Looping_Contorl = True Then
                ...
                tempstep = TheExec.flow.var(Looping_Index_Name).value
                For Each site In TheExec.sites
                    pin_count = 0
                    Print_Looping_Inf = vbNullString
                    ...
                    For Each pin In power_pin_ary
                        ...
                        End If
                        pins = UCase(EVS_Detail_value(pin_count).PinName)
                    ...
                    Next pin
                    TheExec.Datalog.WriteComment "EVS results: SITE" & site & ";" & "X,Y: " & Die_X_location & "," & Die_Y_location & ";" & Print_Looping_Inf & EVS_Type & ";" & "Step" & tempstep   '& Chr(10)
                Next site
...
On Error GoTo errHandler
    ...
    If UCase(Direction) = "UP" Then
                ''default = 20
        If TheExec.enableWord("EVS_IVCurve") = False Then
...
''
        ...
        Dim RampUp_measure_Value As New PinListData
    '///////////Print out final value of each pin (Power up and down)///////////
        ...
        For Each pin In power_pin_ary
            'If EVS_Detail_value(pin_count).Pin_rise Then
                Power_Name = m_InstanceName & "_" & "RampUp_MeasI"
                TheHdw.DCVS.pins(PinName).Meter.mode = tlDCVSMeterCurrent
                RampUp_measure_Value = TheHdw.DCVS.pins(pin).Meter.Read(tlStrobe, 10, , tlDCVSMeterReadingFormatAverage)
                TheExec.flow.TestLimit resultVal:=RampUp_measure_Value, scaletype:=scaleNone, unit:=unitCustom, formatStr:="%.3f", Tname:=Power_Name, customUnit:="A"     'BurstResult=1:Pass
                Set RampUp_measure_Value = Nothing
            'End If
            pin_count = pin_count + 1
        Next pin
        ...
        For Each site In TheExec.sites
            ...
            For Each All_Power In All_Core_Power
                ...
                If LCase(gl_GetInstrumentType_Dic(sPinName)) Like "*dcvs*" Then
                    ...
                    End If
                ElseIf LCase(gl_GetInstrumentType_Dic(sPinName)) Like "*dcvi*" Then
                    If All_Power Like "*DCVI*" Then    '''tahiti_230606_ZWQIN, Fixed the DCVI pins
                        All_Power = Replace(All_Power, "_DCVI", "")
                    End If
                    Gatecheck = TheHdw.DCVI.pins(All_Power).Gate
...
'    Call Test_time_breakdown_Start(TExec_Before_Pat, Test_time_breakdown, EVS_Sequence)
    ...
    If Open_LatchUp_Measure = False Then 'Or Looping_Contorl = False Then
        'TheHdw.Alarms.StartMonitoringAlarms
    End If
'    Dim analysis1 As Boolean
'    analysis1 = True
'
'    Dim analysis2 As Boolean
'    analysis2 = True
'    Dim power_pin_ary1() As String
'    Dim power_pin_ary2() As String
'    power_pin_ary1 = Split(PowerPin1, ",")
'    power_pin_ary2 = Split(PowerPin2, ",")
...
Public Function EVS_Cooling(Time As Double) As Long
'''    TheHdw.Digital.ApplyLevelsTiming True, True, True, tlPowered
    TheHdw.Wait Time
    TheExec.Datalog.WriteComment "---------------------------------------------------------------------"
    TheExec.Datalog.WriteComment "-------------------------EVS Wait Time : " + CStr(Time) + " S-------------------------"
    TheExec.Datalog.WriteComment "---------------------------------------------------------------------"
End Function

Public Function EVS_TestFunction(sLoop As Long)
Dim TempBinNumber As Long
Dim TempSortNumber As Long
Dim site As Variant
Dim TestName As String
Dim SortNumResult As New SiteLong
Dim BinNumResult As New SiteLong
Dim TestNum As Long
Dim mLoopCount As Long
Dim mMaxLoopCount As Long

Dim tmplot As String, tmpwfID As String, tmpwfInfo As String, tmpX As Long, tmpY As Long
tmplot = TheExec.Datalog.Setup.LotSetup.lotId
tmpwfID = TheExec.Datalog.Setup.WaferSetup.ID

TestName = TheExec.DataManager.instancename

Dim ws As Worksheet
Set ws = Sheets("storebinnumber")

For Each site In TheExec.sites.Starting
    If TheExec.sites(site).SortNumber = -1 Then
        TheExec.sites.item(site).SortNumber = ws.Cells(site + 1, 1)
        TheExec.sites.item(site).BinNumber = ws.Cells(site + 1, 2)
        If sLoop <> 1 Then
            TheExec.sites(site).SiteVariableValue("SortNumber_sv") = ws.Cells(site + 1, 1)
            TheExec.sites(site).SiteVariableValue("BinNumber_sv") = ws.Cells(site + 1, 2)
        End If
    Else
        ws.Cells(site + 1, 1) = TheExec.sites(site).FirstSortNumber
        ws.Cells(site + 1, 2) = TheExec.sites(site).FirstBinNumber
    End If
Next site

For Each site In TheExec.sites.Starting
    If TheExec.sites(site).FirstSortNumber = -1 Then
        SortNumResult = ws.Cells(site + 1, 1)
        BinNumResult = ws.Cells(site + 1, 2)
        TempSortNumber = ws.Cells(site + 1, 1)
        TempBinNumber = ws.Cells(site + 1, 2)
        
        TheExec.Datalog.WriteComment "Bin Result-----> Site" & site & "     Sort_Loop:  " & TempSortNumber & "     Bin_Loop:  " & TempBinNumber
        TestNum = TheExec.sites(site).TestNumber
    Else
        SortNumResult = TheExec.sites.item(site).FirstSortNumber
        BinNumResult = TheExec.sites.item(site).FirstBinNumber
        TempSortNumber = TheExec.sites.item(site).FirstSortNumber
        TempBinNumber = TheExec.sites.item(site).FirstBinNumber
...
mLoopCount = TheExec.flow.var("Loop_INDEX").value
mMaxLoopCount = TheExec.flow.var("MaxLoopCount").value
TestNum = mLoopCount + TestNum
TheExec.flow.TestLimit resultVal:=SortNumResult, Tname:="Sort_Loop" & mLoopCount, tNum:=TestNum, scaletype:=scaleNoScaling
TheExec.flow.TestLimit resultVal:=BinNumResult, Tname:="Bin_Loop" & mLoopCount, tNum:=TestNum + 10

For Each site In TheExec.sites.Starting

    tmpX = TheExec.Datalog.Setup.WaferSetup.GetXCoord(site)
    tmpY = TheExec.Datalog.Setup.WaferSetup.GetYCoord(site)
    tmpwfInfo = tmplot & "-" & tmpwfID & "-X" & tmpX & "Y" & tmpY
    ...
    If TheExec.sites(site).FirstSortNumber = -1 Then
        ...
        TheExec.Datalog.WriteComment "Bin Result_" & sLoop & "  ----->  " & tmpwfInfo & "  Site" & site & "     Sort_Loop:  " & TempSortNumber & "     Bin_Loop:  " & TempBinNumber
        TestNum = TheExec.sites(site).TestNumber
    Else
        SortNumResult = TheExec.sites.item(site).FirstSortNumber
        BinNumResult = TheExec.sites.item(site).FirstBinNumber
...
Public Function EVS_setbinnum()

    Dim tmpstr0 As String
    Dim tmpstr1 As String
    Dim tmpint0() As String
    Dim tmpint1() As String
    Dim i As Integer
    Dim j As Integer
    ...
    For Each site In TheExec.sites.Starting
        tmpstr0 = tmpstr0 & site & ","
    Next
    
    For Each site In TheExec.sites.Active
        tmpstr1 = tmpstr1 & site & ","
    Next
    
    tmpstr0 = left(tmpstr0, Len(tmpstr0) - 1)
    tmpstr1 = left(tmpstr1, Len(tmpstr1) - 1)
    tmpint0 = Split(tmpstr0, ",")
    tmpint1 = Split(tmpstr1, ",")
    For i = 0 To UBound(tmpint0)
        For j = 0 To UBound(tmpint1)
            If tmpint0(i) = tmpint1(j) Then
                j = UBound(tmpint1)
            ElseIf j = UBound(tmpint1) Then
                TheExec.sites.item(tmpint0(i)).SortNumber = ws.Cells(tmpint0(i) + 1, 1)
                TheExec.sites.item(tmpint0(i)).BinNumber = ws.Cells(tmpint0(i) + 1, 2)
                TheExec.sites(tmpint0(i)).SiteVariableValue("SortNumber_sv") = ws.Cells(tmpint0(i) + 1, 1)
                TheExec.sites(tmpint0(i)).SiteVariableValue("BinNumber_sv") = ws.Cells(tmpint0(i) + 1, 2)
            End If
        Next
    Next
    
    If TheExec.flow.var("Loop_INDEX").value = 2 Then
        For Each site In TheExec.sites.Starting
            ws.Cells(site + 1, 1) = ""
            ws.Cells(site + 1, 2) = ""
        Next
