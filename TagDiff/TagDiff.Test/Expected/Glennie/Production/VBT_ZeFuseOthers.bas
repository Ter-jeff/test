        ...
        For Row = startRow To SheetMaxRow
            If ParsingArr(Row, bankNameCol) <> "" Then
                ...
                With BinCheckData(flagIdx).CateArr(cateIdx)
                    ...
                    .skipCheck = IIf(stageColIdx > 0, CheckCurrentJob(UCase(Trim(ParsingArr(Row, stageColIdx)))), False)
                End With
...
Public Function EvsFuse_WriteToFuse(bankName As String, fieldName As String, evsAlarmflag As String, evsConductflag As String, evsIDSHighflag As String)
On Error GoTo errHandler
    Dim funcName As String: funcName = "EvsFuse_WriteToFuse"
    Dim fieldArray As Variant
    Dim opbank As eFuseBdfBank
    Dim field As eFuseBdfField
    Dim FuseVal As New SiteVariant
    FuseVal = 0
    Set opbank = GetBdfBank(bankName)
    
    'init blank check count
    opbank.BlankCheckCount = 0
    opbank.IsSameCheckVar = 1

    Set field = opbank.Fields(fieldName)
    For Each site In TheExec.sites
        If TheExec.sites.item(site).FlagState(evsAlarmflag) = logicTrue And TheExec.sites.item(site).FlagState(evsConductflag) = logicTrue Then
            FuseVal = 3
        ElseIf TheExec.sites.item(site).FlagState(evsAlarmflag) = logicTrue And TheExec.sites.item(site).FlagState(evsConductflag) = logicTrue Then
            FuseVal = 2
        ElseIf TheExec.sites.item(site).FlagState(evsAlarmflag) = logicFalse And TheExec.sites.item(site).FlagState(evsConductflag) = logicTrue Then
            FuseVal = 1
        ElseIf TheExec.sites.item(site).FlagState(evsConductflag) = logicFalse Then
            FuseVal = 0
        End If
        TheExec.Datalog.WriteComment vbTab & "Site(" & site & ")  " & bankName & "Fuse SetWriteDecimal/Variable_SiteAware    " & field.name & " = " & FuseVal
    Next site
...
Public Function EvsFuse_TestedWriteToFuse(bankName As String, fieldName As String)
On Error GoTo errHandler
    Dim funcName As String: funcName = "EvsFuse_TestedWriteToFuse"
    Dim opbank As eFuseBdfBank
    Dim field As eFuseBdfField
    ...
    opbank.SetEfuse field.name, field.DsscDecValue, , , , , True
Exit Function
errHandler:
    GlbUtility.WriteDlg "!!!Error!!! " + funcName + ":: please check it out."
    If AbortTest Then Exit Function Else Resume Next
...
' C2L1 -> DRAM_Hynix6GB
' C2L2 -> DRAM_Hynix6GB
' C4L1 -> DRAM_Samsung6GB
' C7XX -> DRAM_Micron8GB
' C3L1 -> DRAM_Hynix8GB
' C3L2 -> DRAM_Hynix8GB
' C5L1 -> DRAM_Samsung8GB

On Error GoTo errHandler

' INITIALIZATION
Dim LotInfo_FilePath As String: LotInfo_FilePath = "C:\Flex\Applications\lotinfo.txt"
Dim DramMap_FilePath As String: DramMap_FilePath = "X:\Production\TMUA28\DRAM_Mapping\"
...
Dim ProgramName As String: ProgramName = TheExec.TestProgram.name       ''new for project 210820
Dim Project_PartName As String: Project_PartName = vbNullString       ''new for project 210820
''''''''''''''''''''''''For offline Dram''''''''''''''''''''''''
        ...
        If (TheExec.flow.enableWord("T0TX_HOT") = False) And (TheExec.flow.enableWord("T0TX_ROOM") = False) Then
            ...
            Else
                ...
                Do While Not txtStream.AtEndOfStream
                    ...
                    If lineNum Mod 2 = 1 Then
                        ' BINOUT: LOTID MISMATCHES
                        'theexec.Datalog.WriteComment "LotID:" & lotId & " (OI)"
                        If (LCase(LineTxt) Like "*lotid*") And UCase(lotId) = Split(LineTxt, ":")(1) Then
...
'                        lineNum = lineNum + 1
                    ...
                    ElseIf lineNum Mod 2 = 0 And found Then
...
'                        lineNum = lineNum + 1
                ...
                ''' can not find the mapping lot id
                If Not found Then
                    TheExec.flow.TestLimit resultVal:=0, lowVal:=1, hiVal:=1, Tname:="LotID Mismatch"
                    Exit Function
                End If
...
Dim item As Variant
    ...
    For Each item In Split(job, ",")
        If Trim(item) = GlbUtility.currStage Then
