...
Public g_DigSrc_LngAry() As Long
Public g_DSSC_Pattern As String
Public g_DigSrc_Size As Double
...
'    Dim xio_spec As String
    ...
    '++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++,NH,0.5,1.260]
            ...
            If (LCase(TheExec.CurrentJob) Like "*cp*") Then
                OutputString = OutputString & "[" & Shmoo_header & "," & lotId & "-" & WaferID & "," & CStr(XCoord(site)) & "," & CStr(YCoord(site))
            Else
        ...
        If FRC_Name = "" Then ' Default use XI0, if no input of FRC info
            FRC_Name = "XI0"
            FRC_Value = TheExec.Specs.AC("XI0_Diff_Freq_VAR").CurrentValue
            All_FRC_Status = FRC_Name & "=" & FRC_Value
        End If
        ...
        End If
            ...
            Else
                For j = 0 To Pin_Cnt - 1
                    PinName = Pin_Ary(j)
                    If TheExec.DataManager.ChannelType(PinName) Like "*DCVS*" Then
                        If j = 0 Then
                            OutputString = OutputString & PinName & "=" & Format(TheHdw.DCVS.pins(PinName).Voltage.Main.value, "0.000")
                        Else
                            OutputString = OutputString & "," & PinName & "=" & Format(TheHdw.DCVS.pins(PinName).Voltage.Main.value, "0.000")
                        End If
                    ElseIf TheExec.DataManager.ChannelType(PinName) Like "*DCVI*" Then
                        If j = 0 Then
                            OutputString = OutputString & PinName & "=" & Format(TheHdw.DCVI.pins(PinName).Voltage.value, "0.000")
                    Else
                             OutputString = OutputString & "," & PinName & "=" & Format(TheHdw.DCVI.pins(PinName).Voltage.value, "0.000")
                        End If
                    End If
                ...
                Else
                    ...
                    For j = 0 To Pin_Cnt - 1
                        ...
                        If TheExec.DataManager.ChannelType(PinName) <> "N/C" Then
                            ...
                            If TheExec.DataManager.ChannelType(PinName) Like "*DCVS*" Then
                                OutputString = OutputString & "," & PinName & "=" & Format(TheHdw.DCVS.pins(PinName).Voltage.Main.value, "0.000")
                            ElseIf TheExec.DataManager.ChannelType(PinName) Like "*DCVI*" Then ' 211219 Fixed for DCVI issue
                                OutputString = OutputString & "," & PinName & "=" & Format(TheHdw.DCVI.pins(PinName).Voltage.value, "0.000")
                            End If
                            End If
                        End If
    ...
    Else '20190321 update: Suspend Datalog =False, add g_TestNum
        ...
        Else
            ...
            End If
        End If
    ''' For AMPLP5 2D Shmoo AI H/LVCC value 220624
    If UCase(instancename) Like "*AMP*" Then
        Dim TestNameSeg() As String
        Dim HVCC_DFTLH As String
        Dim LVCC_DFTLH As String
        Dim Outer_Value As Double
        Dim Outer_Value_S As String
        TestNameSeg = Split(instancename, "_")
        For i = 0 To Outer_RangeSteps - 1
            Outer_Value = Outer_RangeFrom + i * Outer_RangeStepSize
            If Outer_Value < 0.000001 And Outer_Value > -0.000001 Then
                Outer_Value_S = "0P0"
            Else
                Outer_Value_S = Replace(CStr(Outer_Value), ".", "P")
            End If
            Outer_Value_S = Outer_StepName & Outer_Value_S
            HVCC_DFTLH = "HFH_MeasV_" & TestNameSeg(1) & "_" & TestNameSeg(0) & "_" & Outer_Value_S & "_X_Vmax"
            LVCC_DFTLH = "HFL_MeasV_" & TestNameSeg(1) & "_" & TestNameSeg(0) & "_" & Outer_Value_S & "_X_Vmin"
            TheExec.flow.TestLimit resultVal:=Shmoo_Vcc_Max(i), lowVal:=CHAR_LSL_HVCC, hiVal:=CHAR_USL_HVCC, ForceResults:=tlForceNone, Tname:=HVCC_DFTLH, PinName:=shmoo_pin_string   ' & " <> " & HVCC_DFTLH

            TheExec.flow.TestLimit resultVal:=Shmoo_Vcc_Min(i), lowVal:=CHAR_LSL_LVCC, hiVal:=CHAR_USL_LVCC, ForceResults:=tlForceNone, Tname:=LVCC_DFTLH, PinName:=shmoo_pin_string ' & " <> " & LVCC_DFTLH
        Next i
    End If
 End If
    Exit Function
...
'20161229 Roy Modified,Prevent Step_NV out of range
            ...
            If Instance_Data.Interpose_PrePat Like "*char_retest" Then
             If Vcc_min = -5555 Or Vcc_min = -9999 Or Vcc_min = -7777 Then
                 TheExec.Datalog.WriteComment "Shmoo Result not expected!!!"
                 sd_retest_char_ForceCondiction(site) = -9999.999
             Else
               
                sd_retest_char_ForceCondiction(site) = Vcc_min
             End If
            End If
        End With
        If InStr(TheExec.DataManager.instancename, "_NV") Then TestVoltage = "NV"   '#16_ELSE_CASE_CHK
...
'++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++,NH,0.5,1.260]
        ...
        Else
            For j = 0 To Pin_Cnt - 1
                PinName = Pin_Ary(j)
                pinType = LCase(TheExec.DataManager.ChannelType(PinName))
                If pinType <> "N/C" Then
                    If pinType Like "dcvi*" Then
                    If j = 0 Then
                            OutputString = OutputString & PinName & "=" & Format(TheHdw.DCVI.pins(PinName).Voltage, "0.000")
                        Else
                            OutputString = OutputString & "," & PinName & "=" & Format(TheHdw.DCVI.pins(PinName).Voltage, "0.000")
                        End If
                    ElseIf pinType Like "dcvs*" Then
                        If j = 0 Then
                        OutputString = OutputString & PinName & "=" & Format(TheHdw.DCVS.pins(PinName).Voltage.Main.value, "0.000")
                    Else
                        OutputString = OutputString & "," & PinName & "=" & Format(TheHdw.DCVS.pins(PinName).Voltage.Main.value, "0.000")
...
'--------------------------------------------------------------------------------------------
        ...
        If UCase(instancename) Like "HFL*" Then '#16_ELSE_CASE_CHK
            ...
            If UCase(TheExec.DataManager.instancename) Like "HFL_X_VDDFIXEDLPDPRX_VMX_LPDPRX_X_EVENRX2F1_X_X_X_CZ_NV" Then
                CHAR_USL_LVCC = 0.71
             For Each site In TheExec.sites
                If CHAR_USL_LVCC < Shmoo_Vcc_Min(site) Then
                    TheExec.sites.item(site).FlagState("F_HardIP_LPDPRX_EVENRX2F1_pp_crea0_s_fulp_an_rx00_mea_jtg_vmx_allfrv_si_even_rx2f1_N_Flag") = logicTrue
                End If
             Next site
            End If
            If UCase(TheExec.DataManager.instancename) Like "HFL_X_VDDFIXEDLPDPRX_VMX_LPDPRX_X_EVENRX2F2_X_X_X_CZ_NV" Then
                CHAR_USL_LVCC = 0.71
              For Each site In TheExec.sites
                If CHAR_USL_LVCC < Shmoo_Vcc_Min(site) Then
                    TheExec.sites.item(site).FlagState("F_HardIP_LPDPRX_EVENRX2F2_pp_crea0_s_fulp_an_rx00_mea_jtg_vmx_allfrv_si_even_rx2f2_N_Flag") = logicTrue
                End If
             Next site
            End If
...
For Each site In TheExec.sites
        ...
        End If
         ...
         If TheExec.flow.enableWord("Debug_LVCC_VminBoundary") = True Then
          ...
          If Shmoo_Vcc_Min(site) > 0 Or (TheExec.enableWord("Find_shmoo_hole_low_power_twice") = True And Shmoo_Vcc_Min(site) = -5555) Then '20231101 Shmoo Hole '20231018
             If (LCase(TheExec.CurrentJob) Like "*cp*") Then
                'If g_VminBoundary_selsrm = True Then
                    FailingDatalog_HLvcc_Boundary_SELSRM Search_String, lotId, CStr(WaferID), CStr(XCoord(site)), _
                    CStr(YCoord(site)), Shmoo_Pattern, "Shmoo LVCC", High_to_Low, site, RangeFrom, RangeTo, RangeSteps, RangeStepSize
    ...
    For i = 0 To p_cnt - 1
        If TheExec.DataManager.ChannelType(p_ary(i)) <> "N/C" Then
            ...
            Select Case InstName
               Case "DC-07"
                  For Each site In TheExec.sites
                    ShmooPower(i)(site) = TheHdw.DCVI.pins(p_ary(i)).Voltage
                  Next site
               Case "VHDVS", "HexVS", "VSM", "VS-5A", "VS-800mA"
   ...
   Create_Pin_Dic Restore_Pin_str, Restore_Pins_Dict
    ...
    For i = 0 To p_cnt - 1
        If TheExec.DataManager.ChannelType(p_ary(i)) <> "N/C" And Restore_Pins_Dict.Exists(LCase(p_ary(i))) = True Then '#16_ELSE_CASE_CHK
            ...
            Case "DC-07"
                   For Each site In TheExec.sites
                       TheHdw.DCVI.pins(p_ary(i)).Voltage = ShmooPowerStored(i)(site)
                   Next site
            Case "VHDVS", "HexVS", "VSM", "VS-5A", "VS-800mA"
...
On Error GoTo errHandler
    ...
    Dim str_len, start_point As Integer '20231018
    Dim search_dif As Boolean '20231018
    Dim report_point As Integer '20231018
    Dim z As Long '20231018
    show_vcc = "[Shmoo,"
    
    Dim Shmoo_setup_name, Shmoo_TestInst_Name As String
    
    step_p = Len(Shmoo_result_PF)
    find_shmoo_hole = False '20231018
    Shmoo_setup_name = TheExec.DevChar.Setups.ActiveSetupName
...
'Set instrument hardware
    ...
    If shmoo_pin <> "" Then ' Add to advoid shmoo_pin no value issue
        ' Leave function when shmoo_pin is N/C
        InstName = GetInstrument(shmoo_pin, 0)
        If InstName = "" Then Exit Function
    End If
...
On Error GoTo errHandler
 ...
 Else
    For i = 0 To UBound(SelsramMapping)
        If UCase(DSSCPatName) Like SelsramMapping(i).Pattern Then
            idx = i
            Exit For
        End If
    Next i
...
'            TheExec.ErrorLogMessage "DC spec " & SP & " is not ended with _VAR_C/S/G/H in " & TheExec.DataManager.instancename
    ...
    TheExec.DataManager.DecomposePinList Power_pins, p_ary, p_cnt
    For i = 0 To p_cnt - 1
        If UCase(TheExec.DataManager.ChannelType(p_ary(i))) = "N/C" Then GoTo nexti
        p_ary(i) = GetUsedPinNameForDCVI(LCase(p_ary(i)))
...
'                              TheExec.ErrorLogMessage "Instrument " & InstName & " for pin " & Shmoo_Apply_Pin_Arry(i) & " is not supported in Shmoo_Restore_Power_per_site_Vbump"
                              For Each site In TheExec.sites
                                  TheHdw.DCVI.pins(Shmoo_Apply_Pin_Arry(i)).Voltage = g_Globalpointval.pins(Shmoo_Apply_Pin_Arry(i)).value(site)
                              Next site
                         Case "VHDVS", "HexVS", "VSM", "VS-5A", "VS-800mA"
                              For Each site In TheExec.sites
...
'                             TheExec.ErrorLogMessage "Instrument " & InstName & " for pin " & p_ary(i) & " is not supported in Shmoo_Restore_Power_per_site_Vbump_Retention"
                            TheHdw.DCVI.pins(p_ary(i)).Voltage = Retention_ForceV_Arr(i)
                       Case "VHDVS", "HexVS", "VSM", "VS-5A", "VS-800mA"
...
'                  Next site
     ...
     '=========================================== Applied voltage to Valt==================================================
      If init = False Or InitAltRecover = True Then
        'TheHdw.DCVS.Pins(Join(instrumentUtility.GetDCVSPinsFromCorePower, ",")).Voltage.Output = tlDCVSVoltageMain '''Tahiti Fixed VRS alarm, 230615
        TheHdw.DCVS.pins(ALL_Power_DCVS_pins).Voltage.Output = tlDCVSVoltageMain
...
'''    Call Get_Tname_FromFlowSheet(InstanceName, HIO_PinName_Updated)      '20180515 TER add
    ...
    For Each site In TheExec.sites
        ...
        If FRC_Name = "" Then ' Default use XI0, if no input of FRC info
            FRC_Name = "XI0"
            FRC_Value = TheExec.Specs.AC("XI0_Diff_Freq_VAR").CurrentValue      '"XI0_Freq_VAR"
            All_FRC_Status = FRC_Name & "=" & FRC_Value
        End If
...
On Error GoTo errHandler
    ...
    If ALL_Power_DCVS_pins <> "" Or ALL_Power_DCVI_pins <> "" Then
...
On Error GoTo errHandler
    ...
    'For Tahiti DCVI Case test 20240527
    If UCase(NameTmpArr(UBound(NameTmpArr))) = "DCVI" Or UCase(NameTmpArr(UBound(NameTmpArr))) = "DCVS" Then
        ReDim Preserve NameTmpArr(UBound(NameTmpArr) - 1)
        NameTmp = Join(NameTmpArr, "_")
    End If
...
On Error GoTo errHandler
    ...
    If UCase(InitSel) = "MAX" Then
        tmpScenarioAry(1) = "HV"
    ElseIf UCase(InitSel) = "MIN" Then
        tmpScenarioAry(1) = "LV"
    End If
    ...
    UsePowerRunDeafult = False
    If UCase(PowerRunScenario) = "INIT_NV_PL_SWEEP" Then UsePowerRunDeafult = True
                        ...
                        For i = 0 To UBound(EQarray)
                            If UCase(.SegDict(EQarray(i))(0)) = "SELSRAM" And SELSRAM_DSSC <> "" Then
                                For SelSramIdx = 0 To UBound(SelsramMapping)
                                    If UCase(PattAry(PatternCnt).value) Like SelsramMapping(SelSramIdx).Pattern And SelsramMapping(SelSramIdx).Pattern <> "*" Then
    ...
    End If
    If Not UsePowerRunDeafult = True Then
    ...
    For i = 0 To UBound(g_CharPattInfoAry)
        If g_CharPattInfoAry(i).Pattern.value <> "" Then
            If g_CharPattInfoAry(i).DigSrcType = "SELSRAM" Then
                Select Case UCase(PmodeSel) '240619
                Case "TYP"
...
Public Function Decide_Shmoo_Voltage(PerforManceMode As String, Power_pins As String, ForceCond As String, RunScenario As String _
                                                                    ...
                                                                    , InitSel As String, PMSel As String, MergeFoeceVDD As String, MergeForceCond As String)
On Error GoTo errHandler
...
'                            ForceVDD_Cnt = ForceVDD_Cnt + 1
                            If Not ForcePin_Dict.Exists(tmpPinAry(i)) Then
                                ForcePin_Dict.Add tmpPinAry(i), ForceVDD_Cnt
                                ReDim Preserve ForcePinAry(ForceVDD_Cnt)
                                ReDim Preserve FroceSpecAry(ForceVDD_Cnt)
                                ReDim Preserve ForceValueAry(ForceVDD_Cnt)
                                ...
                                ForceVDD_Cnt = ForceVDD_Cnt + 1
                            Else
                                ForceValueAry(ForcePin_Dict.item(tmpPinAry(i))) = CDbl(pininfo(2))
                            End If
                        End If
                    Next i
            ...
            ElseIf UBound(pininfo) = 1 And UCase(pininfo(0)) = "USL" Then
                CHAR_USL_HVCC = CHAR_USL_LVCC = FormatNumber(CDbl(pininfo(1)), 3)
            ElseIf UBound(pininfo) = 1 And UCase(pininfo(0)) = "LSL" Then
                CHAR_LSL_HVCC = CHAR_LSL_LVCC = FormatNumber(CDbl(pininfo(1)), 3)
            End If
        Next FC
    End If
    ...
    Case "TYP"
        'Read_Init_NV = True
        ContentNV = True
        If UCase(RunScenario) Like "*INIT#_HV*" Or UCase(RunScenario) Like "*INIT##_HV*" Then Read_Init_HV = True
        If UCase(RunScenario) Like "*INIT#_LV*" Or UCase(RunScenario) Like "*INIT##_LV*" Then Read_Init_LV = True
    Case "MAX"
        'Read_Init_HV = True
        ContentHV = True
        If (UCase(RunScenario) Like "*INIT#_NV*" Or UCase(RunScenario) Like "*INIT##_NV*") And Not UCase(RunScenario) Like "*INIT1_NV*" Then Read_Init_NV = True
        If UCase(RunScenario) Like "*INIT#_LV*" Or UCase(RunScenario) Like "*INIT##_LV*" Then Read_Init_LV = True
    Case "MIN"
        'Read_Init_LV = True
        ContentLV = True
                    ...
                    For ForceVDD_Cnt = 0 To UBound(ForcePinAry)
                        ...
                        If ForcePinAry(ForceVDD_Cnt) <> "" Then
                            ...
                            If Not Dict_PL_NV.Exists(ForcePinAry(ForceVDD_Cnt)) Then
                                Dict_PL_NV.Add ForcePinAry(ForceVDD_Cnt), CDbl(ForceValueAry(ForceVDD_Cnt))
                            End If
                        End If
                    Next ForceVDD_Cnt
                End If
                    ...
                    For ForceVDD_Cnt = 0 To UBound(ForcePinAry)
                        If ForcePinAry(ForceVDD_Cnt) <> "" Then
                            ...
                            If Not Dict_PL_HV.Exists(ForcePinAry(ForceVDD_Cnt)) Then
                                Dict_PL_HV.Add ForcePinAry(ForceVDD_Cnt), CDbl(ForceValueAry(ForceVDD_Cnt))
                            End If
                        End If
                    Next ForceVDD_Cnt
                End If
                    ...
                    For ForceVDD_Cnt = 0 To UBound(ForcePinAry)
                        If ForcePinAry(ForceVDD_Cnt) <> "" Then
                            ...
                            If Not Dict_PL_LV.Exists(ForcePinAry(ForceVDD_Cnt)) Then
                                Dict_PL_LV.Add ForcePinAry(ForceVDD_Cnt), CDbl(ForceValueAry(ForceVDD_Cnt))
                            End If
                        End If
                    Next ForceVDD_Cnt
                End If
...
'                End If
            ...
            For Each site In TheExec.sites.Active
                ...
                If UCase(SubSourceData) = "SELSRAM" Then
                    ...
                    TheExec.Datalog.WriteComment "site " & site & " DigSrc data from SelSram: " & DigEQAry(i) & " = " & Tmp_SelSram(site)
                Else
        ...
        If UCase(DigSrcType) = "SELSRAM" Then
...
''===========================================================SET RUN LEVEL=========================================================
    ...
    For CountMultiPat = 0 To MultiPatCount  'Multi pat function
     ...
     '********************************************************** SSN VBT **************************************
        ...
        Dim patcnt As Long
        Dim IGXLversion As Long
        '10.40.91_uflx --> 104091
        IGXLversion = CLng(Split(Split(TheExec.SoftwareVersion, "_")(0), ".")(0) + Split(Split(TheExec.SoftwareVersion, "_")(0), ".")(1) + Split(Split(TheExec.SoftwareVersion, "_")(0), ".")(2))
        ...
        End If
            'If IGXL version is higher than 10.40.91 then UF support SSN
            If glb_TesterType = "UltraFLEXplus" Or IGXLversion >= 104091 Then
...
On Error GoTo errHandler
    ...
    If Shmoo_Apply_Pin <> "" Then
        ...
        For i = 0 To UBound(p_ary)
            If InStr(RetentionPin, p_ary(i)) = 0 Then RetentionPin = RetentionPin & IIf(RetentionPin <> "", ",", "") & p_ary(i)
    ...
    For i = 0 To RestoreMainOrAlt.pins.Count - 1
        ...
        For Each site In TheExec.sites
            ...
            '    RestoreMainOrAlt.Pins(PinName).value = g_CharPattInfoAry(seq - 1).ForceVoltage.Pins(PinName).value
            'End If
            If InStr(g_Retention_VDD, PinName) Then TargetVal.pins(PinName).value = g_RetntionVal.pins(PinName).value
            If InStr(Shmoo_Apply_Pin, PinName) Then TargetVal.pins(PinName).value = g_Globalpointval.pins(PinName).value
...
On Error GoTo errHandler
    ...
    For i = 0 To UBound(tmpPatStrAry)
        ...
        If UBound(tmpPatStr) >= 4 And UCase(tmpPatStr(3)) Like "*PL*" Then
            If UCase(tmpPatStr(4)) = "SC" Or UCase(tmpPatStr(4)) = "CH" Or UCase(tmpPatStr(4)) = "FU" Then
                If patType = "" Then
                    patType = "SCAN"
                Else
