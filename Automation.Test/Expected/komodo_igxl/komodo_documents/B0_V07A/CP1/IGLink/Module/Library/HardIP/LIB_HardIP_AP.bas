...
''        For Each site In theexec.sites
            ...
            For i = 0 To UBound(SelsramMapping(PattIdx).logic_Pin)
                ...
                If powerDomain <> "PRESERVED" And powerDomain <> "RESERVED" Then
                    ...
                    str_sram_vth = str_sram_vth & "," & alphaPowerDomain & "=" & CStr(Format(Floor(glb_BCCallHIPinst_instinfo.sram_Vth(VddBinStr2Enum(powerDomain))) / 1000, "#0.000")) & "V"
                    str_logic_voltage = str_logic_voltage & "," & alphaPowerDomain & "=" & CStr(Format(Floor(glb_BCCallHIPinst_instinfo.voltage_SelsrmBitCalc(VddBinStr2Enum(powerDomain))) / 1000, "#0.000")) & "V"
                End If
            Next i
