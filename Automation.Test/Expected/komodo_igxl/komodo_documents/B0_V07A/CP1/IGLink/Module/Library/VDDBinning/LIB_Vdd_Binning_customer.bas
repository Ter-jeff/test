            ...
            If Row_of_Title > 0 Then
                ...
                Else
                    ...
                    If col_stage = 0 Then
                        ...
                        'TheExec.ErrorLogMessage "Column STAGE doesn't exist in header of " & sheetName & ". Error!!!"    ''20230807: Removed by sanity check    ''20230807: Removed by sanity check
                    End If
                    ...
                    If col_BitSeq = 0 Then
                        ...
                        'TheExec.ErrorLogMessage "Column BITS doesn't exist in header of " & sheetName & ". Error!!!"    ''20230807: Removed by sanity check    ''20230807: Removed by sanity check
                    End If
...
'''//==================================================================================================================================================================================//'''
    For Each site In TheExec.sites
        ...
        Else
            ...
            ''TheExec.ErrorLogMessage "site:" & site & "," & VddBinName(inst_info.p_mode) & ",powerDomain:" & ", BinCut payload voltage should be larger than 0. Error occurs in OtherRail_Pwr_Setting while " & inst_info.inst_name & ". Error!!!"    ''20230807: Removed by sanity check
        End If
    Next site
Exit Function
errHandler: 'Add ErrHandler 2023/08/01
...
'                Call Print_Error_Message (Error_Info, "LIB_Vdd_Binning_customer", "","Header of " & sheetname & " doesn't contain any  correct keyword on header row Error!!!")
'                'TheExec.ErrorLogMessage "Header of " & sheetname & " doesn't contain any  correct keyword on header row Error!!!"    ''20230807: Removed by sanity check
'            End If
'        Next Row
'
'        If isHeadFound Then
...
'                    Else
'                        Call Print_Error_Message (Error_Info, "LIB_Vdd_Binning_customer", "","Harvest Bin name: " & LCase(ws_def.Cells(Row, col_fuseBin).value) & " is duplicate. Error!!!")
'                        'TheExec.ErrorLogMessage "Harvest Bin name: " & LCase(ws_def.Cells(Row, col_fuseBin).value) & " is duplicate. Error!!!"    ''20230807: Removed by sanity check
'                    End If
'                End If
'                Row = Row + 1
'            Wend
...
'Exit Function
'errHandler:
'    Call Print_Error_Message (Error_Info, "LIB_Vdd_Binning_customer", "","Error encountered in VBT Function of Parsing_Harv_Core_Map_Table")
'    'TheExec.ErrorLogMessage "Error encountered in VBT Function of Parsing_Harv_Core_Map_Table"    ''20230807: Removed by sanity check
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    For i = 0 To UBound(strAry_powerDomain)
        ''20230728: Added for pwrbinning with offset
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    If strTemp <> "" Then
        For i = 0 To UBound(strAry_powerDomain)
            ''20230728: Added for additional format for power binning "MAX(VDD_GPU + 15 MV,VDD_SRAM_GPU-10MV)"
...
'''//==================================================================================================================================================================================//'''
    For Each site In TheExec.sites
            ...
            For i = 0 To UBound(strAry_TestCondition)
                If strAry_TestCondition(i) Like "*sram*" Then '''Calculate voltage for OtherRail, ex: "VDD_SRAM_SOC CP HVCC".
                    ...
                    Else
                        ...
                        ''TheExec.ErrorLogMessage strAry_TestCondition(i) & " from testCondition:" & str_testCondition & " for powerDomain:" & VddBinName(idxPin_CorePower) & " from sheet Non_Binning_Rail has the incorrect content. Error!!!"    ''20230807: Removed by sanity check
                    End If
                Else '''//Calculate voltage for CorePower.
                    ...
                    Else
                        ...
                        ''TheExec.ErrorLogMessage "HVCC testCondition:" & str_testCondition & " for powerDomain:" & VddBinName(idxPin_OtherRail) & " from sheet Non_Binning_Rail has the incorrect content. Error!!!"    ''20230807: Removed by sanity check
                    End If
        ...
        Else
            ...
            ''TheExec.ErrorLogMessage "site:" & site & ","  & VddBinName(inst_info.p_mode) & ",powerDomain:" & VddBinName(idxPin_CorePower) &  ", BinCut payload voltage should be larger than 0. Error occurs in Apply_testcondition_InFlowSheet while " & inst_info.inst_name & ". Error!!!"    ''20230807: Removed by sanity check
        End If
    Next site
Exit Function
errHandler: 'Add ErrHandler 2023/08/01
...
'''//==================================================================================================================================================================================//'''
    If UBound(voltage_forBinCut) = cntVddbinPin Then
        ...
        If inst_info.is_DevChar_Running = True And gb_ApplyBV = False Then
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    '''20240411: Remove unused "*h*l*a*m*" test condition
    '''20210823: Modified to revise the vbt code for testCondition such as "950mV (ms004 Product)", "MAX(MD003 E1 Product +10%, VDD_SRAM_SOC CPHV)".
    If str_testCondition Like "*(*)*" Then '''ex: "950mV (ms004 Product)", "MAX(MD003 E1 Product +10%, VDD_SRAM_SOC CPHV)"
        split_content = Split(str_testCondition, ")")
        If str_testCondition Like "*product*-*gb*" Or str_testCondition Like "*product*+*gb*" Then '''ex: "MI001 Product-CP2GB", "MCS603 Product-CP2GB", "Product-CP2GB".
        '''20210914: Modified to revise the vbt code for testCondition such as "cp_gb_hot" and "ft_gb_hot".
    ...
    ElseIf str_testCondition Like "*product*-*gb*" Then '''ex: "MI001 Product-CP2GB", "MCS603 Product-CP2GB", "Product-CP2GB".
        ...
        Else
            ...
            ''TheExec.ErrorLogMessage "Non_Binning_Rail: " & testCondition & ". It has the keyword error about calculating the extra voltage! Error!!!"    ''20230807: Removed by sanity check
        End If
    ...
    Else
        ...
        ''TheExec.ErrorLogMessage "testCondition:" & testCondition & " from sheet Non_Binning_Rail has the keyword error about calculating the extra voltage! Error!!!"    ''20230807: Removed by sanity check
    End If
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    Else
        ...
        ''TheExec.ErrorLogMessage "site:" & thissite & ",str_Input:" & str_Input & ",split_keyword:" & split_keyword & ", it has the error while Split_special_string_for_calc. Error!!!"    ''20230807: Removed by sanity check
    End If
Exit Function
errHandler: 'Add ErrHandler 2023/08/01
...
BinResult_to_E1voltage:
    ...
    If (str_testCondition Like "m*##*") Or (str_testCondition Like "m*#*" And UCase(VddBinName(idxPin_CorePower)) Like "*VDD*_SRAM*") Then 'ex: "MC601 E1 Voltage","MPS0G E1 Voltage".
        ...
        Else
            ...
            ''TheExec.ErrorLogMessage "site:" & thissite & "," &  inst_info.performance_mode & ",powerDomain:" & VddBinName(idxPin_CorePower) & ",testCondition:" & testCondition & ", it doesn't have any correct keyword about " & nonbinning_pmode & " for Calculate_Voltage_for_CorePower. Error!!!"    ''20230807: Removed by sanity check
        End If
    ...
    ElseIf str_testCondition Like "#*mv*" Then '''ex: "650mV"
        ...
        If is_Selsrm_LogicPower = True Then
            ...
            If Trim(split_content(1)) Like "*(*)*" Then '''ex: "900mV (MS003)", "950mV (MS004 Product)"
                ...
                performance_mode = UCase(Trim(split_content(0)))
                If performance_mode Like "*+*" Then
                    split_content = Split(performance_mode, "+")
                    performance_mode = UCase(Trim(split_content(0)))
                ElseIf performance_mode Like "*-*" Then
                    split_content = Split(performance_mode, "-")
                    performance_mode = UCase(Trim(split_content(0)))
                End If
...
'''//==================================================================================================================================================================================//'''
    ...
    If str_testCondition Like "m*##*" Then '''ex: "MCS601 CPVmax", "MCS608 CPHV", "MCS601 Product-CP2GB", "MCS608 QAHV".
        ...
        Else
            ...
            ''TheExec.ErrorLogMessage "site:" & thissite & "," & inst_info.performance_mode & ",powerDomain:" & VddBinName(idxPin_OtherRail) & ",testCondition:" & testCondition & ", it doesn't have any correct keyword about the performance mode of OtherRail for Calculate_Voltage_for_OtherRail. Error!!!"    ''20230807: Removed by sanity check
        End If
    Else
...
'        If inst_info.StartBin1E1_Flag = True And LCase(inst_info.inst_name) Like "*group*" Then ''20221014 Add multifstp offset
'            MGX_temp = Split(inst_info.inst_name, "_")
'            dynamicOffset_BinCut_multiFSTP = dict_offset(MGX_temp(2))
            
'        End If
'        dynamicOffset_BinCut = dynamicOffset_BinCut + dynamicOffset_BinCut_multiFSTP
        '''***********************************************************************************'''
        ...
        Else
            ...
            ''TheExec.ErrorLogMessage "site:" & site & "," &  inst_info.powerDomain & ", BinCut payload voltage should be larger than 0. Error occurs in Calculate_Binning_CorePower_with_DynamicOffset while " & inst_info.inst_name & ". Error!!!"    ''20230807: Removed by sanity check
        End If
    Next site
...
'''//==================================================================================================================================================================================//'''
    ...
    Else
        ...
        ''TheExec.ErrorLogMessage "instance: " & inst_info.inst_name & " doesn't end with any _bv or _hbv keyword. Error!!!"    ''20230807: Removed by sanity check
    End If
    ...
    If Get_BinCut_TestCondition = "" Or Get_BinCut_TestCondition = "0" Then
        ...
        ''TheExec.ErrorLogMessage "PassBinNum:" & PassBinNum & ", " & VddBinName(inst_info.p_mode) & ", selected powerDomain: " & VddBinName(idxPin_PowerDomain) & " has incorrect content in the Non_Binning_Rail. Error!!!"    ''20230807: Removed by sanity check
    End If
Exit Function
errHandler: 'Add ErrHandler 2023/08/01
...
'''//==================================================================================================================================================================================//'''
    ...
    If Flag_VDD_Binning_Offline = True Or TheExec.Flow.enableWord("Vddbinning_OpenSocket") = True Then
        If TheExec.Flow.enableWord("Vddbin_CMEM_Collection") = True Then
            ...
            ''TheExec.ErrorLogMessage "Flag_Enable_CMEM_Collection is enabled, but it doesn't support CMEM while Opensocket or Offline. Please check the flag setting. Error!!!"    ''20230807: Removed by sanity check
        End If
    End If
...
errHandler: 'Add ErrHandler 2023/08/01
    ...
    If AbortTest Then Exit Function Else Resume Next 'Add ErrHandler 2023/08/01
End Function
'20230710: Modified "Warning!!! No vbump to select DCVS Valt before the payload pattern. Please check the pattern set. Warning!!!" and binout. Request by T-bra.
...
'''//==================================================================================================================================================================================//'''
    If flagEnableRailSwitch Then
        ...
        If enable_DecomposePatt = True And (LCase(inst_name) Like "*td*") And currentDcvsOutput = tlDCVSVoltageMain And hasKeywordPayload = True Then
            ...
            For Each site In TheExec.sites
                    'theexec.sites.item(site).FlagState(strGlb_Flag_Vddbinning_Fail_Stop) = logicTrue 'Request by T-bra. 20230710
                    'theexec.sites.item(site).SortNumber = VddbinFailStopSortNumber
                    'theexec.sites.item(site).BinNumber = VddbinFailStopBinNumber
...
'''//==================================================================================================================================================================================//'''
    ...
    For i = 0 To UBound(pinGroup_BinCut)
        ...
        '    Call Print_Error_Message (Error_Info, "LIB_Vdd_Binning_customer", "","site:" & site & ",powerDomain:" & powerPin & ",Efuse category:" & ",it can't get Efuse category to get Efuse resolution for generate_offline_IDS_IGSim_Parallel. Please check Efuse_BitDef_Table or instances of Harvest result update. Error!!!")
        '    'TheExec.ErrorLogMessage "site:" & site & ",powerDomain:" & powerPin & ",Efuse category:" & ",it can't use Efuse category to get Efuse resoultion for generate_offline_IDS_IGSim_Parallel. Please check Efuse_BitDef_Table or instances of Harvest result update. Error!!!"    ''20230807: Removed by sanity check
        'End If
                ...
                If dict_IsCorePowerInBinCutFlowSheet.item(powerPin) = True And Not UCase(powerPin) Like "*VDD*_SRAM*" Then '''CorePower
                    ...
                    For Each site In TheExec.sites
                        ...
                        If got_Efuse_category = True Then
                            ...
                            '''20240411: Assign both Dssc/TrimAte field to overwrite real IDS value during open socket
                            '''20210716: Modified to decide Efuse setReadDecimal or setWriteDecimal for generate_offline_IDS_IGSim_Parallel.
...
'                        Call Print_Error_Message (Error_Info, "LIB_Vdd_Binning_customer", "","Column HarvestOtherPinGrp doesn't exist in header of " & sheetName & ". Error!!!")
'                        'TheExec.ErrorLogMessage "Column HarvestOtherPinGrp doesn't exist in header of " & sheetName & ". Error!!!"    ''20230807: Removed by sanity check
'                    End If
                End If
                Exit For
            End If
        ...
        If enableRowParsing = True Then
            ...
            For i = Row_of_Title + 1 To MaxRow
                ...
                ''20231120: Split harvest/ssn into different structure
                ''SSN pattern is determined by "SSC" and "SSU" keyword
    ...
    Else    ''If isSheetFound = False
        ...
        ''Clear dict for projects without harvest table
    ...
    ''Clear dict for projects without harvest table
    If ssnPatternsDict.Count <> 0 And glb_SSN_Enabled = False Then
        ssnPatternsDict.RemoveAll
...
'20230725 : Modified multifstp to support calculate and *(())*
