...
On Error GoTo errHandler
    ...
    ' Disable for duplicate setting with outside level sheet,240617
''''    TheHdw.DCVS.Pins("SPI_PWR").Connect
...
''''    TheHdw.Wait 0.001
    ...
    If BootUsingPattern Then
      ...
      'TheHdw.Digital.Pins("DFU_STATUS").DisableCompare = True
        ...
        Next i
    Else
        TheHdw.Digital.Pins("OE_N").InitState = chInitLo    'For Level Shifter
        TheHdw.Wait 0.01
...
On Error GoTo errHandler
    ...
    TheHdw.Protocol.ports("UART_TX").Enabled = False
    TheHdw.Protocol.ports("UART_RX").Enabled = False
    theexec.Flow.enableWord("ReloadUART") = False
    Exit Function
errHandler:
...
TheHdw.Digital.Pins(frequency_meas_pins).Levels.value(chVoh) = CDbl((XI0_ref_VOH - XI0_ref_VOL) / 2)
TheHdw.Digital.Pins(frequency_meas_pins).Levels.value(chVol) = CDbl((XI0_ref_VOH - XI0_ref_VOL) / 2)
...
'            g_Reboot_Flag = False
    ...
    TResult_coreup = SendCmd("core up acc;fs mount SPI1;", 0.1)
