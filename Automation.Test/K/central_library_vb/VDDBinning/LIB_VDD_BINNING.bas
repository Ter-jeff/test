            ...
            If VBIN_RESULT(inst_info.p_mode).tested = False Or VBIN_RESULT(inst_info.p_mode).GRADE > 0 Then
                If TheExec.enableWord("Vddbin_PTE_Debug") = True Then
                    If Flag_BinX_Info_Parsed = True Then         ''BinX merge, set last bin1 EQN step
                        If DYNAMIC_VBIN_IDS_ZONE(inst_info.p_mode).step_Mapping(VBIN_RESULT(inst_info.p_mode).passBinCut, AllBinCut(inst_info.p_mode).Last_Bin1_EQN) <> -1 Then
        ...
        Else '''If VBIN_RESULT(AllBinCut(inst_info.p_mode).PREVIOUS_Performance_Mode).tested = False
            If TheExec.enableWord("Vddbin_PTE_Debug") = True Then
                If Flag_BinX_Info_Parsed = True Then
                    If DYNAMIC_VBIN_IDS_ZONE(inst_info.p_mode).step_Mapping(VBIN_RESULT(inst_info.p_mode).passBinCut, AllBinCut(inst_info.p_mode).Last_Bin1_EQN) <> -1 Then         ''BinX merge, set last bin1 EQN step
                    step_inherit = DYNAMIC_VBIN_IDS_ZONE(inst_info.p_mode).step_Mapping(1, AllBinCut(inst_info.p_mode).Last_Bin1_EQN)
                    End If
                ElseIf Flag_BinX_Info_Parsed = False And VBIN_RESULT(inst_info.p_mode).passBinCut = 1 And DYNAMIC_VBIN_IDS_ZONE(inst_info.p_mode).step_Mapping(VBIN_RESULT(inst_info.p_mode).passBinCut, 1) <> -1 Then '''//"Vddbin_PTE_Debug" should use step of Bin1 EQN1 in Dynamic_IDS_Zone.
                    step_inherit = DYNAMIC_VBIN_IDS_ZONE(inst_info.p_mode).step_Mapping(VBIN_RESULT(inst_info.p_mode).passBinCut, 1)
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    Else
        ...
        ''TheExec.ErrorLogMessage performance_mode & " doesn't have the matched definition of Enum p_mode in VddBinStr2Enum. Error!!!"    ''20230807: Removed by sanity check
    End If
Exit Function
errHandler: 'Add ErrHandler 2023/08/01
...
Public Function judge_PF_func(inst_info As Instance_Info, Test_Type As testType, patt_result As SiteBoolean, Optional isPostBV As Boolean = False)
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    Dim site As Variant
        ...
        For Each site In TheExec.sites
            If patt_result(site) = False Or alarmFail(site) = True Then
                ...
                Else
                    ...
                    '''20240710,judge harv result for bin check. if only harv fail then turn on harv flag
                    If inst_info.enable_HarvPinGrp = True Then
                        
                        If inst_info.HarvPinGrpCommnFailResult(site) = True Then
                            If isPostBV = False Then
                                Call BinoutSite(site, BinCut(inst_info.p_mode, VBIN_RESULT(inst_info.p_mode).passBinCut).SBIN_LVCC_FAIL(0, Test_Type), _
                                     BinCut(inst_info.p_mode, VBIN_RESULT(inst_info.p_mode).passBinCut).HBIN_LVCC_FAIL(0, Test_Type), strGlb_Flag_Vddbinning_Fail_Stop)
                            Else
                                TheExec.sites.item(site).FlagState(inst_info.Harv_CommonFailFlag) = logicTrue
                                TheExec.Datalog.WriteComment "site:" & site & "," & inst_info.inst_name & "," & VddBinName(inst_info.p_mode) & ", test failed, it has to update Harvest Common failFlag:" & inst_info.Harv_CommonFailFlag & "=True."
                            End If
                            ...
                            For HarvPatCnt = 0 To UBound(inst_info.HarvPinGrpCheckData)
                                For HarvCnt = 0 To UBound(HarvPinFlagMapping(inst_info.HarvPinGrpCheckData(HarvPatCnt).MappingIndex).HarvPinGrpConditionArr)
                                    If inst_info.HarvPinGrpCheckData(HarvPatCnt).HarvFailResult(HarvCnt) = True Then        '' Check fail result from inst_info 20220304
                                        ...
                                        '' 20240710: Print harvest flag state for datalog
                                        ...
                                        TheExec.Flow.TestLimit 1, 0, 0, _
                                                        Tname:=HarvPinFlagMapping(inst_info.HarvPinGrpCheckData(HarvPatCnt).MappingIndex).HarvPinGrpConditionArr(HarvCnt)
                                                                '' Pin Group
                                        TheExec.Datalog.WriteComment "Site: " & site & " Failed in Bincut check, so harvest flag is assigned true!"
'                                        HarvestSite_in_Bincheck = True
                                    End If
                                Next HarvCnt
                            Next HarvPatCnt
                        End If
...
'                                TheExec.Datalog.WriteComment "site:" & site & "," & inst_info.inst_name & "," & VddBinName(inst_info.p_mode) & ", test failed, so that update Harvest AllCorePass"
...
'
'                                '20240726 BV check and postBV add pattern harvest feature
...
'
'                                Else
'                                    TheExec.sites.item(site).FlagState("") = logicTrue
'                                End If
'                            End If
'                        Else
...
'                        End If
                    Else
                    '''20240710,judge harv result for bin check. if only harv fail then turn on harv flag
                    'If HarvestSite_in_Bincheck = False Then
...
'''//==================================================================================================================================================================================//'''
    ...
    For Each site In TheExec.sites
        ...
        Select Case LCase(bincutJobName)
            ...
            ''TheExec.ErrorLogMessage "site:" & site & ", " & bincutJobName & " is the incorrect BinCut testJob for judge_PF. Error!!!"    ''20230807: Removed by sanity check
        End Select
    ...
    For Each site In TheExec.sites
        If inst_info.HarvestBinningFlag <> "" And inst_info.Harvest_With_BIN1E1 = True _
        And Check_EQN_UpToBinX(inst_info.p_mode, site) And TheExec.sites.item(site).FlagState(strGlb_Flag_HarvestBinningFlag_SingleFSTP) = logicFalse _
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    If inst_info.is_DevChar_Running = False Then
        ...
        Else '''BinCut check.
            judge_PF_func inst_info, inst_info.Test_Type, pattPass
        End If
    End If
Exit Function
errHandler: 'Add ErrHandler 2023/08/01
            ...
            For Row = Row_of_Title + 1 To MaxRow
                If LCase(ws_def.Cells(Row, col_TestType(testType.TD)).value) = "ids range" And LCase(ws_def.Cells(Row, col_TestType(testType.TD) + 1).value) = "start bin" Then '''//Find the row with "IDS Range","Start Bin"
                    ...
                    Else
                        ...
                        ''TheExec.ErrorLogMessage SheetName & " doesn't have any correct Performance_mode in row" & Row & ". Error!!!"    ''20230807: Removed by sanity check
                    End If
                        ...
                        For Test_Type = 0 To testType.Total_Count - 1
                            ...
                            If col_testTypeSelected > 0 Then
                                ...
                                Else
                                    ...
                                    ''TheExec.ErrorLogMessage SheetName & " doesn't have the correct Performance_mode in row" & Row & ", col" & col_TestType(TestType.TD) & " consistent with other TestType columns. Error!!!"    ''20230807: Removed by sanity check
                                End If
                            End If
                        Next Test_Type
                    End If
            ...
            DYNAMIC_VBIN_IDS_ZONE(p_mode).interpolated(site) = True
                ...
                If AllBinCut(p_mode).INTP_SKIPTEST = True Then
                    ...
                    If CurrentPassBinCutNum(site) <> DYNAMIC_VBIN_IDS_ZONE(p_mode).passBinCut(step_Calc) Then
                        CurrentPassBinCutNum(site) = DYNAMIC_VBIN_IDS_ZONE(p_mode).passBinCut(step_Calc)                    '' 20220208 Modify to move Bin 1 -> Bin X when interpolated step is in Bin X
                        '''20210726: Modified to add the argument "bincutNum As Long" to the vbt function Adjust_Multi_PassBinCut_Per_Site.
...
'**********************************************
        For indexPatt = 0 To count_PrePatt_decomposed - 1
            ...
            If (inst_info.enable_HarvPinGrp = True And inst_info.BeSSNTest = False) Then '240703
...
'''//==================================================================================================================================================================================//'''
    ...
    If CLng(site) < 0 Then
        ...
        ''TheExec.ErrorLogMessage "Please check the site number of get_I_VDD_values. Error!!!"    ''20230807: Removed by sanity check
    End If
    ...
    Else
        ...
        ''TheExec.ErrorLogMessage "site:" & site & ",powerDomain:" & powerDomain & ",Efuse category:" & str_IDS_PowerDomain & ", it can't use Efuse category for get_I_VDD_values, Please check Efuse_BitDef_Table. Error!!!"    ''20230807: Removed by sanity check
    End If
Exit Function
errHandler: 'Add ErrHandler 2023/08/01
...
'''//==================================================================================================================================================================================//'''
    ...
    ElseIf strTemp Like "*ft_hot*" Or strTemp Like "*ft2*" Then
    ...
    Else
        ...
        ''TheExec.ErrorLogMessage "getBinCutJobDefinition = " & strInput & " doesn't exist in enum BinCutJobDefinition. Error!!!"    ''20230807: Removed by sanity check
    End If
Exit Function
errHandler: 'Add ErrHandler 2023/08/01
...
'''//==================================================================================================================================================================================//'''
    ...
    If dict_IsCorePowerInBinCutFlowSheet.Exists(UCase(powerDomain)) = True Then
        ...
        Else
            ...
            ''TheExec.ErrorLogMessage "PowerDomain:" & powerDomain & ",it isn't BinCut CorePower for check_voltageInheritance_for_powerDomain. Error!!!"    ''20230807: Removed by sanity check
            Exit Function
        End If
    Else
        Call Print_Error_Message(Error_Info, "LIB_VDD_BINNING", "check_voltageInheritance_for_powerDomain", "PowerDomain:" & powerDomain & ",it isn't BinCut CorePower for check_voltageInheritance_for_powerDomain. Error!!!")
    ...
    For Each site In TheExec.sites
        ...
        For i = 0 To UBound(MG_keys)
            ...
            For j = 0 To UBound(temp_MG_keys) '"MG001,MG101,MG201,MG301"
                ...
                If i = 1 Then                ''First cycle don't compare max
...
'        End If
        ...
        '20230504: Added for parsing sheet defining BinX EQNs and IDS value
        Dim i As Long
        If col_eqn_bin > 0 And col_binx_ids > 0 And col_binx_ftids > 0 Then
            ...
            For Row = Row_of_Title + 1 To MaxRow
                ...
                If ws_def.Cells(Row, col_mode).value Like "M*" And LCase(ws_def.Cells(Row, col_binned).value) = "true" Then
                    ...
                    If ws_def.Cells(Row, col_eqn).value Like "E#*" Then '''read the E1 ~ En
                        ...
                        '''20200824: Modified to check TotalStepPerMode. Revised by Leon Weng.'20230526
                            ...
                            If (ws_def.Cells(Row, col_intSkipTest).value <> "") Then
                                If LCase(ws_def.Cells(Row, col_intSkipTest).value) = "yes" Then
                                    ...
                                    Else
                                        ...
                                        'TheExec.ErrorLogMessage ws_def.Cells(Row, col_mode) & " has interpolation skipTest=yes in row " & Row & ", but interpolation factor doesn't have the correct value in sheet " & sheetName & ". Error!!!"    ''20230807: Removed by sanity check
                                    End If
                                Else
                                    BinCut(main_p_mode, passBinCut).INTP_SKIPTEST(idx_step) = False
                                End If
    ...
    If isSheetFound = True Then
        ...
        If LCase(sheetName) Like LCase("*" & sheetName_OutsideBinCut & "*") Then
            ...
            Else
                ...
                'TheExec.ErrorLogMessage "Non_Binning_Rail should be parsed prior to sheet Non_Binning_Rail_Outside_BinCut"    ''20230807: Removed by sanity check
            End If
        Else
            isParsingOutsideBinCutFlow = False '''//Parsing BinCutFlow (sheet "Non_Binning_Rail")
        End If
        ...
        If TheExec.enableWord("T0TX_HOT") = True Then job_keyword = LCase("T0TX_HOT")   'WWWWW20220631 T0TX
...
'''//==================================================================================================================================================================================//'''
    ...
    If cnt_testJob > 0 Then
        For i = 0 To cnt_testJob - 1
            If LCase(ws_def.Cells(row_of_testJob, col_testJob(i) + 1).value) Like job_keyword Then
                idx_testJob = i
                        ...
                        'For i = 0 To cntVddbinPin - 1
                            For passBinCut = binNumStart To binNumStop
                                ...
                                Else
                                    ...
                                    If testCondition = "" Then '20231017 T0TX modified
                                        Call Print_Error_Message(Error_Info, "LIB_VDD_BINNING", "Parsing_EquationVoltages", "Please check the cell (row " & Row & ", column " & Col & ") of sheet " & sheetName & ". The cell should not be empty. Error!!!!")
                                        MsgBox ("The cell should not be empty. Error!!!!")
                                        'TheExec.ErrorLogMessage "Please check th  e cell (row " & Row & ", column " & Col & ") of sheet " & sheetName & ". The cell contains the incorrect format. Error!!!"    ''20230807: Removed by sanity check
                                    End If
                                    '''==========================================================================================================================================='''
                                    '''//Check if testCondition contain keyword "*Evaluate*Bin*" to decide "is_BinCutJob_for_StepSearch" = True (BinCut stepSearch) while isParsingOutsideBinCutFlow = False.
                                    '''==========================================================================================================================================='''
...
'#End If
            ...
            'powerPinValue = TheHdw.DCVS.pins(PowerPins).Meter.Read(tlStrobe, 10, 10000, tlDCVSMeterReadingFormatAverage)
                    '20240509: Remove meter read and use new syntax PinListData
                    powerPinValue = TheHdw.DCVS.Pins(PowerPins).Voltage.PinListData
        End If
...
'''//==================================================================================================================================================================================//'''
    If inst_info.Test_Type = testType.TD Then
        ...
        If inst_info.MultiFSTP_Enable = True And inst_info.idxDigSrcPattern <= UBound(inst_info.digSrcPatterns) Then
            ...
            'inst_info.idxDigSrcPattern points to the ith MFSTP pattern (in case 1 inst has multi MFSTP patterns)
            If InStr(str_pattern, inst_info.digSrcPatterns(inst_info.idxDigSrcPattern)) Then
...
'            Call Calculate_Harvest_Core_DSSC_Source(inst_info.inst_name, VddBinName(inst_info.p_mode), str_pattern, inst_info.Harvest_Core_DigSrc_Pin, inst_info.Harvest_Core_DigSrc_SignalName, inst_info.Pattern_Pmode, inst_info.By_Mode)
        End If
        Syncup_Dict_boolean = True '20240227
        If Syncup_Dict.Exists(UCase(str_pattern)) Then
            Flag_SyncUp_DCVS_Output_enable = False '20240227
        Else
            Flag_SyncUp_DCVS_Output_enable = True
        End If
       '' TheExec.Datalog.WriteComment "Flag_SyncUp_DCVS_Output_enable = " & Flag_SyncUp_DCVS_Output_enable
        If Flag_Enable_Rail_Switch Then '''For projects with Rail Switch
            '''20190626: As the discussion with TSMC PSYAO, we found vbump didn't exist in all BinCut powerDomain of BinCut patterns.
            ...
            For i = 0 To UBound(selsramLogicPingroup)
                ...
                If UCase(selsramLogicPingroup(i)) <> "PRESERVED" And UCase(selsramLogicPingroup(i)) <> "RESERVED" Then
                    ...
                    Else
                        ...
                        If Flag_SyncUp_DCVS_Output_enable = False Then Exit For '20240227 TTR by T-BraC only do 1st pins
                    End If
                End If
            Next i
            ...
            If Flag_SyncUp_DCVS_Output_enable Then
                If inst_info.previousDcvsOutput <> inst_info.currentDcvsOutput Then       ''20221128: Only do SyncUp when voltage state changed
                    Call SyncUp_DCVS_Output(inst_info.p_mode, inst_info.currentDcvsOutput, SyncUp_PowerPin_Group, str_pattern) '''This is to sync up logic powers and sram powers on the same DCVS output (for TD testing)
                End If
            End If
    ...
    If Not Syncup_Dict.Exists(UCase(str_pattern)) And Syncup_Dict_boolean = True Then '20240227 other pattern
        Syncup_Dict.Add UCase(str_pattern), True
    End If
...
'20201118: Modified to use "TheHdw.Digital.Patgen.PatternBurstPassedPerSite" to get siteResult of pattern pass/fail.
Public Function run_patt_from_FuncPat_for_BinCut(inst_info As Instance_Info, indexPatt As Long, str_pattern As String, funcPatPass As SiteBoolean, instSSNinfo As Inst_SSN, _
...
'''//==================================================================================================================================================================================//'''
    ...
    If Flag_VDD_Binning_Offline = False Then '''If the test mode is Online.
        ...
        If inst_info.enable_HarvPinGrp = True Then
            ''20230907: Modified to use tester type instead of conditional compiling
        ...
        If inst_info.patCnt_HarvPinGrp <> 0 And inst_info.enable_CoreMask = True And Flag_HarvPinFlag_Mapping_Table_Parsed = True And instSSNinfo.bSSNTest = False Then
            ...
            If strAry_PatNameSplit(3) Like "*pl*" Then
                ...
                Next chkloop
            End If
        ElseIf (inst_info.enable_CoreMask = True Or inst_info.enable_HarvPinGrp = True) And Flag_HarvPinFlag_Mapping_Table_Parsed = True And instSSNinfo.bSSNTest = True Then '240702
            strAry_PathSplit = Split(LCase(str_pattern), "\")       '' Pattern name
            strAry_PatNameSplit = Split(LCase(strAry_PathSplit(UBound(strAry_PathSplit))), "_")     '' "in" or "pl"
            If strAry_PatNameSplit(3) Like "*pl*" Then
                    Call SSNConfigSetting(str_pattern, instSSNinfo.bSSNTest, instSSNinfo.bSSNCoreHarvest, instSSNinfo)
            End If
        End If
        ...
        '''//Warning!!! currently "TheHdw.Digital.Patgen.PatternBurstPassedPerSite" doesn't support "result_mode=tlResultModeModule" with PatternBurst=Yes and DecomposePatt=No.
        'T-Col TTR approve by Si -- 230413
        '20230824: Added back for getting correct site result after pattern test, remove other API calling
        ...
        If inst_info.enable_HarvPinGrp = True Then
            ...
            If strAry_PatNameSplit(3) Like "*pl*" Then
                ...
                If instSSNinfo.bSSNCoreHarvest = False Then
                For chkloop = 0 To UBound(inst_info.HarvPinGrpCheckData)
                    If LCase(strAry_PathSplit(UBound(strAry_PathSplit))) Like LCase(inst_info.HarvPinGrpCheckData(chkloop).keyword) Then
                        ...
                        For Each site In TheExec.sites
                            ...
                            If inst_info.is_BinSearch = True And Flag_BinX_Info_Parsed = True Then
                                ...
                                End If
                            ElseIf inst_info.is_BinSearch = True And Flag_BinX_Info_Parsed = False Then
                                If inst_info.HarvPinGrpCommnFailResult(site) = False And HarvFailResultOr = True And inst_info.Step_Current = DYNAMIC_VBIN_IDS_ZONE(inst_info.p_mode).step_Mapping(VBIN_RESULT(inst_info.p_mode).passBinCut, 1) And inst_info.Harvest_With_BIN1E1 = True Then
                                    inst_info.All_Site_Mask = inst_info.All_Site_Mask - 2 ^ site
                                    inst_info.grade_found(site) = False
                                    TheExec.Datalog.WriteComment "Site: " & site & " harvest on EQN = " & DYNAMIC_VBIN_IDS_ZONE(inst_info.p_mode).EQ_Num(inst_info.Step_Current) & ", so force result to EQN = " & DYNAMIC_VBIN_IDS_ZONE(inst_info.p_mode).EQ_Num(inst_info.Step_Current) & "!!!"
                                End If
                            Else
                                'Do nothing
                            End If
                ...
                If isHarvPat = False Then
                    ...
                    Next
                End If
                ElseIf instSSNinfo.bSSNCoreHarvest = True Then
                        Dim SSN_InterposeFunc As InterposeName ': SSN_InterposeFunc = "LogPinCoreFailsBySite"
                        SSN_Body instSSNinfo, True, True, str_pattern, , , SSN_InterposeFunc  ', SSN_ScanPins
                        SSN_PostBody False
                End If
            Else
                '20230824: Added back for getting correct site result after pattern test, remove other API calling
...
Public Function SyncUp_DCVS_Output(p_mode As Integer, currentDcvsOutput As Integer, powerGroup As String, Optional str_pattern As String)
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    Dim PinGroup As String
...
'''//==================================================================================================================================================================================//'''
    ...
    If powerGroup <> "" Then
        ...
        For i = 0 To UBound(split_powerGroup)
                ...
                If cnt_DecomposedPinList > 0 Then
                    For j = 0 To cnt_DecomposedPinList - 1
                        ...
                        Else
                            ...
                            ''TheExec.ErrorLogMessage strAry_pinSyncup(j) & " of " & powerDomain & " in " & powerGroup & " doesn't exist in PinMap or ChannelMap. Error!!!"    ''20230807: Removed by sanity check
                        End If
                    Next j
                Else
            ...
            If currentDcvsOutput = tlDCVSVoltageAlt Then
                '20240227
                If TheHdw.DCVS.Pins(powerDomain).Voltage.Output = tlDCVSVoltageAlt Then ' cost a lot of times
                    '/Good
                Else
                    TheExec.Datalog.WriteComment split_powerGroup(i) & "Pins without Vbump , please check pattern. Warning!!!" ' Don't add in Dic
                    Syncup_Dict_boolean = False
                    'Exit Function
                End If
            Else
                If TheHdw.DCVS.Pins(powerDomain).Voltage.Output = tlDCVSVoltageMain Then ' cost a lot of times
                    '/Good
                Else
                    TheExec.Datalog.WriteComment split_powerGroup(i) & "Pins without Vbump , please check pattern. Warning!!!" ' Don't add in Dic
                    Syncup_Dict_boolean = False
    ...
    Else
        ...
        ''TheExec.ErrorLogMessage "The argument powerGroup for SyncUp_DCVS_Output is empty. Error!!!"    ''20230807: Removed by sanity check
    End If
...
errHandler: 'Add ErrHandler 2023/08/01
    ...
    If AbortTest Then Exit Function Else Resume Next 'Add ErrHandler 2023/08/01
End Function
'20230720: Modified to let FLEX and PLUS both used meter.read
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    Dim restore_Voltage_Output_PinListData As New PinListData
    Dim restore_Meter_mode_PinListData
    Dim PinArray() As String
    Dim idx_PinArray As Long
    ...
    For i = 0 To UBound(pinGroup_BinCut)
        ...
        If powerPin <> "" Then
            ...
            'restore_Voltage_Output_PinListData.AddPin powerPin 'Restore DCVS 20230720
            'restore_Meter_mode_PinListData.AddPin powerPin     'Restore meter mode 20230720
...
'        restore_Voltage_Output_PinListData = TheHdw.DCVS.Pins(TempString_Pinlist).Voltage.Output 'Restore DCVS 20230720
'        'restore_Meter_mode_PinListData = TheHdw.DCVS.pins(TempString_Pinlist).Meter.mode
...
'    End If
    ...
    For i = 0 To UBound(pinGroup_BinCut)
        ...
        End If
    Next i
'    TheHdw.DCVS.Pins(TempString_Pinlist).Voltage.Output = restore_Voltage_Output_PinListData
    'TheHdw.DCVS.pins(TempString_Pinlist).Meter.mode = restore_Meter_mode_PinListData
    'TheHdw.DCVS.pins(TempString_Pinlist).Voltage.Output = tlDCVSVoltageMain 'Restore DCVS 20230720
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    Else
        ...
        ''TheExec.ErrorLogMessage "Vddbin Domain=" & vddbinDomain & ", but it doesn't exist in VddbinDomain2Pin. Error!!!"    ''20230807: Removed by sanity check
    End If
Exit Function
errHandler: 'Add ErrHandler 2023/08/01
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    Else
        ...
        ''TheExec.ErrorLogMessage "Pin:" & vddbinPin & ", but it doesn't exist in the dictionary pin2domainDict. Please check BinCut voltage table and DC specs sheets for VddbinPin2Domain. Error!!!"    ''20230807: Removed by sanity check
    End If
Exit Function
errHandler: 'Add ErrHandler 2023/08/01
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    Else
        ...
        ''TheExec.ErrorLogMessage "Pin:" & powerPin & ", it is not a correct powerPin for IsDcvsConnected. Error!!!"    ''20230807: Removed by sanity check
    End If
Exit Function
errHandler: 'Add ErrHandler 2023/08/01
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    Else
        ...
        ''TheExec.ErrorLogMessage "Pin:" & vddbinPin & ", it doesn't exist in VddbinPinDcvsType. Error!!!"    ''20230807: Removed by sanity check
    End If
Exit Function
errHandler: 'Add ErrHandler 2023/08/01
    ...
    If AbortTest Then Exit Function Else Resume Next 'Add ErrHandler 2023/08/01
End Function
'20230620: Modified Get1stPinFromPingroup when first pin get N/C pin , Bypass it
'20200106: Modified to remove the ErrorLogMessage.
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    Else
        ...
        ''TheExec.ErrorLogMessage "powerDomain: " & powerDomain & ", it is incorrect for Get1stPinFromPingroup. Error!!!"    ''20230807: Removed by sanity check
    End If
Exit Function
errHandler: 'Add ErrHandler 2023/08/01
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    For i = 0 To UBound(split_content)
        If Not DCspecTable.Exists(split_content(i) & "_VAR") Then
            ...
            ''TheExec.ErrorLogMessage "Pin:" & split_content(i) & "_VAR" & ", it doesn't exist in DCspec. Error!!!"    ''20230807: Removed by sanity check
        End If
    Next i
    ...
    If Adjust_Max_Enable = True Then
        ...
        For j = 0 To UBound(group_array)
            ...
            If UBound(Max_array) < 1 Then
                ...
                ''TheExec.ErrorLogMessage "Adjust_Power_Max pin list of adjust_VddBinning is less than 2"    ''20230807: Removed by sanity check
            End If
    ...
    If Adjust_Min_Enable = True Then
        ...
        For j = 0 To UBound(group_array)
            ...
            If UBound(Min_array) < 1 Then
                'TheExec.ErrorLogMessage "Adjust_Power_Min pin list of adjust_VddBinning is less than 2"    ''20230807: Removed by sanity check
            End If
...
'''//==================================================================================================================================================================================//'''
    If Flag_Skip_ReApplyPayloadVoltageToDCVS = True Then
        ...
        End If
    ElseIf TheExec.enableWord("T0TX_ROOM") = True Or TheExec.enableWord("T0TX_HOT") = True Then 'WWWWW
        Call TheExec.DataManager.GetInstanceContext(DCCategory, DCSelector, ACCategory, ACSelector, TimeSetSheet, EdgeSetSheet, LevelsSheet, Overlay)
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    For Each site In TheExec.sites
        If inst_info.grade_found(site) = False Then
            ...
            '''20230926: Added to save non-harvest pattern result in harvest instance
            If inst_info.enable_HarvPinGrp = True And inst_info.HarvStartWithBin1(site) = True And inst_info.BeSSNTest = False Then
            ...
            End If
        End If
        If inst_info.grade_found(site) = True And inst_info.enable_HarvPinGrp = True And inst_info.BeSSNTest = True Then
            VBIN_RESULT(inst_info.p_mode).step_in_IDS_Zone = DYNAMIC_VBIN_IDS_ZONE(inst_info.p_mode).step_Mapping(VBIN_RESULT(inst_info.p_mode).passBinCut, 1)
            VBIN_RESULT(inst_info.p_mode).step_in_BinCut = DYNAMIC_VBIN_IDS_ZONE(inst_info.p_mode).EQ_Num(VBIN_RESULT(inst_info.p_mode).step_in_IDS_Zone) - 1
            VBIN_RESULT(inst_info.p_mode).GRADE = DYNAMIC_VBIN_IDS_ZONE(inst_info.p_mode).Voltage(VBIN_RESULT(inst_info.p_mode).step_in_IDS_Zone)
            VBIN_RESULT(inst_info.p_mode).GRADEVDD = DYNAMIC_VBIN_IDS_ZONE(inst_info.p_mode).Product_Voltage(VBIN_RESULT(inst_info.p_mode).step_in_IDS_Zone)
...
'''//==================================================================================================================================================================================//'''
    ...
    If inst_info.enable_DecomposePatt = False Then '''without decomposing pattern sets
        ...
        '''20201117: Modified to use "tlResultModeDomain" for pattern burst=Yes and decomposePatt=No. Requested by Leon Weng.
        '''20230710: Modified to use real result_mode from TestInst
        '''20230718: Modified to set domain mode for UF
        If glb_TesterType = "UltraFLEXplus" Then
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    If pattSet <> "" Then
        ...
        If count_patt_decompose > 0 Then
            ...
            If Flag_Vddbin_PatternBurst_PrintOut = True And inst_info.result_mode = tlResultModeDomain Then
                If BurstYesPatDict.Exists(LCase(CStr(pattSet))) Then
                    TheExec.Datalog.WriteComment "----Start printing pattern name under Burst=Yes with domain mode, EnableWord 'Vddbin_PatternBurst_PrintOut' = True----"
                    TheExec.Datalog.WriteComment "PatternSet = " & CStr(pattSet)
                End If
            End If
            ...
            For i = 0 To count_patt_decompose - 1
                ...
                For idxBlock = 0 To UBound(SelsramMapping)
                    If LCase(SelsramMapping(idxBlock).Pattern) <> "*" And LCase(ary_patt_decompose(i)) Like LCase(SelsramMapping(idxBlock).Pattern) Then ''20220803 Wyatt dssc modify
...
'''ToDo: Maybe it can use pattern-loop to find DSSC DigSrc pattern of Harvest Core here...
                ...
                If Flag_Vddbin_PatternBurst_PrintOut = True And inst_info.result_mode = tlResultModeDomain Then
                    If BurstYesPatDict.Exists(LCase(CStr(pattSet))) Then
                        'TheExec.Datalog.WriteComment "Print pattern name under Burst=Yes with domain mode:"
                        TheExec.Datalog.WriteComment ary_patt_decompose(i)
                    Else
                        ''Cannot find burst pattern or result mode != domain
                    End If
                End If
            Next i
            ...
            If Flag_Vddbin_PatternBurst_PrintOut = True And inst_info.result_mode = tlResultModeDomain Then
                If BurstYesPatDict.Exists(LCase(CStr(pattSet))) Then
                    TheExec.Datalog.WriteComment "----End printing pattern name under Burst=Yes with domain mode, EnableWord 'Vddbin_PatternBurst_PrintOut' = True----"
                End If
            End If
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    Else
        ...
        ''TheExec.ErrorLogMessage "Vddbin Domain=" & vddbinDomain & ", " & vddbinDomain & " doesn't exist in dictDomain2DcSpecGrp. Error!!!"    ''20230807: Removed by sanity check
    End If
Exit Function
errHandler: 'Add ErrHandler 2023/08/01
...
'''//==================================================================================================================================================================================//'''
    '''//Check if alarmFail(site) is triggered or not.
    ''20230823: Remove checking IGXL version condition, requeste by Verity
    ''20230824: Added UF for using OnAlarmOccurred alarmfail array
    If glb_TesterType = "UltraFLEXplus" Then
        Dim alarmOccurred As New SiteBoolean
        alarmOccurred = False
...
'    Next idx_powerDomain
    ...
    '''20210819: Modified to move the vbt code about resetting globalVariables of BinCut testCondition from the vbt function initVddBinCondition to the vbt function Reset_BinCut_GlobalVariable_for_initVddBinning.
'    For p_mode = 0 To MaxPerformanceModeCount   '0~60 '20230629 Dynamic initial
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    If performance_mode <> "" Then
        ...
        inst_info.BeSSNTest = False '240703
        inst_info.Harv_CommonFailFlag = vbNullString
        '20230602
        'inst_info.selsrm_DigSrc_SignalOrder = 0
                ...
                For p_mode = 0 To MaxPerformanceModeCount - 1
                    If AllBinCut(p_mode).powerPin = powerDomain Then
                        If sram_mode_cnt > 0 Then
                            If Power_Seq_Tmp <> mid(Replace(VddBinName(p_mode), powerDomain & "_", ""), 1, 4) Then
                        ...
                        Power_Seq_Tmp = mid(Power_Seq(sram_mode_cnt), 1, 4)
    ...
    If LCase(str_selector) = "read" Or LCase(str_selector) = "write" Then
        ...
        If dict_strPmode2EfuseCategory.Exists(UCase(str_keyword_EfuseCategory)) = True Then
            ...
            For i = 0 To UBound(strAry_temp_EfuseCategory)
                ...
                Else
                    str_Efuse_temp = vbNullString
                    'Call Print_Error_Message (Error_Info, "LIB_VDD_BINNING", "","BinCut_testJob:" & bincutJobName & ",keyword_EfuseCategory:" & str_keyword_EfuseCategory & ", it doesn't have any correct programming stage to get the Efuse category for get_Efuse_category_by_BinCut_testJob. Please check Efuse_BitDef_Table. Error!!!")
                    ''TheExec.ErrorLogMessage "BinCut_testJob:" & bincutJobName & ",keyword_EfuseCategory:" & str_keyword_EfuseCategory & ", it doesn't have any correct programming stage to get the Efuse category for get_Efuse_category_by_BinCut_testJob. Please check Efuse_BitDef_Table. Error!!!"    ''20230807: Removed by sanity check
                End If
            Next i
        Else
            str_Efuse_temp = vbNullString
        ...
        If AllBinCut(p_mode).PREVIOUS_Performance_Mode = AllBinCut(p_mode).Allow_Equal And AllBinCut(p_mode).Allow_Equal <> 0 Then '''for AllowEqual
            ...
            'Or (CDec(VBIN_RESULT(p_mode).GRADE) < CDec(grade_last) And exit_while_flag = False)
                ...
                If idx_step > AllBinCut(p_mode).Mode_Step Then
                    '''20210610: Modified to revise the message for the vbt function find_next_bin_eq_interpolation if it can't find any step to define the product voltage.
'                    Call Print_Error_Message (Error_Info, "LIB_VDD_BINNING", "","The Performance Mode " & VddBinName(p_mode) & " EQ 1 Product Value is wrong. Error!!!")
...
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    ...
    ElseIf Flag_HarvPinFlag_Mapping_Table_Parsed = True Then        ''20220712: HarvPinGrp Table exist
        ...
        If inst_info.BeSSNTest = False Then
                For i = 0 To UBound(inst_info.ary_FuncPat_decomposed) - 1
...
Public Function CtrlFailFlag_byHarvPinGrpResult(inst_info As Instance_Info)
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
    Dim site As Variant
    Dim HarvCnt As Long, HarvPatCnt As Long
    If inst_info.enable_HarvPinGrp = True Then
        For Each site In TheExec.sites
            ...
            If inst_info.HarvPinGrpCommnFailResult(site) = True Then
                ...
                TheExec.Datalog.WriteComment "site:" & site & "," & inst_info.inst_name & "," & VddBinName(inst_info.p_mode) & ", test failed, it has to update Harvest Common failFlag:" & inst_info.Harv_CommonFailFlag & "=True."
                '''//Shut down the failed site. As per discussion with Chihome, he suggested us to ensure that Sort Number/Bin Number/fail-stop should be updated before .result = tlResultFail.
                'TheExec.sites.item(site).Result = tlResultFail '20240802
            Else
                For HarvPatCnt = 0 To UBound(inst_info.HarvPinGrpCheckData)
                    For HarvCnt = 0 To UBound(HarvPinFlagMapping(inst_info.HarvPinGrpCheckData(HarvPatCnt).MappingIndex).HarvPinGrpConditionArr)
...
Public Function CtrlFailFlag_ByPatternHarvResult(inst_info As Instance_Info)
On Error GoTo errHandler 'Add ErrHandler 2023/08/01
...
Exit Function
errHandler:
    Call Print_Error_Message(Error_Info, "LIB_VDD_BINNING", "CtrlFailFlag_ByPatternHarvResult") 'Add ErrHandler 2023/08/01
...
End Sub
Public Function CheckInstForSSN_BV(PatArray() As String, inData As Inst_SSN, SSNMapping() As ssnMappingType, _
                                Optional CoreHarvFlag As Boolean = False, Optional CoreMaskFlag As Boolean = False, Optional isCharInst As Boolean = False)

On Error GoTo errHandler
Dim funcName As String:: funcName = "CheckInstForSSN_BV"
    Dim i As Long
    Dim Pat As Variant
...
'    End If
     If CoreHarvFlag = False And CoreMaskFlag = False Then Exit Function
    ...
    If CoreHarvFlag = True Then isSSNHarvestInst = True
    If CoreMaskFlag = True Then isSSNCoreMaskInst = True
