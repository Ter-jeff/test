...
namespace CommonLib.Extension
{
    public static class ListExtensions
    {
        ...
        {
            private class ChunkItem(TSource tSource)
            {
                public readonly TSource Value = tSource;
            ...
            public Chunk(TKey tKey, IEnumerator<TSource> tSources, Func<TSource, bool> predicate)
            {
                Key = tKey;
                _enumerator = tSources;
                _predicate = predicate;

                // A Chunk always contains at least one element.
                _head = new ChunkItem(tSources.Current);
        ...
        public static IEnumerable<IGrouping<TKey, TSource>> ChunkBy<TSource, TKey>(this IEnumerable<TSource> tSources, Func<TSource, TKey> keySelector)
        {
            return tSources.ChunkBy(keySelector, EqualityComparer<TKey>.Default);
        }

        public static IEnumerable<IGrouping<TKey, TSource>> ChunkBy<TSource, TKey>(this IEnumerable<TSource> tSources, Func<TSource, TKey> keySelector, IEqualityComparer<TKey> tKeys)
        {
            // Flag to signal end of source sequence.
            const bool noMoreSourceElements = true;

            // Auto-generated iterator for the source array.       
            IEnumerator<TSource> enumerator = tSources.GetEnumerator();
            ...
            while (true)
            {
                ...
                var current =
                    new Chunk<TKey, TSource>(key, enumerator, value => tKeys.Equals(key, keySelector(value)));
                ...
                // returned only when the client code foreach over this chunk. See Chunk.GetEnumerator for more info.
